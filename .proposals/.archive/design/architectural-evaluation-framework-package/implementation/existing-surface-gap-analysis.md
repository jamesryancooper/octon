...
Determine whether Octon already contains a like-for-like architecture
...
|---|---|---|---|
| `audit-domain-architecture` skill | External architecture critique for a Octon domain | Domain-only, optimized for external robustness criteria rather than governed architecture-readiness scoring and promotion gates | Keep separate; optionally reuse as supplemental evidence |
...
| `evaluate-harness` workflow | Structure and token-efficiency review of a `.octon` directory | Meta structure evaluator, not a governed autonomous engineering architecture audit | Do not extend |
...
Octon does not already contain a like-for-like architecture-readiness
