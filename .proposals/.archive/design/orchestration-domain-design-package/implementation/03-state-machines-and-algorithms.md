...
4. reconciler chooses same-executor resume or `abandon_and_escalate`
5. update run and incident as needed

Fail-closed:
- no new side-effectful execution on same key until recovery resolves

## Resolved Algorithm Ambiguities

- none for v1 routing or recovery semantics; remaining open choices are
  transport and storage implementation details
