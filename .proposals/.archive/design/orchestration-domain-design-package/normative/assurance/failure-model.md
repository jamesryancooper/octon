...
| running run with expired heartbeat | mark liveness failure, preserve lineage, and require deterministic recovery with same-executor resume or abandonment only |
...
V1 recovery does not transfer ownership to a different executor. If the
original executor cannot safely resume, the run is abandoned and containment or
operator review must take over.
