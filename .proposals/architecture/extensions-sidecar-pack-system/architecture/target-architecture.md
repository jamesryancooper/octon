...
- `/.octon/engine/governance/**` remains authoritative for precedence,
  fail-closed behavior, and trust boundaries.
- `/.octon/engine/_ops/state/extensions/effective/**` holds derived effective
...
`/.octon/engine/governance/` must define:
...
`/.octon/engine/runtime/` must implement:
...
- compilation of effective catalogs and artifact maps under
  `/.octon/engine/_ops/state/extensions/effective/`
...
`/.octon/capabilities/runtime/` must support pack fragments for:
...
`/.octon/assurance/runtime/` must provide:
...
- how Octon recognizes `/.extensions/`
...
- scaffolding support under `/.octon/scaffolding/` for creating new extension
  packs with the approved directory shape
- Octon-managed commands or workflows under
  `/.octon/orchestration/runtime/workflows/` and
  `/.octon/capabilities/runtime/commands/` for validating, enabling, and
