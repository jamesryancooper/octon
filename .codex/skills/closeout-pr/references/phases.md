...
2. Resolve the target lifecycle outcome when provided or clearly inferable:
   `published`, `ready`, `landed`, `cleaned`, `blocked`, `escalated`, or
   `denied`. Ask before ready, merge, cleanup, or sync actions when the target
   is ambiguous.
3. Inspect tracked, unstaged, and untracked files with `git status` and
   `git diff`.
4. Define the intended task-scoped file set.
5. If unrelated changes are present and cannot be separated credibly, stop and
   report the blocker.
