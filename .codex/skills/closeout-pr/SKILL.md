...
This skill accepts an optional PR number/URL, optional target lifecycle
outcome, optional commit/PR metadata, and file-scope controls. When invoked
directly from an existing PR context, resolve whether the target is `published`,
`ready`, `landed`, `cleaned`, or blocker reporting before requesting ready,
merge, cleanup, or sync actions.
