...
- aligned decision evidence and durable run evidence with their continuity-owned
  storage boundaries
- reorganized the package into stable navigation, normative, contracts,
  conformance, implementation, reference, and history modules
- fixed the remaining routing semantics by codifying severity ordering and
  `source_ref_globs` behavior
- fixed the remaining recovery ambiguity by limiting v1 recovery to
  same-executor resume or abandonment
- added semantic conformance scenarios and a package-local evaluator
...
- [ ] V1 recovery allows only same-executor resume or `abandon_and_escalate`;
      it does not transfer execution to a different executor.
...
- [ ] `conformance/validate_scenarios.py` passes for routing, scheduling, and
      recovery suites.
- [ ] No active package document depends on pre-reorg flat-root paths for
      normative material.
