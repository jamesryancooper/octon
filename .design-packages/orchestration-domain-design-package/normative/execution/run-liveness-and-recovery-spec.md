...
- resume with the same executor if the original `executor_id` re-acknowledges
  ownership and any required coordination is still valid before further side
  effects begin
- mark the run `abandoned` and require containment or operator review

V1 does not transfer ownership to a different executor during recovery.
...
- the same `executor_id` finalizes the run before the run is marked
  `abandoned`

Late terminal updates from an executor are rejected after the reconciler marks a
run `abandoned`.

## Abandonment Rule

When the reconciler selects `abandoned`, it must:

- preserve the existing run lineage and evidence pointers
- open or enrich an incident when policy requires containment
- keep the coordination key blocked from new side effects until containment or
  explicit override resolves the situation
- avoid fabricating success, cancellation, or ownership transfer
