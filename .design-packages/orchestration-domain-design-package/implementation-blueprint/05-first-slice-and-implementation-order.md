...
| 10 | Expand validator/runtime conformance tests | all prior | static and runtime enforcement coverage exists |

## Open Residual Ambiguities

| Ambiguity | Risk | Temporary Default | Must Resolve Before Coding? |
|---|---|---|---|
| Severity ordering semantics | medium | `info < warning < high < critical` | yes |
| `source_ref_globs` matching dialect | medium | POSIX-style glob match | yes |
| Reconciler choice policy for resume vs transfer vs abandon | medium | conservative default: abandon unless same executor resumes | no, if policy documents default |
| Concrete transport between launcher and executor | low | internal async queue or RPC | no |
| Storage backend | low | choose any backend meeting CAS/consistency guarantees | no |
