...
- A functional `TraceWriter` exists at `.harmony/engine/runtime/crates/core/src/trace.rs`.
- It writes NDJSON trace events to `.harmony/engine/_ops/state/traces/<trace_id>.ndjson`.
...
- The server enforces a max line length (default 1 MiB) in `.harmony/engine/runtime/crates/core/src/jsonlines.rs`.
...
- Prebuilt runtime execution currently ships:
  - `.harmony/engine/_ops/bin/harmony-macos-arm64`
- `.harmony/engine/_ops/bin/harmony-macos-x64`
