...
> Notes:
> - The CLI binary is `harmony` (kernel crate). For development, use `.harmony/engine/runtime/run ...` which falls back to `cargo run`.
...
```bash
.harmony/engine/runtime/run serve-stdio
```
...
Edit policy file `.harmony/engine/runtime/config/policy.yml` to remove `storage.local` from `interfaces/kv`, then invoke `kv.get`.
...
```bash
.harmony/engine/runtime/run studio --help
```
