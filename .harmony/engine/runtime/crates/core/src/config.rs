...
impl ConfigLoader {
    pub fn load(harmony_dir: &Path) -> Result<RuntimeConfig> {
        ...
        let state_dir = harmony_dir.join("engine").join("_ops").join("state");
        ...
        let cache_config_path = harmony_dir
            .join("engine")
            .join("runtime")
            .join("config")
    ...
    /// - If `.harmony/harmony.yml` contains `engine.runtime.policy_file`, use that (relative to `.harmony/`).
    /// - Else, if `.harmony/engine/runtime/config/policy.yml` exists, use that.
