...
#[derive(Parser, Debug)]
#[command(name = "harmony-assurance")]
#[command(about = "Harmony assurance score and gate tools")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
struct ScoreArgs {
    ...
    #[arg(long, default_value = ".harmony/output/assurance/effective")]
    effective_dir: PathBuf,

    #[arg(long, default_value = ".harmony/output/assurance/results")]
    results_dir: PathBuf,
...
fn default_out_dir(now: &TimestampMeta) -> PathBuf {
    ...
    PathBuf::from(format!(
        ".harmony/output/assurance/scorecards/{}/{}",
        day, now.run_id
    ))
}
