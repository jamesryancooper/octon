...
VALIDATE_SCRIPT=".harmony/assurance/runtime/_ops/scripts/validate-continuity-memory.sh"
...
create_fixture_repo() {
  ...
  mkdir -p \
    ...
    "$fixture_root/.harmony/continuity/_meta/architecture/schemas" \
    "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01" \
    "$fixture_root/.harmony/continuity/runs"

  cp "$REPO_ROOT/.harmony/assurance/runtime/_ops/scripts/validate-continuity-memory.sh" \
    "$fixture_root/.harmony/assurance/runtime/_ops/scripts/validate-continuity-memory.sh"
  cp "$REPO_ROOT/.harmony/continuity/_meta/architecture/schemas/decision-record.schema.json" \
    "$fixture_root/.harmony/continuity/_meta/architecture/schemas/decision-record.schema.json"

  cat > "$fixture_root/.harmony/continuity/tasks.json" <<'EOF'
{
  "schema_version": "1.2",
  ...
  "tasks": [
    {
      ...
      "knowledge_links": {
        ...
        "contracts": [
          ".harmony/continuity/_meta/architecture/schemas/decision-record.schema.json"
        ],
        "decisions": [
          "dec-20260308-weekly-freshness-audit-allow-01"
        ],
        "evidence": [
          ".harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
        ]
      }
    }
  ]
...
EOF
  ...
  cat > "$fixture_root/.harmony/continuity/entities.json" <<'EOF'
{
  "schema_version": "1.1",
...
EOF
  ...
  cat > "$fixture_root/.harmony/continuity/next.md" <<'EOF'
# Next
...
EOF
  ...
  cat > "$fixture_root/.harmony/continuity/decisions/README.md" <<'EOF'
# Continuity Decisions

Routing, authority, and prerequisite decision evidence.
EOF

  cat > "$fixture_root/.harmony/continuity/decisions/retention.json" <<'EOF'
{
  "schema_version": "1.0",
...
EOF
  ...
  cat > "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json" <<'EOF'
{
...
EOF
  ...
  cat > "$fixture_root/.harmony/continuity/runs/README.md" <<'EOF'
# Continuity Runs

Execution receipts, digests, and related run evidence.
EOF

  cat > "$fixture_root/.harmony/continuity/runs/retention.json" <<'EOF'
{
  "schema_version": "1.0",
  "default_action": "retain",
  "classes": [
...
case_missing_decision_json_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  rm "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
  run_validator_in_fixture "$fixture_root"
}
...
case_invalid_outcome_fails() {
  ...
  fixture_root="$(create_fixture_repo)"
  jq '.outcome = "deny"' \
    "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json" \
    > "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.tmp"
  mv \
    "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.tmp" \
    "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
  run_validator_in_fixture "$fixture_root"
...
case_stray_file_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  touch "$fixture_root/.harmony/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/extra.txt"
  run_validator_in_fixture "$fixture_root"
}
