...
MANIFEST_FILE="$HARMONY_DIR/harmony.yml"
...
main() {
  ...
  elif [[ ! -f "$HARMONY_DIR/$migration_workflow" ]]; then
    fail "migration_workflow target missing: .harmony/$migration_workflow"
  else
    pass "migration workflow path resolves: .harmony/$migration_workflow"
  fi

  if [[ -z "$migration_overview" ]]; then
    fail "missing versioning.harness.migration_overview"
  elif [[ ! -f "$HARMONY_DIR/$migration_overview" ]]; then
    fail "migration_overview target missing: .harmony/$migration_overview"
  else
    pass "migration overview path resolves: .harmony/$migration_overview"
  fi
