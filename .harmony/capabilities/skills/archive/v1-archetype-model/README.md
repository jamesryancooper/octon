...
│     □ Choose archetype: Atomic (no refs) / Complex (5+ refs)                │
│                                                                             │
...
|:-----------------------------|:-----------------------------------------------------------|:-----------------------------------------|
| **Atomic**                   | None                                                       | Single-purpose, obvious I/O              |
| **Atomic (with examples)**   | `examples.md` only                                         | Single-purpose, output needs demo        |
| **Complex**                  | Core: io-contract, safety, examples, behaviors, validation | Multi-phase execution                    |
...
| Question | Yes → | No → |
|----------|-------|------|
| Can you explain the skill in one sentence? | Consider Atomic | Continue ↓ |
| Is the output format non-obvious or would examples help? | Atomic (with examples) | Atomic |
| Do phases need documentation for correct execution? | Complex | Atomic or Atomic (with examples) |
| Are there safety boundaries or escalation rules? | Complex | Atomic or Atomic (with examples) |
| Does the skill require domain-specific terminology? | Complex + `glossary.md` | Complex |
| Are there complex error recovery procedures? | Complex + `errors.md` | Complex |

**Decision Examples:**

- **Format JSON** → Atomic (single-purpose, no phases, obvious I/O)
- **Summarize Text** → Atomic (with examples) (single-purpose, but output format benefits from demonstration)
- **Refine Prompt** → Complex (10 phases, context analysis, safety boundaries)
- **Financial Audit** → Complex + optional files (terminology glossary, compliance rules, audit trail requirements)

The template includes **Complex** archetype files. See [reference-artifacts.md](../../docs/architecture/workspaces/skills/reference-artifacts.md) for details.
