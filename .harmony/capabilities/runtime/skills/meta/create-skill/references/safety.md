...
#
# Current allowed-tools: Read Glob Grep Write(/.harmony/capabilities/runtime/skills/*) Write(_ops/state/runs/*) Write(_ops/state/logs/*) Bash(mkdir) Bash(ln) Bash(cp)
#
...
- `Write(/.harmony/capabilities/runtime/skills/*)` — Create new skill directory and files
...
|------|------------|---------|
| `.harmony/capabilities/runtime/skills/{{new-skill}}/` | Create files | New skill directory |
| `.harmony/capabilities/runtime/skills/manifest.yml` | Append entry | Register skill |
| `.harmony/capabilities/runtime/skills/registry.yml` | Append entry | Add metadata |
| `.harmony/catalog.md` | Append row | Add to catalog table |
| `.harmony/capabilities/runtime/skills/_ops/state/runs/create-skill/` | Create files | Execution state (session recovery) |
| `.harmony/capabilities/runtime/skills/_ops/state/logs/create-skill/` | Create/update files | Run logs and indexes |
| `.harmony/capabilities/runtime/skills/_ops/state/logs/index.yml` | Update | Top-level log index |
...
2. No directory `.harmony/capabilities/runtime/skills/{{skill_name}}/` exists
...
1. Delete skill directory:
   ```bash
   rm -rf .harmony/capabilities/runtime/skills/{{skill_name}}/
   ```

2. Remove manifest entry:
   - Edit `.harmony/capabilities/runtime/skills/manifest.yml`
   - Remove entry with `id: {{skill_name}}`

3. Remove registry entry:
   - Edit `.harmony/capabilities/runtime/skills/registry.yml`
...
6. (Optional) Delete execution state and logs:
   ```bash
   rm -rf .harmony/capabilities/runtime/skills/_ops/state/runs/create-skill/{{run_id}}/
   rm .harmony/capabilities/runtime/skills/_ops/state/logs/create-skill/{{run_id}}.md
   ```
