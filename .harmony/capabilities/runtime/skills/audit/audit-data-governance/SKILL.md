...
```text
/audit-data-governance scope=".harmony"
```

With explicit baseline references:

```text
/audit-data-governance scope=".harmony" classification_baseline_ref=".harmony/assurance/practices/standards/data-handling-and-retention.md" privacy_baseline_ref=".harmony/assurance/practices/standards/security-and-privacy.md"
```

With stricter threshold:

```text
/audit-data-governance scope=".harmony/capabilities/runtime/services" severity_threshold="high"
```
...
- `.harmony/output/reports/analysis/YYYY-MM-DD-data-governance-audit-<run-id>.md` -- Structured findings report
- `.harmony/output/reports/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `_ops/state/logs/audit-data-governance/` -- Execution logs with index
