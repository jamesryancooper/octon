...
```text
/audit-api-contract scope=".harmony/capabilities/runtime/services/interfaces"
```

With explicit baseline references:

```text
/audit-api-contract scope=".harmony" contract_baseline_ref=".harmony/cognition/governance/principles/contract-first.md" api_design_baseline_ref=".harmony/scaffolding/governance/patterns/api-design-guidelines.md"
```

With stricter threshold:

```text
/audit-api-contract scope=".harmony/capabilities/runtime/services/interfaces" severity_threshold="high"
```
...
- `.harmony/output/reports/analysis/YYYY-MM-DD-api-contract-audit-<run-id>.md` -- Structured findings report
- `.harmony/output/reports/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `_ops/state/logs/audit-api-contract/` -- Execution logs with index
