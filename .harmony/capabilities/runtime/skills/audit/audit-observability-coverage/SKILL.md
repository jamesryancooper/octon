...
```text
/audit-observability-coverage scope=".harmony/capabilities/runtime/services"
```

Use an explicit contract reference:

```text
/audit-observability-coverage scope=".harmony/capabilities/runtime/services" observability_contract_ref=".harmony/cognition/_meta/architecture/observability-requirements.md"
```

Narrow discovery behavior:

```text
/audit-observability-coverage scope=".harmony/capabilities/runtime/services" service_manifest_glob="**/SERVICE.md" severity_threshold="high"
```
...
- `.harmony/output/reports/analysis/YYYY-MM-DD-observability-coverage-audit-<run-id>.md` -- Structured findings report
