...
```text
/release-readiness-audit scope=".harmony"
```

With explicit baseline references:

```text
/release-readiness-audit scope=".harmony" release_baseline_ref=".harmony/cognition/practices/methodology/ci-cd-quality-gates.md" operations_baseline_ref=".harmony/engine/practices/incident-operations.md"
```

With stricter threshold:

```text
/release-readiness-audit scope=".harmony/capabilities/runtime/services" severity_threshold="high"
```
...
- `.harmony/output/reports/YYYY-MM-DD-release-readiness-audit-<run-id>.md` -- Structured findings report
- `.harmony/output/reports/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `_ops/state/logs/release-readiness-audit/` -- Execution logs with index
