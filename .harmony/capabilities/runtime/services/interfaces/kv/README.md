...
Local key/value storage service for Harmony runtime state. This interface
...
- `rust/wit/world.wit` - `harmony:runtime@1.0.0` world contract used by generated bindings.
- `.harmony/capabilities/runtime/services/manifest.runtime.yml` - Tier 1 discovery (`interfaces/kv`).
- `.harmony/capabilities/runtime/services/registry.runtime.yml` - Tier 2 metadata (owner, risk, state path).
...
From `.harmony/capabilities/runtime/services/interfaces/kv/rust/`:
...
```text
target/wasm32-wasip1/release/harmony_kv_service.wasm
```
