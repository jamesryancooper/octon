...
This is the harness-native quick reference for the production services under `.harmony/capabilities/runtime/services`.
...
```bash
echo '{"content":"example"}' | .harmony/capabilities/runtime/services/governance/guard/impl/guard.sh
```
...
echo '{"promptId":"workflow/implement","variables":{"target":".harmony"}}' | \
  .harmony/capabilities/runtime/services/modeling/prompt/impl/prompt.sh
```
...
echo '{"operation":"estimate","workflowType":"code-from-plan","tier":"T2"}' | \
  .harmony/capabilities/runtime/services/operations/cost/impl/cost.sh
```
...
bash .harmony/capabilities/runtime/services/_ops/scripts/validate-service-independence.sh
```
