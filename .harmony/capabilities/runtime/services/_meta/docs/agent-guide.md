...
**Harmony-aligned.** Agents provide judgment; services provide deterministic capabilities. The Agent layer is a thin, swappable policy that sequences service calls, enforces budgets, and emits concise decision telemetry (no raw chain-of-thought).
...
For canonical responsibilities and runtime boundaries, see also `.harmony/capabilities/runtime/services/execution/service-roles.md`.
