...
SERVICE_DIR_SNAPSHOT="$HARMONY_DIR/capabilities/runtime/services/interfaces/filesystem-snapshot"
SERVICE_DIR_DISCOVERY="$HARMONY_DIR/capabilities/runtime/services/interfaces/filesystem-discovery"
SERVICE_DIR_WATCH="$HARMONY_DIR/capabilities/runtime/services/interfaces/filesystem-watch"
COMMANDS_MANIFEST="$HARMONY_DIR/capabilities/runtime/commands/manifest.yml"
COMMANDS_DIR="$HARMONY_DIR/capabilities/runtime/commands"
...
required_files=(
  ...
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/build-filesystem-interfaces-benchmark-fixture.sh"
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/download-filesystem-interfaces-slo-history.sh"
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-integration.sh"
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-slo.sh"
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-perf-regression.sh"
  "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/tune-filesystem-interfaces-slo-budgets.sh"
...
if ! bash "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-determinism.sh" >/dev/null 2>&1; then
  echo "ERROR: filesystem interfaces determinism regression"
  errors=$((errors + 1))
fi

# Runtime integration regression check.
if ! bash "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-integration.sh" >/dev/null 2>&1; then
...
if [[ "$validate_slo" == "1" ]]; then
  ...
  for attempt in 1 2; do
    if bash "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-slo.sh" \
        --profile ci \
        --no-report \
...
if [[ "$validate_perf" == "1" ]]; then
  ...
  perf_ok=0
  for attempt in 1 2; do
    if bash "$HARMONY_DIR/capabilities/runtime/services/_ops/scripts/test-filesystem-interfaces-perf-regression.sh" \
        --profile ci \
        --no-report \
