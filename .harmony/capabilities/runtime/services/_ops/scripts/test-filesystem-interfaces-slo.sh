...
SLO_BUDGETS="$HARMONY_DIR/capabilities/runtime/services/interfaces/filesystem-snapshot/contracts/slo-budgets.tsv"
...
if [[ -z "$report_path" && "$emit_report" == "1" ]]; then
  report_path="$HARMONY_DIR/output/reports/analysis/$(date +%F)-filesystem-interfaces-slo-report.md"
fi
