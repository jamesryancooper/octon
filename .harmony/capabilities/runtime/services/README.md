...
- Shell entrypoints enforce runtime policy via `_ops/scripts/enforce-deny-by-default.sh`
  which delegates to the shared `harmony-policy` engine.
...
- Required runtime context (defaults applied for local/dev runs):
  - `HARMONY_AGENT_ID`
  - `HARMONY_AGENT_IDS` (comma-separated distinct agents for quorum checks)
  - `HARMONY_RISK_TIER` (`low|medium|high`)
- Tier-specific controls:
  - `medium`: `HARMONY_REVIEW_AGENT_ID`, `HARMONY_ROLLBACK_PLAN_ID`
  - `high`: `HARMONY_REVIEW_AGENT_ID`, `HARMONY_ROLLBACK_PLAN_ID`,
    `HARMONY_QUORUM_TOKEN`
...
.harmony/capabilities/_ops/scripts/policy-profile-resolve.sh tests --emit-grant \
