#!/usr/bin/env bash
# test-filesystem-graph-slo.sh - latency/error SLO gate across filesystem-graph ops.

set -o pipefail

HARMONY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
RUNTIME_RUN="$HARMONY_DIR/runtime/run"
FIXTURE_BUILDER="$HARMONY_DIR/capabilities/services/_ops/scripts/build-filesystem-graph-benchmark-fixture.sh"
SLO_BUDGETS="$HARMONY_DIR/capabilities/services/interfaces/filesystem-graph/contracts/slo-budgets.tsv"
...
bootstrap_out="$("$RUNTIME_RUN" tool interfaces/filesystem-graph snapshot.build --json "$bootstrap_payload" 2>&1)"
...
while IFS=$'\t' read -r op budget_samples budget_p95_ms max_error_rate; do
  ...
  for attempt in $(seq 1 "$run_samples"); do
    ...
    start_ms="$(now_ms)"
    if raw_out="$("$RUNTIME_RUN" tool interfaces/filesystem-graph "$op" --json "$payload" 2>&1)"; then
