#!/usr/bin/env bash
# build-filesystem-graph-benchmark-fixture.sh - generate deterministic benchmark tree.

set -o pipefail

HARMONY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
REPO_ROOT="$(cd "$HARMONY_DIR/.." && pwd)"
PROFILE_FILE="$HARMONY_DIR/capabilities/services/interfaces/filesystem-graph/fixtures/benchmark-profile.tsv"
...
if [[ -z "$fixture_root" ]]; then
  run_id="$(date +%s)-$$"
  fixture_root=".harmony/output/.tmp/filesystem-graph-bench/${profile}-${run_id}"
fi
