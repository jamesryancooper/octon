#!/usr/bin/env bash
# tune-filesystem-graph-slo-budgets.sh - tighten filesystem-graph SLO budgets from history window.

set -o pipefail

HARMONY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
budgets_file="$HARMONY_DIR/capabilities/services/interfaces/filesystem-graph/contracts/slo-budgets.tsv"
history_dir=""
min_runs="6"
headroom_percent="25"
