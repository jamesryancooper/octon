...
maybe_auto_rollback() {
  ...
  if [[ "${HARMONY_ENABLE_AUTO_ROLLBACK:-false}" != "true" ]]; then
    printf '%s\n' "auto-rollback skipped (enable with HARMONY_ENABLE_AUTO_ROLLBACK=true)" >> "$log_file"
    return 0
  fi
...
cmd_run() {
  ...
  local scope="global"
  local owner="${HARMONY_AGENT_ID:-agent-local}"
