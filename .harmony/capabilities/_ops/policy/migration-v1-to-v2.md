...
- Remove parser/evaluator drift by centralizing decisions in `harmony-policy`.
...
- Runtime command-scope checks in v1 shell guard map to `harmony-policy enforce`
...
1. `harmony-policy doctor` passes for v2 policy and governance files.
2. `harmony-policy preflight/enforce` parity tests pass against v1 wrappers.
