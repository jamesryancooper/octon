...
MARKER="HARMONY AUTONOMY CLEANUP HOOK"
...
Removes managed Harmony post-merge/post-checkout cleanup hooks.
...
uninstall_hook() {
  ...
  local backup_path="${hooks_dir}/${hook_name}.harmony-local"
  ...
  if ! rg -q "${MARKER}" "${hook_path}"; then
    info "${hook_name} hook is not managed by Harmony; leaving as-is."
    return 0
  fi
...
main() {
  ...
  echo "[OK] Uninstall flow completed for Harmony autonomy hooks in ${hooks_dir}."
}
