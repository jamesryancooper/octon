...
POLICY_FILE="$REPO_ROOT/.harmony/agency/practices/standards/ci-latency-policy.json"
DEFAULT_OUT_DIR="$REPO_ROOT/.harmony/output/reports"
...
cargo run --quiet \
  --manifest-path "$REPO_ROOT/.harmony/engine/runtime/crates/Cargo.toml" \
  -p harmony_assurance_tools \
  -- ci-latency analyze \
  --policy "$POLICY_FILE" \
  --runs "$runs_file" \
  --jobs "$jobs_file" \
