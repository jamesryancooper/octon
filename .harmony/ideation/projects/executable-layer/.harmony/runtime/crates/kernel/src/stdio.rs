...
use harmony_core::jsonlines::{read_json_line, write_json_line};
use harmony_core::registry::{ServiceKey};
use harmony_core::trace::TraceWriter;
use harmony_wasm_host::cancel::CancelHandle;
...
pub fn serve_stdio(ctx: Arc<KernelContext>) -> anyhow::Result<()> {
    ...
    if msg_type != "hello" || protocol != "harmony-stdio-v1" {
        let err = KernelError::new(
            ErrorCode::ProtocolUnsupported,
            "unsupported or missing protocol (expected harmony-stdio-v1)",
        )
    ...
    loop {
        ...
        match method {
            "tool.invoke" => {
                ...
                let service_name = params.get("service").and_then(|v| v.as_str()).unwrap_or("");
                let category = params
                    .get("category")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
