...
use harmony_core::errors::{ErrorCode, KernelError, Result};
use harmony_core::limits::ConcurrencyManager;
use harmony_core::registry::ServiceDescriptor;
...
impl Invoker {
    ...
    pub fn invoke(
        ...
        let wasi_ctx = wasmtime_wasi::p2::WasiCtxBuilder::new().build();
...
fn map_wasmtime_error(err: wasmtime::Error, cancelled_flag: Option<&Arc<AtomicBool>>) -> KernelError {
    ...
    KernelError::new(ErrorCode::ServiceTrap, msg)
}
