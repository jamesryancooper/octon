...
**Original Prompt:** I want to set up a plugins/extensions packs feature that allows creating cross-system packages that enable specialization and leverage Harmony domains and surfaces (for instance, a Next.js package would expand on Harmony with skills, workflows, etc., specialized to the needs of Next.js development). How should we design this plugin/extension architecture to make it plug-and-play, support multiple extension packs, and leverage and expand Harmony's native domains and surfaces without modifying them?
...
You are a principal systems architect working inside the Harmony codebase. Be pragmatic, contract-first, and explicit about tradeoffs. Optimize for the smallest robust extension model that preserves Harmony's governance, portability, and bounded-surface architecture.

## Repository Context

- Harmony is an agent-first, system-governed harness organized into bounded domains and surfaces.
- Relevant architectural references:
  ...
  - `.harmony/capabilities/runtime/skills/platforms/provider-nextjs-astro-runtime/SKILL.md`
- Harmony already uses progressive disclosure and canonical runtime/governance/practices surfaces. Any extension model must work with those patterns rather than bypass them.

## Intent

Explore and recommend the best architecture for a new Harmony extension-pack system that allows optional specialization bundles, such as a Next.js pack, to add value across existing Harmony domains without requiring pack-specific modifications to core surfaces after the extension mechanism exists.

## Requirements

1. Treat extension packs as optional adapter layers that compose with Harmony rather than forks of Harmony.
...
4. Preserve canonical authority of Harmony's native runtime, governance, and practices surfaces.
...
6. Explain how packs can leverage and expand native Harmony domains without mutating core artifacts in place or relying on weak implicit overrides.
...
9. Use a concrete example of a Next.js pack that adds framework-specific skills, workflows, templates, and guidance while remaining subordinate to Harmony core contracts.
10. If the design requires one-time Harmony core changes to introduce stable extension points, call that out explicitly and separate those foundational changes from the long-term plug-and-play pack lifecycle.

## Assumptions Made

- This request is for architecture exploration, not implementation.
- "Without modifying them" means future packs should not require direct edits to Harmony's native domains and surfaces once the extension mechanism is in place.
- The preferred design should reuse existing Harmony patterns such as manifest or registry discovery, bounded surfaces, and deny-by-default governance rather than introducing a parallel framework unless clearly justified.

## Negative Constraints (What NOT To Do)

- Do not propose a design that requires direct edits to Harmony core for every new pack.
...
1. Summarize the current Harmony architectural constraints and natural extension points.
...
- Preserves Harmony's bounded surfaces, progressive-disclosure discovery, and governance model.
...
- Quality: grounded in actual Harmony surfaces and existing specialization patterns.
...
You are a principal systems architect working inside the Harmony codebase. Analyze how Harmony should support a new extension-pack system that allows optional specialization bundles, such as a Next.js pack, to contribute framework-specific capabilities across Harmony without turning packs into core forks.

Use the existing Harmony architecture as the baseline, especially these references:
...
3. Leverages and expands Harmony's native domains and surfaces without requiring pack-specific edits to core surfaces once the extension mechanism exists.
4. Preserves Harmony's canonical authority model, bounded surfaces, progressive-disclosure discovery, portability, and deny-by-default governance.

In your response:

1. Identify the natural extension points and constraints in the current Harmony architecture.
2. Present 2-3 viable architecture options for extension packs.
3. Compare those options across:
   ...
   - fit with Harmony's existing manifest, registry, and runtime patterns
...
6. Explain how packs can extend Harmony behavior without silently overriding canonical runtime or governance authority.
...
8. Show a concrete Next.js pack example that adds framework-specific skills, workflows, templates, and guidance while remaining subordinate to Harmony core contracts.
9. Call out any one-time foundational changes Harmony itself would need in order to support this model, and distinguish those from the future pack authoring workflow.
10. End with a minimal implementation plan for the recommended design.

Do not:

- propose a design that requires modifying Harmony core for every new pack
