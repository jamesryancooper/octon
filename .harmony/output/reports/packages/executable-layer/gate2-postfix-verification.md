...
Test method:
1. Back up `.harmony/runtime/config/policy.yml`
2. Remove `storage.local` grant for `interfaces/kv`
3. Run:

```bash
.harmony/runtime/run tool kv put --json '{"key":"deny-test","value":"1"}'
```
...
```bash
.harmony/runtime/run service new agent-platform slow
```

2. Set timeout in:
`/.harmony/capabilities/services/agent-platform/slow/service.json`
...
`/.harmony/capabilities/services/agent-platform/slow/rust/src/lib.rs`

4. Built service:

```bash
.harmony/runtime/run service build agent-platform slow
```

5. Invoked:

```bash
.harmony/runtime/run tool slow ping --json '{"message":"timeout"}'
```
...
- `/.harmony/runtime/crates/wasm_host/src/invoke.rs`
...
- `cargo check --workspace` in `/.harmony/runtime/crates` passes.
- `cargo component check` in `/.harmony/capabilities/services/interfaces/kv/rust` passes.
