...
- Audited artifacts:
  ...
  - All `SKILL.md` files under `.harmony/capabilities/skills/` (34 files)
...
- Warnings: 12 files use comma-delimited `allowed-tools` strings (e.g., `Read, Grep, Glob, ...`) while Harmony docs define space-delimited format (`docs/architecture/harness/skills/skill-format.md:90`) and validator parser tokenizes on spaces (`.harmony/capabilities/skills/scripts/validate-skills.sh:337-339`).
...
- `capabilities.yml` defines `foundations` members as foundation wrapper IDs (`python-api-foundation`, etc.) and `platforms` member `vercel-foundation` (`.harmony/capabilities/skills/capabilities.yml:80-88`).
- Manifest uses operational skills like `react-best-practices`, `react-composition-patterns`, `react-native-best-practices`, `postgres-best-practices`, `vercel-deploy` (`.harmony/capabilities/skills/manifest.yml:160-307`).
...
- This breaks the stated capability→reference contract in `capabilities.yml` (`.harmony/capabilities/skills/capabilities.yml:130-147`).
...
- Drift risk: `validate-skills.sh` currently assumes flat `SKILLS_DIR/$skill_id` directories and fails immediately against grouped paths (e.g., `synthesis/...`) (`.harmony/capabilities/skills/scripts/validate-skills.sh:1085-1089`, `.harmony/capabilities/skills/scripts/validate-skills.sh:1147`).
...
- Outdated internal paths:
  - `README.md` points to `.harmony/capabilities/skills/refactor/` and `.harmony/capabilities/skills/refine-prompt/`, but real paths are grouped (`quality-gate/refactor/`, `synthesis/refine-prompt/`) (`docs/architecture/harness/skills/README.md:156-157`).
  - `creation.md` references `.harmony/capabilities/skills/create-skill/` instead of `meta/create-skill/` (`docs/architecture/harness/skills/creation.md:18`).
