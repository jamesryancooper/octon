...
Scope: integrate four approved controls into Harmony as an enforceable stack:
...
- `harmony-platform`: cross-domain sequencing, cutover authority, rollback authority
...
```bash
bash .harmony/assurance/runtime/_ops/scripts/alignment-check.sh --profile harness,agency,workflows,weights,intent-layer
# Alignment check summary: errors=0

bash .harmony/assurance/runtime/_ops/scripts/assurance-gate.sh --help >/dev/null
...
bash .harmony/assurance/runtime/_ops/scripts/alignment-check.sh --profile harness,agency,workflows,weights
```
...
1. `.harmony/engine/runtime/spec/intent-contract-v1.schema.json`
...
```bash
test -f .harmony/engine/runtime/spec/intent-contract-v1.schema.json && \
...
1. `.harmony/engine/runtime/spec/policy-interface-v1.md`
2. `.harmony/engine/runtime/config/policy-interface.yml`
...
1. `.harmony/engine/runtime/spec/policy-receipt-v1.schema.json`
2. `.harmony/engine/runtime/spec/policy-digest-v1.md`
...
rg -n "intent|boundary|workflow mode|classification" .harmony/engine/runtime/spec/policy-digest-v1.md
```
...
1. `.harmony/capabilities/_ops/scripts/policy-receipt-write.sh`
...
1. `.harmony/agency/governance/delegation-boundaries-v1.yml`
2. `.harmony/agency/governance/delegation-boundaries-v1.schema.json`
...
```bash
test -f .harmony/agency/governance/delegation-boundaries-v1.yml && \
...
rg -n "delegation-boundaries-v1" .harmony/agency/governance/DELEGATION.md
```
...
1. `.harmony/capabilities/governance/policy/deny-by-default.v2.yml`
2. `.harmony/engine/runtime/config/policy.yml`
...
rg -n "BOUNDARY_UNRESOLVED|BOUNDARY_BLOCKED|BOUNDARY_ESCALATION_REQUIRED|allow|escalate|block" \
  ...
  .harmony/engine/runtime/config/policy.yml
```
...
1. `.harmony/orchestration/governance/capability-map-v1.yml`
2. `.harmony/orchestration/governance/capability-map-v1.schema.json`
...
```bash
test -f .harmony/orchestration/governance/capability-map-v1.yml && \
jq -e '.type=="object" and .properties and .properties.workflows' \
  .harmony/orchestration/governance/capability-map-v1.schema.json && \
rg -n "agent-ready|agent-augmented|human-only" .harmony/orchestration/governance/capability-map-v1.yml
```
...
2. `.harmony/orchestration/runtime/workflows/registry.yml`
3. `.harmony/orchestration/governance/README.md`
...
1. `.harmony/capabilities/governance/policy/deny-by-default.v2.yml`
2. `.harmony/engine/runtime/spec/policy-interface-v1.md`
...
1. `.harmony/assurance/runtime/_ops/scripts/validate-intent-layer.sh`
2. `.harmony/assurance/runtime/_ops/scripts/alignment-check.sh`
...
```bash
bash .harmony/assurance/runtime/_ops/scripts/validate-intent-layer.sh && \
...
1. `.harmony/assurance/governance/scores/scores.yml`
2. `.harmony/assurance/governance/weights/weights.yml`
3. `.harmony/assurance/governance/CHARTER.md`
...
bash .harmony/assurance/runtime/_ops/scripts/compute-assurance-score.sh --help >/dev/null
```
...
1. `.harmony/cognition/runtime/decisions/044-intent-contract-and-boundary-enforcement.md`
2. `.harmony/cognition/runtime/decisions/045-capability-map-and-alignment-drift-gates.md`
3. `.harmony/cognition/runtime/decisions/index.yml`
...
```bash
test -f .harmony/cognition/runtime/decisions/044-intent-contract-and-boundary-enforcement.md && \
test -f .harmony/cognition/runtime/decisions/045-capability-map-and-alignment-drift-gates.md && \
...
1. `.harmony/continuity/log.md`
2. `.harmony/continuity/tasks.json`
3. `.harmony/continuity/next.md`
...
2. Optional rollback state: `.harmony/capabilities/_ops/state/rollout-mode.state`
