...
Harmony runtime executable discovery (`manifest.runtime.yml` / `registry.runtime.yml`) is wasm-only,
...
- Update `.harmony/capabilities/services/manifest.runtime.yml`
- Update `.harmony/capabilities/services/registry.runtime.yml`
- Update `.harmony/runtime/config/policy.yml` capability grants for `interfaces/filesystem-graph`

4. Compatibility and evidence updates:
- Add build/validation evidence under `.harmony/runtime/_meta/evidence/`
...
5. Run `harmony validate` and runtime policy tests.
6. Keep shell entrypoint as fallback until parity verification is signed off.

## Exit Gate

1. `harmony validate` passes with runtime tiers enabled.
