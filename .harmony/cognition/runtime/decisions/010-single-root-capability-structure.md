...
description: Consolidate .harmony/ and .workspace/ into a single .harmony/ root organized by cognitive capability
date: 2026-02-07
status: accepted
---
...
The workspace uses a two-root structure: `.harmony/` for portable framework definitions and `.workspace/` for project-specific state. This split organizes by deployment boundary (portable vs local), but humans navigate by cognitive function ("where are decisions?" not "is this portable?").

Pain points with the two-root approach:

- Constant "is this in .harmony or .workspace?" lookup overhead
- Capabilities split across roots (skills definitions in one, skills state in the other)
- The `.harmony`/`.workspace` names don't communicate what's inside
- Portability — the primary reason for the split — is a rare operation (once per new repo) while navigation is constant

## Decision

Consolidate into a single `.harmony/` root organized by capability category. Handle portability via a `harmony.yml` manifest instead of directory structure.

```
.harmony/
├── harmony.yml          # Portability, autonomy, resolution rules
...
| Commit | Scope |
|--------|-------|
| 1. Scaffold | Create `harmony.yml`, new directories, this ADR |
