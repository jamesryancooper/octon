...
The harness previously used a top-level `/.harmony/runtime/` domain for executable runtime artifacts. The repository is moving to explicit bounded surfaces (`runtime`, `governance`, `practices`) where that separation is materially present and independently owned.
...
Adopt `/.harmony/engine/` as the top-level domain and enforce these canonical surfaces:

- `/.harmony/engine/runtime/` for executable authority
- `/.harmony/engine/governance/` for normative runtime policy/contracts
- `/.harmony/engine/practices/` for operating standards
- `/.harmony/engine/_ops/` for mutable operational assets
- `/.harmony/engine/_meta/` for architecture/evidence documentation

Remove `/.harmony/runtime/` in the same migration and update validators/CI references to fail on legacy reintroduction.
