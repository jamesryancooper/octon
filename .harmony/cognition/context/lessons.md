...
| Skipping progress updates | Breaks session continuity | Always update `progress/log.md` before session end |
| Deep nesting in workflows | Hard to follow, agents lose track | Keep workflows to 3-7 steps, use sub-workflows |
| Explanatory prose in agent files | Wastes tokens, agents need actions | Move rationale to `.scratchpad/` or `docs/`, keep agent content terse |
...
3. Reference the relevant task or session in `progress/log.md`
