...
Confirm that the request can produce one valid standard-governed design package
under `/.design-packages/`.

## Actions

1. Validate `package_id` against `^[a-z][a-z0-9-]*$`.
2. Validate `package_class` as `domain-runtime` or `experience-product`.
3. Parse `implementation_targets` as one or more repo-relative durable paths.
4. Resolve target package path:
   `.design-packages/<package_id>/`
