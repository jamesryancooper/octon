...
This guide walks you through adding a new API endpoint using Harmony's AI-assisted workflow.
...
```bash
harmony spec "GET /api/users/:id returns user profile"
...
```bash
harmony spec "GET /api/users/:id returns user profile with name, avatar, and bio"
```
...
harmony ship user-profile-endpoint

# Enable for yourself first
harmony flag enable feature.user-profile-endpoint --scope internal

# Test in production, then enable for all
harmony flag enable feature.user-profile-endpoint
```
...
```bash
harmony spec "POST /api/orders creates a new order with items array"
```
...
```bash
harmony spec "PATCH /api/users/:id/profile updates user bio and avatar"
```
...
```bash
harmony spec "DELETE /api/users/:id/data removes all user data (GDPR)"
```
...
harmony spec "GET /api/products returns product list (public)"

# Requires auth (default assumption for user data)
harmony spec "GET /api/me returns current user"
```

### Pagination

```bash
harmony spec "GET /api/orders returns paginated order list with cursor-based pagination"
```
...
# Add a constraint
harmony spec refine --context "also include created_at timestamp"
```
...
```bash
# Request changes
harmony refine user-profile-endpoint --context "add email field for admin users only"
...
harmony spec "CRUD for /api/widgets with name, description, price"
```
...
```bash
harmony spec "GET /api/users/:userId/orders returns orders for a user"
```

### Webhooks

```bash
harmony spec "POST /api/webhooks/stripe handles Stripe webhook events"
```
...
harmony tier-up "#142" --reason "touches user authentication"

# Bump down if simpler
harmony tier-down "#142" --reason "public endpoint, no auth"
```
...
```bash
# Public
harmony spec "GET /api/products (public, no auth required)"

# User's own data
harmony spec "GET /api/me/settings (returns current user's settings)"

# Admin only
harmony spec "GET /api/admin/users (admin role required)"
```
...
```bash
# Minimal - AI infers
harmony spec "GET /api/users/:id returns user"

# Explicit - more control
harmony spec "GET /api/users/:id returns { id, name, avatar, bio, createdAt }"
```

### Include Error Cases

```bash
harmony spec "GET /api/orders/:id returns order, 404 if not found, 403 if not owner"
```
