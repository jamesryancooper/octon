...
description: "Run release-readiness-audit as mandatory release-core stage."
---
...
1. Invoke release core skill:
   ...
   ```text
   /release-readiness-audit scope="{{scope}}" severity_threshold="{{severity_threshold}}" post_remediation="{{post_remediation}}" convergence_k="{{convergence_k}}" seed_list="{{seed_list}}"
   ```
...
- `.harmony/output/reports/YYYY-MM-DD-release-readiness-audit-<run-id>.md`
