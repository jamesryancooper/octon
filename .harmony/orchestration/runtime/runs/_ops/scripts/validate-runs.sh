...
validate_run_record() {
  ...
  [[ -n "$decision_id" && -f "$HARMONY_DIR/continuity/decisions/$decision_id/decision.json" ]] && pass "run '$run_id' decision link resolves" || fail "run '$run_id' decision link missing"
  [[ -n "$continuity_run_path" && -d "$HARMONY_DIR/${continuity_run_path#.harmony/}" ]] && pass "run '$run_id' continuity path resolves" || fail "run '$run_id' continuity path missing"
