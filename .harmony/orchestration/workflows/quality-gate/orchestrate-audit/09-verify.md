...
- [ ] If `run_cross_subsystem=true`: cross-subsystem report exists or stage failure is documented
- [ ] If `run_freshness=true`: freshness report exists or stage failure is documented
- [ ] Fix batches are actionable
- [ ] Idempotency metadata recorded (manifest hash, partition plan hash, file count, stage parameters)
...
2. Document verification results:
   ...
   | Findings from successful partitions | K/K included | PASS |
   ...
   | Cross-subsystem stage (if enabled) | Completed/Skipped/Failed+documented | PASS |
   | Freshness stage (if enabled) | Completed/Skipped/Failed+documented | PASS |
...
**Marker:** `checkpoints/orchestrate-audit/09-verify.complete`
