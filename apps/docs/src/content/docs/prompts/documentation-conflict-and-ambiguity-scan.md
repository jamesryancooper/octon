...
3) **Monorepo boundaries**: Enforce ESLint boundaries from `eslint.config.js` and TS path aliases from `tsconfig.base.json` (`@domain/*`, `@adapters/*`, `@contracts/*`, `@ui/*`, `@config/*`, `@infra/*`). Flag any guidance that suggests imports violating: `domain` (no deps); `adapters` → `domain|contracts|config`; `ui` → `config`; `app` → `domain|adapters|contracts|ui|config`.
