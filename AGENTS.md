...
- **Capabilities Schema:** `.harmony/skills/capabilities.yml` (skill sets, valid capabilities, refs)
...
3. For validation/expansion, read `.harmony/skills/capabilities.yml` (skill sets, capabilities, refs)
4. After matching, read `registry.yml` for extended metadata (commands, parameters, context)
5. Load `SKILL.md` when a skill is activated (includes `allowed-tools` for tool permissions)
6. Load `references/` or `scripts/` only if needed
