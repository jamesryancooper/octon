...
design material into standalone live authority under `/.octon/orchestration/`.
...
2. Preserve continuity ownership.
   - Durable decision evidence stays under `/.octon/continuity/decisions/`.
   - Durable run evidence stays under `/.octon/continuity/runs/`.
...
1. Every orchestration-domain implementation PR must cite:
   ...
   - if the change originated from temporary design material, record that
     ...
     canonical `/.octon/orchestration/` files
