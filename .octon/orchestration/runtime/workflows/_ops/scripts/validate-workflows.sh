...
check_guide_drift() {
  ...
  for id in "${!WORKFLOW_PATHS[@]}"; do
    ...
    generated="$tmp_root/.octon/orchestration/runtime/workflows/$rel_path/README.md"
    if [[ ! -f "$generated" ]]; then
...
check_legacy_workflow_authoring_references_absent() {
  local targets=(
    ...
    "$OCTON_DIR/cognition/runtime/context/workflow-quality.md"
    "$OCTON_DIR/cognition/runtime/context/workflow-gaps.md"
  )
  local target
  local stale_checks=(
...
check_workflow_system_audit_static() {
  ...
  if bash "$WORKFLOW_SYSTEM_AUDIT" --mode ci-static --scope ".octon/orchestration/runtime/workflows/"; then
