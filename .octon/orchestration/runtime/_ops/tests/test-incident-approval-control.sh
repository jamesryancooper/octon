...
VERIFY_SCRIPT=".octon/orchestration/runtime/_ops/scripts/verify-approval-artifact.sh"
INCIDENT_SCRIPT=".octon/orchestration/runtime/_ops/scripts/manage-incident.sh"
...
create_fixture() {
  ...
  mkdir -p "$fixture_root/.octon/orchestration/governance"
  mkdir -p "$fixture_root/.octon/continuity/decisions/approvals"
  mkdir -p "$fixture_root/.octon/continuity/runs"

  cp -R "$REPO_ROOT/.octon/orchestration/runtime" "$fixture_root/.octon/orchestration/"
  cp "$REPO_ROOT/.octon/orchestration/governance/incidents.md" "$fixture_root/.octon/orchestration/governance/incidents.md"
  cp "$REPO_ROOT/.octon/orchestration/governance/approver-authority-registry.json" "$fixture_root/.octon/orchestration/governance/approver-authority-registry.json"
  ...
  cp "$REPO_ROOT/.octon/continuity/decisions/approvals/README.md" "$fixture_root/.octon/continuity/decisions/approvals/README.md"
...
write_approval() {
  ...
  cat > "$fixture_root/.octon/continuity/decisions/approvals/${approval_id}.json" <<EOF
{
...
case_valid_close_requires_approval() {
  ...
  [[ -f "$fixture_root/.octon/orchestration/runtime/incidents/inc-test-001/closure.md" ]]
  yq -r '.status' "$fixture_root/.octon/orchestration/runtime/incidents/inc-test-001/incident.yml" | grep -qx "closed"
}
