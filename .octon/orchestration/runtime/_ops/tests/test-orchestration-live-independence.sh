...
VALIDATE_SCRIPT=".octon/orchestration/runtime/_ops/scripts/validate-orchestration-live-independence.sh"
...
create_fixture_repo() {
  ...
  mkdir -p \
    "$fixture_root/.octon/orchestration/runtime/_ops/scripts" \
    "$fixture_root/.octon/orchestration/runtime/workflows/meta/create-design-proposal" \
    "$fixture_root/.octon/orchestration/runtime/workflows/meta/create-migration-proposal" \
    "$fixture_root/.octon/orchestration/practices" \
    "$fixture_root/.octon/orchestration/runtime/queue"

  cp "$REPO_ROOT/$VALIDATE_SCRIPT" \
    "$fixture_root/.octon/orchestration/runtime/_ops/scripts/validate-orchestration-live-independence.sh"

  cat > "$fixture_root/.octon/orchestration/practices/workflow-authoring-standards.md" <<'EOF'
# Workflow Authoring Standards

`/.proposals/` may inform work, but must never be a live dependency.
EOF

  cat > "$fixture_root/.octon/orchestration/runtime/workflows/manifest.yml" <<'EOF'
workflows:
...
EOF
  ...
  cat > "$fixture_root/.octon/orchestration/runtime/workflows/registry.yml" <<'EOF'
workflows:
  create-design-proposal:
    parameters:
...
EOF
  ...
  cat > "$fixture_root/.octon/orchestration/runtime/workflows/meta/create-design-proposal/README.md" <<'EOF'
# Create Design Proposal

Scaffold `.proposals/design/{{proposal_id}}/`.
EOF

  cat > "$fixture_root/.octon/orchestration/runtime/workflows/meta/create-migration-proposal/README.md" <<'EOF'
# Create Migration Proposal

Scaffold `.proposals/migration/{{proposal_id}}/`.
EOF

  cat > "$fixture_root/.octon/orchestration/runtime/queue/README.md" <<'EOF'
# Queue
...
case_live_backreference_fails() {
  ...
  printf '\nSee `.proposals/runtime-package/navigation/source-of-truth-map.md`.\n' >> \
    "$fixture_root/.octon/orchestration/runtime/queue/README.md"
  run_validator_in_fixture "$fixture_root"
}
