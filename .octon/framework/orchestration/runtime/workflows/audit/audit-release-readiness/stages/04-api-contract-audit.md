...
Assess contract definition coverage, implementation conformance, and lifecycle compatibility readiness.
...
1. Invoke:
   ...
   ```text
   /audit-api-contract scope="{{scope}}" severity_threshold="{{severity_threshold}}" post_remediation="{{post_remediation}}" convergence_k="{{convergence_k}}" seed_list="{{seed_list}}"
   ```
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-api-contract-audit-<run-id>.md` (if run)
- API contract stage findings summary for merge
