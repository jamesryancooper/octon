...
RUNTIME_DIR_DEFAULT="$(cd -- "$WORKFLOWS_DIR_DEFAULT/.." && pwd)"
ORCHESTRATION_DIR_DEFAULT="$(cd -- "$RUNTIME_DIR_DEFAULT/.." && pwd)"
FRAMEWORK_DIR_DEFAULT="$(cd -- "$ORCHESTRATION_DIR_DEFAULT/.." && pwd)"
OCTON_DIR_DEFAULT="$(cd -- "$FRAMEWORK_DIR_DEFAULT/.." && pwd)"
...
AUDIT_REPORTS_DIR=""
TMP_DIR=""
SCOPE=".octon/framework/orchestration/runtime/workflows/"
TARGET=""
FORMAT="markdown"
OUTPUT=""
SEVERITY_THRESHOLD=""
...
init_paths() {
  ...
  ORCHESTRATION_DIR="$OCTON_DIR/framework/orchestration"
  ...
  RUNTIME_AUDITS_DIR="$OCTON_DIR/instance/cognition/context/shared/audits"
  RUNTIME_INDEX="$RUNTIME_AUDITS_DIR/index.yml"
  REPORTS_DIR="$OCTON_DIR/state/evidence/validation"
...
score_workflow() {
  ...
  local artifact="$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/$artifact_rel"
...
system_level_checks() {
  ...
  for workflow_id in "${!WORKFLOW_PATHS[@]}"; do
    path="$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/${WORKFLOW_PATHS[$workflow_id]}"
    if [[ ! -e "$path" ]]; then
...
check_authoring_surface_alignment() {
  local targets=(
    "$ROOT_DIR/.octon/framework/orchestration/practices/workflow-authoring-standards.md"
    "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/meta/create-workflow/stages/02-analyze-requirements.md"
    "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/meta/create-workflow/stages/03-select-template.md"
    "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/meta/create-workflow/stages/06-integrate-gap-fixes.md"
    "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/meta/create-workflow/stages/08-verify.md"
    "$ROOT_DIR/.octon/instance/cognition/context/shared/workflow-quality.md"
    "$ROOT_DIR/.octon/instance/cognition/context/shared/workflow-gaps.md"
  )
  local target
  local stale_checks=(
...
run_representative_scenarios() {
  ...
  while IFS=$'\t' read -r workflow_id mode expected_format expected_profile; do
    ...
    artifact_path="$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/$artifact_rel"
...
score_target_workflow() {
  ...
  for id in "${!WORKFLOW_PATHS[@]}"; do
    ...
    if [[ "$(cd "$(dirname "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/$artifact_rel")" && pwd)/$(basename "$ROOT_DIR/.octon/framework/orchestration/runtime/workflows/$artifact_rel")" == "$(cd "$(dirname "$resolved")" && pwd)/$(basename "$resolved")" ]]; then
      workflow_id="$id"
      break
    fi
  done
  if [[ -z "$workflow_id" ]]; then
    ...
    artifact_rel="${target_path#.octon/framework/orchestration/runtime/workflows/}"
