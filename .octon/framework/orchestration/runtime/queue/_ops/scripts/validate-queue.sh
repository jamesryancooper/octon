...
check_definition_surfaces_are_local() {
  local matches=""
  matches="$(
    grep -R -n -F -- ".octon/inputs/exploratory/proposals/" \
  ...
  if [[ -n "$matches" ]]; then
    fail "queue surface definition artifacts must not depend on temporary .octon/inputs/exploratory/proposals paths"
    printf '%s\n' "$matches"
  else
    pass "queue surface definition artifacts avoid temporary .octon/inputs/exploratory/proposals paths"
  fi
}
