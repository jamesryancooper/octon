    ...
    raw_matches="$(
      cd "$ORCHESTRATION_DIR" && \
        rg -n --hidden --glob '!.git' \
          ...
          '\.octon/inputs/exploratory/proposals/' . || true
    )"
  else
    raw_matches="$(
      grep -R -n -E '\.octon/inputs/exploratory/proposals/' "$ORCHESTRATION_DIR" 2>/dev/null \
...
if [[ -n "$matches" ]]; then
  fail "live orchestration artifacts must not depend on temporary .octon/inputs/exploratory/proposals paths"
  printf '%s\n' "$matches"
else
  pass "live orchestration artifacts avoid temporary .octon/inputs/exploratory/proposals paths"
fi
