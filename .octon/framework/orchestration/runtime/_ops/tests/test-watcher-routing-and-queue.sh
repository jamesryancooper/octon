...
case_fan_out_and_order() {
  ...
  cp -R "$fixture_root/.octon/framework/orchestration/runtime/automations/runtime-contract-drift-remediation" "$fixture_root/.octon/framework/orchestration/runtime/automations/alpha-freshness-audit"
  cp -R "$fixture_root/.octon/framework/orchestration/runtime/automations/runtime-contract-drift-remediation" "$fixture_root/.octon/framework/orchestration/runtime/automations/zeta-freshness-audit"
  perl -0pi -e 's/runtime-contract-drift-remediation/alpha-freshness-audit/g' "$fixture_root/.octon/framework/orchestration/runtime/automations/alpha-freshness-audit/automation.yml"
  perl -0pi -e 's/runtime-contract-drift-remediation/zeta-freshness-audit/g' "$fixture_root/.octon/framework/orchestration/runtime/automations/zeta-freshness-audit/automation.yml"
...
case_match_mode_any_and_glob() {
  ...
  fixture_root="$(create_fixture)"
  envs=("OCTON_DIR_OVERRIDE=$fixture_root/.octon" "OCTON_ROOT_DIR=$fixture_root")
  cat > "$fixture_root/.octon/framework/orchestration/runtime/automations/runtime-contract-drift-remediation/trigger.yml" <<'EOF'
kind: "event"
event:
  watcher_ids:
...
case_stale_ack_and_dead_letter() {
  ...
  find "$fixture_root/.octon/framework/orchestration/runtime/queue/receipts" -type f -name 'q-retry-001-ack-*.json' | grep -q .

  jq '.claim_deadline = "2000-01-01T00:00:00Z"' "$fixture_root/.octon/framework/orchestration/runtime/queue/claimed/q-retry-001.json" > "$fixture_root/.octon/framework/orchestration/runtime/queue/claimed/q-retry-001.json.tmp"
  mv "$fixture_root/.octon/framework/orchestration/runtime/queue/claimed/q-retry-001.json.tmp" "$fixture_root/.octon/framework/orchestration/runtime/queue/claimed/q-retry-001.json"
  env "${envs[@]}" bash "$REPO_ROOT/$QUEUE_SCRIPT" expire >/dev/null
  [[ -f "$fixture_root/.octon/framework/orchestration/runtime/queue/dead-letter/q-retry-001.json" ]]
  [[ -n "$good_token" ]]
}
