---
title: Daily Flow
description: How your day looks as a governance owner working in Octon's agent-first, system-governed model.
---

# Daily Flow

This document describes the daily rhythm of working with Octon. Octon runs agent workflows by default under system-governed contracts and checks. Your role is governance oversight: set direction, review risk-tiered output, and handle policy exceptions or escalation.
...
```bash
# Start a new feature
octon feature "add webhook notifications for order updates"
...
> **CLI Reference**: Run `octon help` to see all available commands and options.
...
**Urgent Bug:**
```bash
octon fix "critical: users can't login" --priority high
```
...
octon explain "why did we use Redis for sessions?"

# Get context on a file
octon explain "what does OrderService do?"

# Understand an error
octon explain "what does this error mean: ECONNREFUSED"
```
...
octon feature "refactor authentication to use OAuth2"
octon build

# Start another task
octon feature "add pagination to /api/orders"
octon build
```
...
1. `octon retro` — AI generates summary of the week
...
| 14:00 | `octon plan-week` — Adjust priorities with Dev B |
| 14:30 | Start T3 OAuth spec: `octon feature "google oauth" --tier T3` |
| 15:00 | Review generated spec with Dev B |
| 15:30 | Approve spec, `octon build oauth-integration` |
...
| 9:10 | `octon investigate "error spike at 3am"` |
| 9:20 | AI proposes fix — approve |
| 9:45 | Review and approve fix PR |
| 10:00 | `octon triage` — Categorize incoming issues |
...
octon pause
# Adjust the task
octon retry --context "actually, we need pagination too"
```

### "AI's approach isn't what I wanted"

```bash
octon retry --constraint "use cursor-based pagination, not offset"
```

### "I need to understand what AI did"

```bash
octon explain <task-id> "why did you use this approach?"
```
...
```bash
octon pause
octon retry --model <alternate-model>
```
...
| Problem | Solution |
|---------|----------|
| AI seems stuck | `octon status --verbose` to see what's happening |
| PR has been pending too long | `octon nudge <pr-number>` |
| Tests keep failing | `octon diagnose <pr-number>` |
| Not sure what tier something should be | AI auto-assigns; you can override with `--tier` |
| Need to stop everything | `octon pause --all` |
...
- **CLI Reference**: `.octon/instance/bootstrap/catalog.md` — Full command reference, options, and integration points
