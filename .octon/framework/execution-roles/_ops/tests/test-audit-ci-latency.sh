...
WRAPPER="$REPO_ROOT/.octon/framework/execution-roles/_ops/scripts/ci/audit-ci-latency.sh"
CONTROL_PLANE="$REPO_ROOT/.octon/framework/execution-roles/practices/standards/github-control-plane-contract.json"
  ...
  cat > "$root/policy.json" <<EOF
{
  ...
  "duplicate_work_patterns": [
    ...
    },
    {
      ...
      "match": ".octon/framework/engine/runtime/run service build interfaces/filesystem-snapshot",
      "severity": "high",
