...
VALIDATE_SCRIPT=".octon/framework/assurance/runtime/_ops/scripts/validate-create-design-proposal-workflow.sh"
...
create_fixture_repo() {
  ...
  cp "$REPO_ROOT/.octon/framework/assurance/runtime/_ops/scripts/validate-create-design-proposal-workflow.sh" \
    "$fixture_root/.octon/framework/assurance/runtime/_ops/scripts/validate-create-design-proposal-workflow.sh"
  ...
  cp -R "$REPO_ROOT/.octon/framework/orchestration/runtime/workflows/meta/create-design-proposal" \
    "$fixture_root/.octon/framework/orchestration/runtime/workflows/meta/"
  printf '%s\n' "$fixture_root"
}
...
case_missing_stage_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  rm "$fixture_root/.octon/framework/orchestration/runtime/workflows/meta/create-design-proposal/stages/04-validate-package.md"
  run_validator_in_fixture "$fixture_root"
}
...
case_missing_registry_reference_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  python3 - "$fixture_root/.octon/framework/orchestration/runtime/workflows/meta/create-design-proposal/stages/03-scaffold-package.md" <<'PY'
from pathlib import Path
import sys
path = Path(sys.argv[1])
path.write_text(path.read_text().replace(".octon/generated/proposals/registry.yml", "registry-omitted"))
PY
  run_validator_in_fixture "$fixture_root"
}
...
case_missing_design_validator_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  python3 - "$fixture_root/.octon/framework/orchestration/runtime/workflows/meta/create-design-proposal/stages/04-validate-package.md" <<'PY'
from pathlib import Path
import sys
path = Path(sys.argv[1])
...
main() {
  ...
  assert_failure_contains \
    ...
    "missing file: .octon/framework/orchestration/runtime/workflows/meta/create-design-proposal/stages/04-validate-package.md" \
    case_missing_stage_fails
