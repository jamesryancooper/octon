...
case_stale_desired_hash_fails() {
  ...
  python3 - "$fixture_root/.octon/instance/extensions.yml" <<'PY'
from pathlib import Path
import yaml
path = Path(__import__("sys").argv[1])
doc = yaml.safe_load(path.read_text())
doc["selection"]["enabled"] = ["example"]
...
main() {
  assert_success "extension publication validator rejects stale desired-config state" case_stale_desired_hash_fails
