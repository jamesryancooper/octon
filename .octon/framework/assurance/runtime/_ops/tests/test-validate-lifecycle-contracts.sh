...
main() {
  ...
  write_valid_contract "$root"
    ...
    assert_schema_query_equals "program orchestration run route declared" "$program_contract" '.routes[]? | select(.route_id == "run-program-implementation-orchestration").completion.expected_receipts[] | select(. == "program-implementation-orchestration-run")' 'program-implementation-orchestration-run'
