...
extract_skill_version_from_ref() {
  local ref="$1"
  git -C "$ROOT_DIR" show "${ref}:.octon/framework/capabilities/runtime/skills/registry.yml" 2>/dev/null | extract_skill_version_from_stream || true
}
...
check_drift_alignment() {
  ...
  local watched_regexes=(
    ...
    '^\.octon/framework/assurance/practices/(complete\.md|session-exit\.md)$'
  ...
  local update_regexes=(
    '^\.octon/framework/capabilities/runtime/skills/audit/audit-subsystem-health/'
  ...
  local logic_regexes=(
    ...
    '^\.octon/framework/capabilities/runtime/skills/audit/audit-subsystem-health/references/.*\.md$'
  )
