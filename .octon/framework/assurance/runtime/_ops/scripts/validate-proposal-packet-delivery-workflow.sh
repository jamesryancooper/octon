...
if [[ -f "$WORKFLOW_PATH" ]] && yq -e '.' "$WORKFLOW_PATH" >/dev/null 2>&1; then
  ...
  require_yaml_value "$WORKFLOW_PATH" '.workflow.authority.final_sync_owner' 'disabled-during-rp00' "final sync disabled"
  require_yaml_value "$WORKFLOW_PATH" '.workflow.authority.branch_cleanup_owner' 'disabled-during-rp00' "branch cleanup disabled"
  require_yaml_value "$WORKFLOW_PATH" '.workflow.authority.terminal_current_state_proof_owner' 'disabled-during-rp00' "terminal Git proof disabled"
  require_yaml_value "$WORKFLOW_PATH" '.workflow.authority.worktree_hygiene_owner' 'preservation-only' "worktree hygiene preservation only"
  ...
  require_yaml_value "$WORKFLOW_PATH" '.constraints.required_target_outcomes[] | select(. == "implemented")' 'implemented' "implemented target admitted"
  require_yaml_value "$WORKFLOW_PATH" '.constraints.required_target_outcomes[] | select(. == "archive-ready")' 'archive-ready' "archive-ready target admitted"
...
require_token "$COMMAND_PATH" "/proposal-packet-delivery target=<proposal-packet-path> outcome=implemented|archive-ready route=stage-only profile=<profile-path> run-id=<id>" "command usage requires contained inputs"
...
require_token "$COMMAND_PATH" "route=stage-only" "command documents stage-only route"
require_token "$COMMAND_PATH" "RP00_CONTAINMENT_PUBLICATION_DISABLED" "command documents containment denial"
...
require_token "$COMMAND_MANIFEST_PATH" "route=stage-only" "command manifest stage-only argument"
require_token "$COMMAND_MANIFEST_PATH" "target=<proposal-packet-path> outcome=implemented|archive-ready route=stage-only profile=<profile-path> run-id=<id>" "command manifest requires contained inputs"
require_token "$SKILL_PATH" "proposal-packet-delivery" "skill file registration"
require_token "$SKILL_PATH" "route=stage-only" "skill documents stage-only route"
require_token "$SKILL_PATH" "/proposal-packet-delivery target=<proposal-packet-path> outcome=implemented|archive-ready route=stage-only profile=<profile-path> run-id=<id>" "skill usage requires contained inputs"
require_token "$SKILL_PATH" "RP00_CONTAINMENT_PUBLICATION_DISABLED" "skill documents containment denial"
...
require_token "$EXTENSION_COMMAND_MANIFEST_PATH" "target=<proposal-packet-path> outcome=implemented|archive-ready route=stage-only profile=<profile-path> run-id=<id>" "proposal lifecycle command manifest requires contained inputs"
...
require_token "$EXTENSION_COMMAND_PATH" "/octon-proposal-run-packet-delivery target=<proposal-packet-path> outcome=implemented|archive-ready route=stage-only profile=<profile-path> run-id=<id>" "proposal lifecycle command requires contained inputs"
...
require_token "$LIFECYCLE_CONTRACT_PATH" "route=stage-only" "proposal packet lifecycle requires stage-only route"
require_token "$LIFECYCLE_CONTRACT_PATH" "RP00_CONTAINMENT_PUBLICATION_DISABLED" "proposal packet lifecycle declares containment reason"
