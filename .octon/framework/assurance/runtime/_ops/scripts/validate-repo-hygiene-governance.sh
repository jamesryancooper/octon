...
PUBLICATION_WRAPPER_COMMON="$OCTON_DIR/framework/assurance/runtime/_ops/scripts/publication-wrapper-common.sh"
...
EVIDENCE_LOCALIZATION_POLICY="$OCTON_DIR/instance/governance/policies/evidence-localization.yml"
EVIDENCE_LOCALIZATION_HELPER="$OCTON_DIR/framework/assurance/runtime/_ops/scripts/evidence-localization.py"
EVIDENCE_LOCALIZATION_VALIDATOR="$OCTON_DIR/framework/assurance/runtime/_ops/scripts/validate-evidence-localization-governance.sh"
...
main() {
  ...
  require_file "$POLICY"
  require_file "$DEFAULT_WORK_UNIT"
  ...
  require_file "$PUBLICATION_WRAPPER_COMMON"
  require_file "$TERMINAL_LOCAL_EVIDENCE_SCHEMA"
  require_file "$TERMINAL_LOCAL_EVIDENCE_WRITER"
  require_file "$TERMINAL_LOCAL_EVIDENCE_VALIDATOR"
  require_file "$EVIDENCE_LOCALIZATION_POLICY"
  require_file "$EVIDENCE_LOCALIZATION_HELPER"
  require_file "$EVIDENCE_LOCALIZATION_VALIDATOR"
  ...
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.root == ".octon/generated/.tmp/**"' "$POLICY" "local artifact hygiene declares generated tmp scratch root"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.max_files == 50000' "$POLICY" "generated tmp scratch file budget is explicit"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.max_bytes == 2147483648' "$POLICY" "generated tmp scratch byte budget is explicit"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.ttl_seconds == 604800' "$POLICY" "generated tmp scratch ttl is explicit"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.producer_refs[] | select(. == ".octon/framework/assurance/runtime/_ops/scripts/publication-wrapper-common.sh")' "$POLICY" "generated tmp scratch policy binds publication wrapper producer"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.producer_refs[] | select(. == ".octon/framework/assurance/runtime/_ops/scripts/cleanup-publication-validation-runs.sh")' "$POLICY" "generated tmp scratch policy binds publication cleanup wrapper"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.cleanup_trigger | contains("--tmp-budget-report")' "$POLICY" "generated tmp scratch policy requires budget reporting route"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.cleanup_trigger | contains("cleanup-local-run-artifacts.sh")' "$POLICY" "generated tmp scratch policy routes cleanup through local artifact helper"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.rebuildability_proof | test("owning publisher")' "$POLICY" "generated tmp scratch policy requires rebuildability proof"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.authority_posture | test("local rebuildable scratch only")' "$POLICY" "generated tmp scratch policy declares non-authority posture"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.refusal_roots[] | select(. == ".octon/state/evidence/**")' "$POLICY" "generated tmp scratch policy refuses retained evidence roots"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.refusal_roots[] | select(. == ".octon/generated/effective/**")' "$POLICY" "generated tmp scratch policy refuses generated effective roots"
  require_yq '.local_artifact_hygiene.generated_tmp_scratch.refusal_roots[] | select(. == ".claude/**")' "$POLICY" "generated tmp scratch policy refuses host projection roots"
  require_yq '.local_artifact_hygiene.local_private_evidence.hosted_closeout_eligible == false' "$POLICY" "local-private evidence is excluded from hosted closeout claims"
  require_yq '.local_artifact_hygiene.local_private_evidence.terminal_closeout_sink.schema_ref == ".octon/framework/product/contracts/terminal-closeout-local-evidence-v1.schema.json"' "$POLICY" "repo-hygiene policy binds terminal local evidence schema"
  require_yq '.local_artifact_hygiene.local_private_evidence.terminal_closeout_sink.writer_ref == ".octon/framework/assurance/runtime/_ops/scripts/write-terminal-closeout-local-evidence.sh"' "$POLICY" "repo-hygiene policy binds terminal local evidence writer"
  require_yq '.local_artifact_hygiene.local_private_evidence.terminal_closeout_sink.validator_ref == ".octon/framework/assurance/runtime/_ops/scripts/validate-terminal-closeout-local-evidence.sh"' "$POLICY" "repo-hygiene policy binds terminal local evidence validator"
  require_yq '.local_artifact_hygiene.local_private_evidence.terminal_closeout_sink.protected_from_generic_cleanup == true' "$POLICY" "repo-hygiene policy protects terminal sink from generic cleanup"
  require_yq '.local_artifact_hygiene.external_evidence_localization.policy_ref == ".octon/instance/governance/policies/evidence-localization.yml"' "$POLICY" "repo-hygiene policy binds external localization policy"
  require_yq '.local_artifact_hygiene.external_evidence_localization.active_or_unverifiable_behavior | test("fail closed")' "$POLICY" "external localization fails closed for active or unverifiable evidence"
  require_yq '.local_artifact_hygiene.publishable_closeout_receipts.root == ".octon/state/evidence/runs/skills/repo-hygiene-cleanup/**"' "$POLICY" "repo-hygiene policy defines publishable cleanup receipt root"
  require_yq '.local_artifact_hygiene.publishable_closeout_receipts.required_for_hosted_or_shared_claims == true' "$POLICY" "repo-hygiene policy requires publishable receipts for hosted/shared claims"
  require_yq '.local_artifact_hygiene.publishable_closeout_receipts.raw_payload_rule | test("do not embed raw local logs")' "$POLICY" "repo-hygiene policy keeps raw local logs out of publishable receipts"
  require_yq '.local_artifact_hygiene.delegated_closeout_route | test("closeout-worktree may delegate")' "$POLICY" "local artifact hygiene documents closeout-worktree delegation boundary"
  require_yq '.local_artifact_hygiene.autonomous_receipt_route.allowed == true' "$POLICY" "local artifact hygiene allows governed autonomous receipt route"
  require_yq '(.local_artifact_hygiene.autonomous_receipt_route.rule | contains("--authorize")) and (.local_artifact_hygiene.autonomous_receipt_route.rule | contains("--authorization"))' "$POLICY" "local artifact hygiene autonomous route uses receipt-backed helper flow"
  require_yq '.local_artifact_hygiene.autonomous_receipt_route.rule | contains("local_filesystem_metadata")' "$POLICY" "local artifact hygiene permits only metadata-scoped input-root .DS_Store cleanup"
  ...
  require_yq '.local_artifact_hygiene.deletion_requires[] | select(test("--cleanup-path"))' "$POLICY" "local artifact hygiene documents selected cleanup path routing"
  ...
  require_yq '.local_artifact_hygiene.classifications.local_private_evidence.action | test("excluded from hosted/shared closeout evidence gates")' "$POLICY" "local-private evidence classification excludes hosted/shared closeout gates"
  ...
  require_yq '.closeout_evidence_boundaries.branch_no_pr_cleaned_rule | test("must not require or publish")' "$DEFAULT_WORK_UNIT" "default work unit excludes local-private raw evidence from hosted branch-no-pr cleaned claims"
  require_yq '.route_lifecycle_outcomes."branch-no-pr".cleaned_requires[]? | select(. == "publishable_evidence_receipt_refs_when_cleanup_evidence_is_hosted_or_shared")' "$DEFAULT_WORK_UNIT" "branch-no-pr cleaned requires publishable receipts for hosted/shared cleanup evidence"
  require_yq '.routes[]? | select(.route_id == "branch-no-pr") | .forbidden_evidence[]? | select(. == "local_private_raw_evidence_refs_as_hosted_or_shared_closeout_proof")' "$DEFAULT_WORK_UNIT" "branch-no-pr forbids local-private raw evidence as hosted/shared closeout proof"
  if yq -e '.route_lifecycle_outcomes."branch-no-pr".cleaned_requires[]? | select(test("local_private|local_raw|state/evidence/local"))' "$DEFAULT_WORK_UNIT" >/dev/null 2>&1; then
    fail "branch-no-pr cleaned must not require local-only raw evidence"
  else
    pass "branch-no-pr cleaned does not require local-only raw evidence"
  fi
  ...
  require_yq '.properties.schema_version.const == "terminal-closeout-local-evidence-v1"' "$TERMINAL_LOCAL_EVIDENCE_SCHEMA" "terminal local evidence schema pins schema_version"
  ...
  require_text 'run_candidate_reference_scan' "$LOCAL_ARTIFACT_CLEANUP_SCRIPT" "local artifact helper uses scalable candidate reference scanning"
  require_text 'failed-overprotect' "$LOCAL_ARTIFACT_CLEANUP_SCRIPT" "local artifact helper fails closed on reference scan failure"
  ...
  require_text 'terminal closeout local evidence sink cannot be authorized by generic cleanup' "$LOCAL_ARTIFACT_CLEANUP_SCRIPT" "local artifact helper rejects terminal sink cleanup authorization"
  require_text 'write-terminal-closeout-local-evidence.sh' "$TERMINAL_LOCAL_EVIDENCE_WRITER" "terminal local evidence writer documents itself"
  require_text 'terminal-closeout-local-evidence-v1' "$TERMINAL_LOCAL_EVIDENCE_VALIDATOR" "terminal local evidence validator binds manifest schema"
  require_text 'targeted receipt authorized unexpected paths' "$LOCAL_ARTIFACT_CLEANUP_TEST" "local artifact helper tests selected .DS_Store-only authorization"
  ...
  require_text 'proposal input file authorization receipt' "$LOCAL_ARTIFACT_CLEANUP_TEST" "local artifact helper tests proposal input file denial"
  ...
  require_text 'terminal closeout local evidence sink authorization receipt' "$LOCAL_ARTIFACT_CLEANUP_TEST" "local artifact helper tests terminal sink denial"
  ...
  require_text 'PUBLICATION_SCRATCH_MAX_FILES' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper declares scratch file budget"
  require_text 'PUBLICATION_SCRATCH_MAX_BYTES' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper declares scratch byte budget"
  require_text 'PUBLICATION_SCRATCH_TTL_SECONDS' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper declares scratch ttl"
  require_text 'publication_scratch_preflight' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper runs scratch preflight"
  require_text 'publication_require_generated_tmp_path' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper refuses scratch paths outside generated tmp"
  require_text 'publication_prune_stale_rebuildable_cache' "$PUBLICATION_WRAPPER_COMMON" "publication wrapper owns stale rebuildable cache pruning"
  require_text '--tmp-budget-report' "$LEGACY_PUBLICATION_CLEANUP_SCRIPT" "legacy publication cleanup emits tmp budget reports"
  ...
  require_text 'closeout-worktree` may delegate eligible residue' "$REPO_HYGIENE_CLEANUP_SKILL" "repo-hygiene-cleanup skill supports delegated closeout route"
  require_text 'without another operator prompt' "$REPO_HYGIENE_CLEANUP_SKILL" "repo-hygiene-cleanup skill documents autonomous receipt route"
  require_text '.octon/state/evidence/local/runs/skills/repo-hygiene-cleanup/' "$REPO_HYGIENE_CLEANUP_SKILL" "repo-hygiene-cleanup skill routes raw helper output to local-private evidence"
  require_text 'raw helper output, raw deleted path lists' "$REPO_HYGIENE_CLEANUP_SKILL" "repo-hygiene-cleanup skill keeps publishable receipts concise"
  ...
  require_text 'repo_hygiene_cleanup_ref' "$CLOSEOUT_WORKTREE_SKILL" "closeout-worktree records delegated repo-hygiene cleanup evidence"
  ...
  require_text 'repo_hygiene_cleanup_ref' "$CLOSEOUT_WORKTREE_VALIDATOR" "closeout-worktree validator checks delegated repo-hygiene cleanup evidence"
  require_text '`cleaned` is route-bound' "$CLOSEOUT_CHANGE_SKILL" "closeout-change limits cleaned to route-bound cleanup"
  require_text 'shared closeout claims also require publishable evidence receipt refs' "$CLOSEOUT_CHANGE_SKILL" "closeout-change requires publishable receipts for hosted/shared claims"
  require_text 'Do not claim hosted/shared closeout' "$CLOSEOUT_CHANGE_SKILL" "closeout-change blocks hosted/shared claims from raw local logs"
  ...
  run_test "terminal local evidence schema parses" yq -e '.' "$TERMINAL_LOCAL_EVIDENCE_SCHEMA"
  ...
  run_test "publication wrapper common parses" bash -n "$PUBLICATION_WRAPPER_COMMON"
  run_test "terminal local evidence writer parses" bash -n "$TERMINAL_LOCAL_EVIDENCE_WRITER"
  run_test "terminal local evidence validator parses" bash -n "$TERMINAL_LOCAL_EVIDENCE_VALIDATOR"
