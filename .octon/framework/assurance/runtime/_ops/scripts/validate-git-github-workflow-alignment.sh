...
main() {
  ...
  require_yq "$CONTRACT" '.closeout.post_landing_cleanup.applies_to_routes[]? | select(. == "branch-no-pr")' "post-landing cleanup applies to branch-no-pr" "post-landing cleanup must apply to branch-no-pr"
  require_yq "$CONTRACT" '.closeout.post_landing_cleanup.applies_to_routes[]? | select(. == "branch-pr")' "post-landing cleanup applies to branch-pr" "post-landing cleanup must apply to branch-pr"
  require_yq "$CONTRACT" '.closeout.post_landing_cleanup.required_before_completed_closeout[]? | select(. == "local main, origin/main, and landed_ref are aligned")' "post-landing cleanup requires final main alignment" "post-landing cleanup must require final main alignment"
  require_yq "$CONTRACT" '.helpers.git_branch_cleanup.safety[]? | select(. == "requires retained rollback posture for mutating cleanup")' "branch cleanup helper requires retained rollback posture" "branch cleanup helper must require retained rollback posture"
  require_yq "$CONTRACT" '.helpers.git_branch_cleanup.safety[]? | select(. == "requires origin/main containment before deleting local or remote refs")' "branch cleanup helper requires origin/main containment" "branch cleanup helper must require origin/main containment"
  require_yq "$CONTRACT" '.helpers.git_branch_cleanup.safety[]? | select(. == "blocks cleanup when no-open-PR proof cannot be completed or an open PR exists")' "branch cleanup helper blocks open-PR or unprovable cleanup" "branch cleanup helper must block open-PR or unprovable cleanup"
  ...
  require_literal "$WORKFLOW_DOC" "clean up obsolete safe local and remote source branches" "workflow overview requires post-landing branch cleanup" "workflow overview must require post-landing branch cleanup"
  require_literal "$WORKFLOW_DOC" 'sync local `main` to `origin/main`' "workflow overview requires local main sync after landing" "workflow overview must require local main sync after landing"
  ...
  require_literal "$PLAYBOOK" "Clean up a landed branch-no-pr source branch" "playbook documents no-PR source branch cleanup" "playbook must document no-PR source branch cleanup"
  require_literal "$PLAYBOOK" "Syncs local \`main\` to \`origin/main\` after cleanup" "playbook documents post-cleanup local main sync" "playbook must document post-cleanup local main sync"
  ...
  require_literal "$BRANCH_CLEANUP_SCRIPT" "Refusing to clean up protected branch" "git-branch-cleanup refuses protected branches" "git-branch-cleanup must refuse protected branches"
  require_literal "$BRANCH_CLEANUP_SCRIPT" "origin/main containment" "git-branch-cleanup requires origin/main containment" "git-branch-cleanup must require origin/main containment"
  require_literal "$BRANCH_CLEANUP_SCRIPT" "retained rollback" "git-branch-cleanup requires retained rollback posture" "git-branch-cleanup must require retained rollback posture"
  require_literal "$BRANCH_CLEANUP_SCRIPT" "open PR exists" "git-branch-cleanup refuses open-PR branches" "git-branch-cleanup must refuse open-PR branches"
  require_literal "$BRANCH_CLEANUP_SCRIPT" "Local \$BASE_BRANCH is synced to origin/main" "git-branch-cleanup verifies local main sync" "git-branch-cleanup must verify local main sync"
