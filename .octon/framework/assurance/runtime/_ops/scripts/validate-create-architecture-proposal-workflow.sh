...
grep -Fq 'external-tool-integrity.yml' "$WORKFLOW_DIR/stages/01-validate-request.md" && pass "request validation binds external-tool integrity" || fail "request validation binds external-tool integrity"
grep -Fq 'External Tool' "$WORKFLOW_DIR/stages/03-scaffold-package.md" && grep -Fq 'Boundary and External Tool Integrity sections' "$WORKFLOW_DIR/stages/03-scaffold-package.md" && pass "scaffold preserves external-tool boundary sections" || fail "scaffold preserves external-tool boundary sections"
grep -Fq 'external-tool modification as an implementation route' "$WORKFLOW_DIR/stages/04-validate-package.md" && pass "package validation rejects external-tool modification" || fail "package validation rejects external-tool modification"
grep -Fq 'does not recommend changing or reengineering an external tool' "$WORKFLOW_DIR/stages/05-report.md" && pass "report excludes external-tool modification recommendations" || fail "report excludes external-tool modification recommendations"
