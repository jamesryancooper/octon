...
RELEASE_HARNESS_CARD="$OCTON_DIR/state/evidence/disclosure/releases/2026-03-30-unified-execution-constitution-closure/harness-card.yml"
...
main() {
  ...
  require_file "$RELEASE_HARNESS_CARD"
  ...
  require_yq '.excluded_or_reduced_surfaces[] | select(.surface_id == "github-control-plane" and .status == "reduced")' "$CLOSURE_MANIFEST" "closure manifest records GitHub as reduced"
  require_yq '.excluded_or_reduced_surfaces[] | select(.surface_id == "ci-control-plane" and .route == "stage_only")' "$CLOSURE_MANIFEST" "closure manifest records CI as stage-only"
  ...
  validate_harness_card "$RELEASE_HARNESS_CARD" "release HarnessCard"
