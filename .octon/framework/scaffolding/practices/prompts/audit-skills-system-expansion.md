...
Use this prompt when you want to assess whether the Octon skills system should stay as-is, be tightened within existing artifacts, or be expanded with a new feature or artifact.

## Inputs

- Target subsystem: `.octon/framework/capabilities/runtime/skills`
- Optional comparison scope: `.octon/framework/capabilities`, `.octon/framework/orchestration`, `.octon/framework/scaffolding`
- Optional severity bar for recommendations: only propose expansion for material gaps

## Instructions

1. Use `audit-domain-architecture` as the primary evaluator for `.octon/framework/capabilities/runtime/skills`.
...
7. If needed, use `audit-cross-subsystem-coherence` to confirm that any proposed addition is not already represented elsewhere in Octon.
...
```text
Use /Users/jamesryancooper/Projects/octon/.octon/framework/scaffolding/practices/prompts/audit-skills-system-expansion.md to evaluate whether .octon/framework/capabilities/runtime/skills needs a new artifact or runtime feature, or whether existing manifests, registries, validators, and skill contracts are sufficient.
```
