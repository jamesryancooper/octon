...
- Produce a migration plan using `/.octon/framework/scaffolding/runtime/templates/migrations/template.clean-break-migration.md` at `/.octon/instance/cognition/context/shared/migrations/<YYYY-MM-DD>-<slug>/plan.md` and link verification evidence.
- Store migration evidence in bundle form at `/.octon/state/evidence/migration/<YYYY-MM-DD>-<slug>/` with `bundle.yml`, `evidence.md`, `commands.md`, `validation.md`, and `inventory.md`.
- Include sections:
...
If an exception is required, produce an exception request per `/.octon/framework/cognition/practices/methodology/migrations/exceptions.md`.
