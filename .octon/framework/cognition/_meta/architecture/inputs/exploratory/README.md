...
| `reviews/<review-family>/<review-id>/` | Non-authoritative review packets and raw reviewer evidence | Review summaries, reconciliation records, and evidence captured from independent reviewers | Runtime, policy, publication, generated, state/control, or implementation authority | Governed proposal revision, retained evidence update outside `inputs/**`, or removal | `validate-exploratory-input-surfaces.sh` |
