...
This document specifies the repository layout, module boundaries, and inter-component contracts for a vertically sliced, modular monolith. Octon standardizes on an `apps/*` + `packages/*` layout. It aims to maximize clarity for a small team while preserving encapsulation and determinism, enabling safe evolution toward more distributed architectures if warranted.
