...
Related docs: [monorepo polyglot (normative)](./monorepo-polyglot.md), [MAPE-K modeling](./mape-k-loop-modeling.md), [kaizen subsystem](./kaizen-subsystem.md), [governance model](./governance-model.md), [knowledge plane](../../runtime/knowledge/knowledge.md), [contracts registry](./contracts-registry.md), [python runtime workspace example](/.octon/framework/scaffolding/practices/examples/stack-profiles/python-runtime-workspace.md), [execution service roles](../../../capabilities/runtime/services/execution/service-roles.md)

This document specifies the technical contracts, governance constraints, and provenance requirements for the three Octon agents that drive continuous improvement within the MAPE-K loop: Planner, Builder, and Verifier. It should be read together with the planning and orchestration role mapping for `PlanKit`, `AgentKit`, `FlowKit`, and the platform runtime service in `../../../capabilities/runtime/services/execution/service-roles.md`, and with the polyglot monorepo blueprint (`monorepo-polyglot.md`) that defines where TS kits and Python runtimes live. For the canonical runtime model, see `runtime-architecture.md`.
...
This is developer-facing documentation for engineers and operators implementing or extending Octon’s agent loop. It defines responsibilities, expected outputs, constraints, and logging for each agent. It does not prescribe tool-specific CI/CD details beyond required outcomes and traceability.
