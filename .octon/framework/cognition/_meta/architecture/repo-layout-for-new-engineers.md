...
description: Onboarding-friendly guide to the Octon monorepo structure using the "run at the top, import from packages" rule of thumb.
version: v1.0.0
date: 2025-11-21
---
...
- Typically combines:
  - audit, remediation, and refactor workflows in `.octon/framework/orchestration/runtime/workflows/**`
...
User → apps/ai-console
     ...
     → .octon/framework/capabilities/runtime/services (composite service contracts)
...
In both cases, **only** `apps/*`, `agents/*`, and `platform/runtimes/*-runtime/**` are actual processes. Everything else in `packages/*` and `contracts/*` is imported, with composition definitions in `.octon/framework/capabilities/runtime/services/**`.
