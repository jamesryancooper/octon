...
allowed-tools: Read Glob Grep Edit Write(../../../**) Bash(npm) Bash(npx) Bash(mkdir) Bash(cp) Bash(node) Write(/.octon/state/evidence/runs/skills/*)
---
...
The MCP server is created in the project directory specified by the user. Execution logs are written to `/.octon/state/evidence/runs/skills/build-mcp-server/`.
