...
```text
/audit-observability-coverage scope=".octon/framework/capabilities/runtime/services"
```

Use an explicit contract reference:

```text
/audit-observability-coverage scope=".octon/framework/capabilities/runtime/services" observability_contract_ref=".octon/framework/cognition/_meta/architecture/observability-requirements.md"
```

Narrow discovery behavior:

```text
/audit-observability-coverage scope=".octon/framework/capabilities/runtime/services" service_manifest_glob="**/SERVICE.md" severity_threshold="high"
```
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-observability-coverage-audit-<run-id>.md` -- Structured findings report
- `.octon/state/evidence/validation/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `/.octon/state/evidence/runs/skills/audit-observability-coverage/` -- Execution logs with index
