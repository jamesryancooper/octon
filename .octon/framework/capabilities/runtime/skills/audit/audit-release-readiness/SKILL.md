...
```text
/audit-release-readiness scope=".octon" release_baseline_ref=".octon/framework/cognition/practices/methodology/ci-cd-quality-gates.md" operations_baseline_ref=".octon/framework/engine/practices/incident-operations.md"
```

With stricter threshold:

```text
/audit-release-readiness scope=".octon/framework/capabilities/runtime/services" severity_threshold="high"
```
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-audit-release-readiness-<run-id>.md` -- Structured findings report
- `.octon/state/evidence/validation/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `/.octon/state/evidence/runs/skills/audit-release-readiness/` -- Execution logs with index
