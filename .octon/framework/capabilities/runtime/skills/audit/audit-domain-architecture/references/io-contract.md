...
io:
  inputs:
    - name: domain_path
      type: text
      required: true
      description: "Domain root path or canonical .octon domain target to critique"
    - name: criteria
      type: text
      required: false
    ...
    - name: domain_profiles_ref
      type: file
      required: false
      default: ".octon/framework/cognition/governance/domain-profiles.yml"
...
- If `domain_path` is missing but represents a valid `.octon/<domain>` target, run in `prospective` mode.
- If `domain_path` cannot be normalized as a Octon target, escalate.
