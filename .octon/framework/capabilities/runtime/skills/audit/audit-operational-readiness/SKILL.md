...
```text
/audit-operational-readiness scope=".octon" operations_baseline_ref=".octon/framework/cognition/practices/methodology/reliability-and-ops.md" incident_baseline_ref=".octon/framework/engine/practices/incident-operations.md"
```

With stricter threshold:

```text
/audit-operational-readiness scope=".octon/framework/capabilities/runtime/services" severity_threshold="high"
```
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-operational-readiness-audit-<run-id>.md` -- Structured findings report
- `.octon/state/evidence/validation/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `/.octon/state/evidence/runs/skills/audit-operational-readiness/` -- Execution logs with index
