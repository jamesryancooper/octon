...
| **Mapping** | A single old→new pair describing one rename or move (e.g., `.workspace/` → `.octon/`) |
...
| **File filter** | A glob pattern (e.g., `.octon/framework/cognition/_meta/architecture/**`) that narrows the scope manifest to only matching files within the `scope` directory |
