...
```text
/audit-test-quality scope=".octon" testing_baseline_ref=".octon/framework/assurance/practices/standards/testing-strategy.md" quality_gate_baseline_ref=".octon/framework/cognition/practices/methodology/ci-cd-quality-gates.md"
```

With stricter threshold:

```text
/audit-test-quality scope=".octon/framework/capabilities/runtime/services" severity_threshold="high"
```
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-test-quality-audit-<run-id>.md` -- Structured findings report
- `.octon/state/evidence/validation/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `/.octon/state/evidence/runs/skills/audit-test-quality/` -- Execution logs with index
