...
description: >
  ...
  .octon subsystem manifests, registries, architecture docs, and quality
...
Layered coherence audit that verifies cross-subsystem contract alignment in `.octon`.
...
- `.octon/state/evidence/validation/analysis/YYYY-MM-DD-cross-subsystem-coherence-audit.md` -- Human-readable findings report
- `.octon/state/evidence/validation/audits/YYYY-MM-DD-<run-id>/` -- Authoritative bounded-audit bundle
- `/.octon/state/evidence/runs/skills/audit-cross-subsystem-coherence/` -- Execution logs with index
