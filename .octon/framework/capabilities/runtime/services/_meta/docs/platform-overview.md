...
Services are **control‑plane capabilities** that live under `.octon/framework/capabilities/runtime/services/*` and are reused across the entire repo.

- **Primary placement (services):**
  - `.octon/framework/capabilities/runtime/services/<domain>/<service-id>/...`
  - Source of truth for:
...
- **Secondary placements (runtimes/adapters) are consumers of services:**
  - `apps/*` — thin HTTP/CLI adapters that call services in `.octon/framework/capabilities/runtime/services/*`.
...
- **Flow (service contract interface):**
  - `.octon/framework/capabilities/runtime/services/execution/flow/` exposes typed input/output contracts and runtime semantics to launch flows (native WASM by default, optional HTTP adapter when configured).
...
- Architecture: services = control‑plane capabilities under `.octon/framework/capabilities/runtime/services`; the LangGraph runtime is infrastructure under `agents/runner/runtime/**`.
- Methodology: services = concrete tools used to implement the Spec → Plan → Flow → Run lifecycle; see also `.octon/framework/capabilities/runtime/services/execution/service-roles.md` for canonical roles.
...
In short: the AI Services Platform is the **practical backbone** of Octon's promise — a modular, local-first framework that lets a tiny team ship outsized results, without sacrificing complexity fitness, safety, or correctness. It turns multi-agent chaos into structured, safe, high-velocity development — **fast, safe, and aligned by design**.
...
These invariants apply to every service and workflow so the Platform operates as a cohesive, deterministic, and governable system across Octon's lifecycle (Spec → Plan → Implement → Verify → Ship → Operate → Learn).
...
Octon is an agent-first, system-governed methodology for cross-project execution with practical solo-builder ergonomics. It pairs spec-led intent capture, context-efficient planning, autonomous AI execution loops, and risk-tiered ACP governance while staying stack-, host-, and environment-agnostic. Octon aligns with OWASP ASVS, NIST SSDF, and STRIDE, and adopts architectural principles from 12‑Factor, Monolith‑First, and Hexagonal—so you can ship fast, ship safe, and ship with confidence. For the full methodology and lifecycle overview, see .octon/framework/cognition/practices/methodology/README.md

This platform maps directly to Octon. Use the services as ready‑to‑run building blocks with AI‑driven quality, security, and reliability built in.
...
Octon alignment notes are called out inline below in the relevant services.

Alignment stamp (2025‑11‑07): PASS — AI Services Platform covers all six Octon pillars and every lifecycle stage. This revision clarifies Next.js bundling/security guidance and UI collaboration patterns; see "Pillars Coverage Matrix" and "Lifecycle Alignment Map" for systemic coverage.

---

## System Coherence: Octon's Pillars Implemented by the AI Services Platform

The AI Services Platform is designed as a cohesive, self-reinforcing system aligned to Octon's six pillars. Each pillar is made concrete by specific services, artifacts, and gates that create a closed-loop from Spec -> Plan -> Implement -> Verify -> Ship -> Operate -> Learn.
...
This catalog clarifies how the essential services reinforce Octon's pillars and lifecycle, and which canonical spans they must emit.
...
| --- | --- | --- | --- | --- | --- | --- |
| Spec | Produce spec one‑pager + ADR | spec | `.octon/framework/capabilities/runtime/services/planning/spec/schema/input.schema.json` | `docs/specs/*.md`, `docs/specs/adr-*.md` | `service.spec.specify` | Policy preflight (ASVS/SSDF), Observe trace open |
| Plan | Produce plan (BMAD) from spec | plan | `.octon/framework/capabilities/runtime/services/planning/plan/schema/input.schema.json` | `plan.json` | `service.plan.plan` | Policy ruleset selected; dry‑run OK |
| Agent | Execute plan (produce artifacts only) | implement | `.octon/framework/capabilities/runtime/services/execution/agent/schema/input.schema.json` | Proposed diffs, tests, notes under `runs/**` | `service.agent.execute` | Guard redaction; idempotency required on mutating ops |
| Tool | Deterministic action wrappers (Git/HTTP/Shell) | implement | `.octon/framework/capabilities/runtime/services/operations/tool/schema/input.schema.json` | Structured logs, proposed changes | `service.tool.call.<action>` | Guard + Cache; fail closed on secret/redaction errors |
| Eval | Verify structure/grounding/style | verify | `.octon/framework/capabilities/runtime/services/quality/eval/schema/input.schema.json` | `runs/eval/*.json` | `service.eval.verify` | Thresholds enforced; fail‑closed on miss |
| Policy | Evaluate policy rulesets (ASVS/SSDF/STRIDE) | spec·plan·verify·ship | `.octon/framework/capabilities/runtime/services/governance/policy/schema/input.schema.json` | `runs/policy/*.json` | `service.policy.check` | Fail‑closed by default (`policy.failClosed = true`) |
| Test | Contract/unit/e2e invoker | verify | `.octon/framework/capabilities/runtime/services/quality/test/schema/input.schema.json` | `runs/test/*.json` | `service.test.run` | OpenAPI diff fail‑closed; contract tests required on changed surfaces |
| Patch | Open PR + changelog | ship | `.octon/framework/capabilities/runtime/services/delivery/patch/schema/input.schema.json` | PR number, preview URL | `service.patch.open_pr` | Feature flag OFF by default; preview smoke recommended |
| Release | Tag and release | ship | `.octon/framework/capabilities/runtime/services/delivery/release/schema/input.schema.json` | Tag, changelog | `service.release.tag` | Requires green policy/eval gates |
| Flag | Server‑side flag evaluation/toggles | ship·operate | `.octon/framework/capabilities/runtime/services/delivery/flag/schema/input.schema.json` | Flag states/rollout plan | `service.flag.evaluate`, `service.flag.toggle` | Progressive delivery; rollback path ready |
...
| Compliance | Assemble evidence pack | verify·ship·learn | `.octon/framework/capabilities/runtime/services/governance/compliance/schema/input.schema.json` | Evidence pack manifest under `runs/**` | `service.compliance.assemble` | Required for high‑risk changes |
| Cache | Idempotency + memoization | implement | `.octon/framework/capabilities/runtime/services/operations/cache/schema/input.schema.json` | Cache hit/miss records | `service.cache.hit`, `service.cache.miss` | Pure ops must declare `--cache-key` |
| Parse | Convert PDFs to Markdown (tables/figures) | implement | `.octon/framework/capabilities/runtime/services/retrieval/parse/schema/input.schema.json` | `docs_out/parsed/*.md`, `runs/**/parse-*.json` | `service.parse.run` | Guard redaction; dry‑run OK |
| Prompt | Manage prompt templates, variables, variants, and fixtures | implement·verify | `.octon/framework/capabilities/runtime/services/modeling/prompt/schema/input.schema.json` | `prompts_out/**` compiled prompts, `prompt_tests/**` fixtures, `runs/**/prompt-*.json` prompt metadata | `service.prompt.compile` | Eval/Test/Policy gates on prompt drift and schema violations |
...
## Lifecycle Alignment Map (Octon ⇄ Services)

| Octon Stage | Primary Services | Artifacts (evidence) | Guards (gates) |
| --- | --- | --- | --- |
...
- Octon's WIP and risk policies are enforced through **Policy** rules and Patch PR templates (risk rubric, rollback, flags).
...
Define consistent states and transitions across all services to improve determinism, observability, and governance. These states are orthogonal to Octon stages and are represented in spans as attributes (`service.state`) and as span events for transitions.
...
To align with Octon governance, Policy rules MUST be explicit and versioned:

- Ruleset identity: `policy.ruleset = <framework>|<profile>@<version>` (e.g., `ASVS@5.0`, `SSDF@1.1`, `Octon-Minimal@2025-11-01`).
...
{
  "policy": {
    "ruleset": "Octon-Minimal@2025-11-01",
    "failClosed": true,
    "riskProfile": {
      "trivial": {
...
Octon mandates policy-gated governance with minimal ceremony:
...
Patch SHOULD generate (or validate) a minimal PR body conforming to Octon's governance and determinism rules. Use this as the default template; Patch fills placeholders and appends links/evidence.
...
- Standard envs
  - `OTEL_EXPORTER_OTLP_ENDPOINT` (default `http://localhost:4318`) and `OTEL_SERVICE_NAME` (auto: `octon.service.<service>`).
  - `OCTON_ENV` (`local|preview|prod`) mapped to `deployment.environment`.
...
- Contracts updated in the service-local schema path (`.octon/framework/capabilities/runtime/services/<domain>/<service>/schema`); registry references refreshed; diffs linked in PR.
...
```json
{
  ...
  "observability": {
    "serviceName": "octon.service.plan",
...
- Location: `.octon/framework/capabilities/runtime/services`
  - OpenAPI: service-specific files as needed in each service folder.
  - JSON Schemas (services): `.octon/framework/capabilities/runtime/services/<domain>/<service>/schema/`
...
- Registry exports: update `.octon/framework/capabilities/runtime/services/registry.yml` (or service-local docs) to expose new or updated contracts to consumers.

This registry is the single source of truth for inter‑service interfaces and aligns with Octon's spec‑first, contract‑driven flow.

Interoperability promise:

- Services depend only on normative contracts in `.octon/framework/capabilities/runtime/services/**/schema` and respect semantic versioning boundaries. Breaking changes require a MAJOR bump with migration notes; MINOR additions must be backward-compatible. Cross‑service integration tests (via Test/Pact/Schemathesis where applicable) verify forward/backward compatibility. Use service metadata `compatibility` and `deprecatedSince` fields to signal support windows and orchestrate safe upgrades.
...
- Invalidation triggers:
  - Content hash or contract version bump; policy ruleset version change; environment change (`OCTON_ENV`).
...
- Maintain a small debt ledger and a hard WIP policy per Octon; protect flow and reverse quickly when needed.
...
- **Service identity**: `service.name = "octon.service.<serviceName>"`; `service.version` from git SHA or package version; `deployment.environment` set (e.g., `local`, `preview`, `prod`).
...
- Resource (set once per process):
  - `service.name = "octon.service.<serviceName>"`
...
- Feature flags provider registration: register the Vercel Flags provider at app startup (API and SSR surfaces) via the repo’s flag config (e.g., `packages/config/flags.ts`) and evaluate flags server‑side. Local/dev may fall back to `OCTON_FLAG_*` envs.
...
- Contracts live alongside services under `.octon/framework/capabilities/runtime/services/<domain>/<service>/schema` (OpenAPI + JSON‑Schema where applicable).
- Naming (alignment with Octon):
...
- Tests:
  ...
  - Place adapter contract tests adjacent to adapters; keep service contract references under `.octon/framework/capabilities/runtime/services/**/schema`.
- CI gates:
