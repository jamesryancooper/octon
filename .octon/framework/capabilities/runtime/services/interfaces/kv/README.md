...
- Persist durable runtime state under `.octon/state/control/engine/kv/`.
...
- State directory: `.octon/state/control/engine/kv`.
- Manifest limits:
  - `max_concurrency`: 4
  - `timeout_ms`: 30000
...
- Host persistence is a JSON map at `.octon/state/control/engine/kv/store.json`.
