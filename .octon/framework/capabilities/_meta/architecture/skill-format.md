...
> **Note:** The authoritative `version` is defined in `.octon/framework/capabilities/runtime/skills/registry.yml`, not in SKILL.md metadata. This prevents version drift between files. See [Specification](./specification.md#manifest-and-registry-files) for the single source of truth principle.
