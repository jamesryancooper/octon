...
```bash
.octon/framework/capabilities/_ops/scripts/validate-deny-by-default.sh --changed --profile dev-fast
.octon/framework/capabilities/_ops/scripts/validate-deny-by-default.sh --all --profile strict
.octon/framework/capabilities/_ops/scripts/validate-agent-only-governance.sh
.octon/framework/capabilities/_ops/scripts/policy-profile-resolve.sh refactor
.octon/framework/capabilities/_ops/scripts/policy-grant-broker.sh create --subject service:agent --tier low --request-id req-1 --agent-id agent-a --plan-step-id step-1
.octon/framework/capabilities/_ops/scripts/policy-kill-switch.sh status
.octon/framework/capabilities/_ops/scripts/policy-rollout-mode.sh get
```
