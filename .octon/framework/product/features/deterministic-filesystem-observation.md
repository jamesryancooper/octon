...
Provides deterministic filesystem snapshots, diffs, discovery, graph/provenance query, and OS-agnostic polling watch surfaces as observation rather than authority.

## Boundary

This note explains the product boundary for `deterministic-filesystem-observation`. The machine-readable catalog entry remains navigation-only and does not mint runtime authority, support claims, generated-effective state, or retained execution evidence.
