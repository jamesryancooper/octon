...
## `cargo check --workspace` (from `.octon/framework/engine/runtime/crates/`)

```bash
$ cd .octon/framework/engine/runtime/crates
$ cargo check --workspace
Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.35s
```

## `cargo component check` (from `.octon/framework/capabilities/runtime/services/interfaces/kv/rust/`)

```bash
$ cd .octon/framework/capabilities/runtime/services/interfaces/kv/rust
$ cargo component check
Generating bindings for octon-kv-service (src/bindings.rs)
Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.02s
```

## Prebuilt kernel binaries (`.octon/framework/engine/_ops/bin/`)
...
- `.octon/framework/engine/_ops/bin/octon-macos-arm64`
- `.octon/framework/engine/_ops/bin/octon-macos-x64`
