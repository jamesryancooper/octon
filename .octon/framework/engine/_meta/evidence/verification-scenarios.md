...
> Notes:
> - The CLI binary is `octon` (kernel crate). For development, use `.octon/framework/engine/runtime/run ...` which falls back to `cargo run`.
...
```bash
.octon/framework/engine/runtime/run serve-stdio
```
...
Edit policy file `.octon/framework/engine/runtime/config/policy.yml` to remove `storage.local` from `interfaces/kv`, then invoke `kv.get`.
...
```bash
.octon/framework/engine/runtime/run studio --help
```

Expected:

- help includes `Launch Octon Studio desktop UI`

## 11) Material ACP run emits instruction-layer manifest

Run an ACP enforce request with `phase=promote`, wrapper receipt emission enabled,
and inspect continuity evidence for the run id.

Expected:

- `.octon/state/evidence/runs/<run_id>/instruction-layer-manifest.json` exists
- manifest has `schema_version=instruction-layer-manifest-v1`
- manifest includes all four layers (`provider`, `system`, `developer`, `user`)

## 12) Context-overhead gate telemetry completeness

For a post-cutover receipt generated by wrapper-assisted ACP enforce:

Expected:

- receipt includes `instruction_layers`, `context_acquisition`, and `context_overhead_ratio`
- `context_acquisition` has non-negative integer counters for:
  - `file_reads`, `search_queries`, `commands`, `subagent_spawns`, `duration_ms`
- `context_overhead_ratio` classifies against policy thresholds:
  - `<0.20` target
  - `>=0.20` warn
  - `>=0.35` soft-fail
  - `>=0.50` hard-fail
