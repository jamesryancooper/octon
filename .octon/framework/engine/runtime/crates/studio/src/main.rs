...
fn main() -> Result<()> {
    ...
    let root = find_octon_root(&launch_dir).unwrap_or(launch_dir);
...
fn find_octon_root(start: &Path) -> Option<PathBuf> {
    RootResolver::resolve_from(start)
        .ok()
        .and_then(|octon_dir| octon_dir.parent().map(Path::to_path_buf))
}
