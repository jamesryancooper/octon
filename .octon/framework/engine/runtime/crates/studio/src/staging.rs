...
const OPERATIONS_REPORTS_DIR_REL: &str = ".octon/state/evidence/runs/operations";
...
fn write_apply_audit(
    ...
    match write_apply_audit_in_dir(&reports_dir, repo_root, execution, edits) {
        Ok(path) => Ok(path),
        Err(primary_error) => {
            let fallback_dir = std::env::temp_dir().join("octon-studio-reports");
...
fn render_apply_audit_markdown(
    ...
    let mut out = String::new();
    out.push_str("# Octon Studio Apply Audit\n\n");
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let stamp = SystemTime::now()
            ...
            .as_nanos();
        let pid = std::process::id();
        let path = std::env::temp_dir().join(format!("octon-studio-{label}-{pid}-{stamp}"));
