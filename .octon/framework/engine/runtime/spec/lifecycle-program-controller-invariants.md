...
| `LA-PC-029` | Blocker ledger and recovery deltas: every program run must write digest-bound `blocker-ledger.yml` and `recovery-delta-summary.yml` compact artifacts under the current workflow evidence root. Recovery readers must prefer those artifacts and failing slices before raw receipt/archive history; stale, missing, digest-mismatched, authority-conflicting, or no-progress recovery state must fail closed. Repeated failure evidence must be grouped by blocker class, child id when present, route id, and disposition; retry diagnostics must name the failing path, accepted recovery value, stale cause, recovery class, and minimal repair hint; child-owned receipt refs must stay direct path-plus-digest refs rather than parent prose. These artifacts summarize blocker state only and must never satisfy child-owned receipts, validation verdicts, promotion evidence, archive metadata, closeout authorization, or terminal lifecycle outcomes. |
| `LA-PC-030` | Blocker taxonomy fidelity: proposal-program contracts must classify blocker examples into `routine-autonomous`, `soft-blocker`, and `hard-blocker` recovery classes. The taxonomy must preserve child-owned authority, keep proposal inputs and generated outputs non-authoritative, treat unknown blocker classes as hard blockers, and prove hard-blocker examples have no automatic recovery route or action. |
