...
| --- | --- | --- | --- |
| `additive/.incoming/<intake-id>/` | Raw additive intake before classification, bounded by `intake.yml` plus `payload/` | classify through `/process-incoming-intake` | `validate-incoming-intake-unit.sh` |
...
Incoming additive units require a non-authoritative `intake.yml` envelope and a
`payload/` raw payload root. Raw material outside `payload/` is invalid. The
envelope records only intake-time facts; route classification, normalization,
activation, publication, archive retention, and durable evidence remain separate
governed steps.
