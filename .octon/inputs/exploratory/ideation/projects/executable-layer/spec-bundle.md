...
These files are intended to live under `.octon/runtime/spec/` (or wherever you prefer).
...
```
.octon/framework/capabilities/services/<category>/<service>/
  service.json
  service.wasm
```
...
```json
{
  ...
  "docs": {
    "summary": "Local key/value storage for Octon runtime state.",
    "help": "Use for durable state under .octon/runtime/_ops/state/kv/. Keys are strings; values are UTF-8 strings in v1."
  }
}
```
...
Create `.octon/runtime/spec/service-manifest-v1.schema.json`:

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://octon.local/spec/service-manifest-v1.schema.json",
  "title": "Octon Service Manifest v1",
  "type": "object",
  "required": [
    "format_version",
    "name",
...
Create `.octon/framework/capabilities/services/manifest.yml`:
...
Create `.octon/framework/capabilities/services/registry.yml`:
...
services:
  interfaces/kv:
    ...
    depends_on: []
    io:
      state_dir: .octon/runtime/_ops/state/kv
    notes:
...
Create `.octon/runtime/spec/stdio-protocol-v1.md` with these normative rules:
...
```json
{"type":"hello","protocol":"octon-stdio-v1","client":{"name":"<string>","version":"<string>"}}
```

Kernel responds:

```json
{"type":"hello","protocol":"octon-stdio-v1","kernel":{"version":"<string>","os":"<string>","arch":"<string>"}}
```
...
1. **RootResolver**
   ...
   * Output: path to active `.octon/` (nearest ancestor)
   * Rules: walk upward until found; error if none

2. **ConfigLoader**

   * Reads `.octon/octon.yml` + optional runtime config file(s)
   * Produces: `RuntimeConfig` (paths, policy, defaults)

3. **ServiceDiscovery**

   * Scans `.octon/framework/capabilities/services/**/service.json`
...
5. **WasmHost**
   ...
   * Responsibilities:
     ...
     * cache compiled artifacts (use `.octon/runtime/_ops/state/wasmtime-cache/`)
...
8. **TraceWriter**
   ...
   * Appends NDJSON events to `.octon/runtime/_ops/state/traces/<trace_id>.ndjson`
   * Events to record:
