...
- **`.octon/runtime/`** contains the executable runtime and its launchers.
- **`.octon/framework/capabilities/services/<category>/<service>/`** is the unit of deployment for a service: one folder per service, containing its WASM artifact + runtime manifest (`service.json` — format defined in [spec-bundle.md §1](spec-bundle.md)).
- **`.octon/runtime/_ops/state/`** is runtime state (caches, traces, indexes). Keep it **gitignored** and **non-portable**.

---

## 2) Portability rules

Portability is metadata-driven, not structural. The following classifications belong in `octon.yml`.

### Portable (framework assets)

- `.octon/runtime/**` (launcher scripts + binaries)
- `.octon/framework/capabilities/services/**/service.wasm`
- `.octon/framework/capabilities/services/**/service.json`
- `.octon/framework/capabilities/services/manifest.yml`
- `.octon/framework/capabilities/services/registry.yml`

### Non-portable (project-local state)

- `.octon/runtime/_ops/state/**` (caches, traces, indexes, KV backing store)
- `.octon/state/continuity/repo/**` (progress, tasks, session state)
...
### `.octon/runtime/run.cmd` (Windows `cmd`)
...
- launch `runtime/_ops/bin/octon-windows-x64.exe` (and optionally arm64 in the future)
...
1. **Harness root resolution** — "nearest `.octon/` ancestor wins" for nested scopes
...
5. **Observability** — structured traces to `.octon/runtime/_ops/state/traces/*.ndjson`
...
### Gate A: Octon governance (existing model)
...
1. Does the caller have permission per the Octon governance layer?
...
- `octon.yml` and/or a dedicated policy file under `.octon/runtime/config/`
...
- `octon info [--json]`
- `octon services list [--json]`
- `octon tool <service> <op> --json '<payload>'`
- `octon run --spec <path>` (executes agent plan/workflow graph)
- `octon validate` (validates manifests, policies, and service artifacts)
- `octon serve-stdio` (NDJSON session mode — protocol defined in [spec-bundle.md §4](spec-bundle.md))
- `octon service new <category> <name>` (scaffold a new service)
- `octon service build <category>/<name>` (build service.wasm + update integrity hash)
...
- `.octon/runtime/_ops/state/wasmtime-cache/`
...
The kernel may optionally write a registry snapshot to `.octon/runtime/_ops/state/registry-cache.json` after scanning `capabilities/services/**/service.json`. On subsequent invocations, this snapshot accelerates service discovery by avoiding a full filesystem scan when the snapshot is fresh.

### Harness runtime state

- `.octon/runtime/_ops/state/kv/` for service-backed state
- `.octon/runtime/_ops/state/indexes/` for search/vector/etc. indexes
- `.octon/runtime/_ops/state/traces/` for NDJSON traces
...
- [ ] Per-OS/arch kernel binaries (`octon-{os}-{arch}`)
- [ ] Wasmtime cache config pointing to `.octon/runtime/_ops/state/wasmtime-cache/`

### B) Kernel

- [ ] `.octon/` root resolver (nearest-harness rule)
...
- [ ] Tracing output to `.octon/runtime/_ops/state/traces/*.ndjson`
