...
- A functional `TraceWriter` exists at `.octon/runtime/crates/core/src/trace.rs`.
- It writes NDJSON trace events to `.octon/runtime/_ops/state/traces/<trace_id>.ndjson`.
...
## `octon run --spec` workflow execution
...
- The server enforces a max line length (default 1 MiB) in `.octon/runtime/crates/core/src/jsonlines.rs`.
...
- Example `manifest.yml` and `registry.yml` are present under `.octon/framework/capabilities/services/`.
