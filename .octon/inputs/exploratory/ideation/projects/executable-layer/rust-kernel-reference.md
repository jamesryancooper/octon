...
This document is the **authoritative Rust host-side reference** for the Octon executable layer kernel. It covers the workspace layout, canonical WIT contract, core types, module implementations, Wasmtime integration, and request execution pipeline.
...
Inside `.octon/runtime/` (or wherever you build the binary), structure it as a Rust workspace so you can keep the kernel clean and testable:
...
> **This is the single source of truth for the WIT world.** Both the kernel (host bindings) and all services (guest bindings) use this definition. The file lives at `.octon/runtime/wit/world.wit` and is copied into each service's `rust/wit/world.wit` at scaffold time.

```wit
package octon:runtime@1.0.0;
...
## 5) Root resolution (nearest `.octon/`)

`core/root.rs`:

* start from `cwd`
* walk parents until `.octon/` exists
...
* scan `.octon/framework/capabilities/services/**/service.json` with `walkdir`
...
* load `.octon/runtime/spec/service-manifest-v1.schema.json` (defined in [spec-bundle.md §1.3](spec-bundle.md))
...
This makes `octon validate` meaningful and prevents unknown fields drifting into runtime.
...
* default workspace root = resolved `.octon/` parent or repo root
...
* open `.octon/runtime/_ops/state/traces/<trace_id>.ndjson` for append
...
impl KvStore {
    /// Opens (or creates) a KV store at `state_dir` (e.g. `.octon/runtime/_ops/state/kv/`).
...
```rust
let repo_root = octon_dir.parent().unwrap().to_path_buf();
let kv = KvStore::open(
    octon_dir
        .join("runtime")
        .join("_ops")
...
bindgen!({
    world: "octon-service",
    path: "../../wit", // points at .octon/runtime/wit/
});
```
...
Configure `wasmtime::Engine` with caching pointed at `.octon/runtime/_ops/state/wasmtime-cache/`.
