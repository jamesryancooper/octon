...
The harness is pure files — markdown, YAML, shell scripts. No runtime dependency, no proprietary platform, no SDK. It works with Claude Code, Cursor, any AI IDE, or a bare terminal. The methodology is explicitly stack-, host-, and environment-agnostic. The `profiles:` list in `octon.yml` means you can seed a new repo with the framework in one copy operation.
...
Portability is **metadata-driven, not structural** (OCTON-SPEC-002). The `profiles:` list in `octon.yml` declares exactly which paths are framework assets that travel to new repos via `octon init`. Everything else (continuity logs, missions, decisions, project context) is project-specific state that stays put.
