...
- [ ] Implement a universal agent + assistant standard in the framework (.octon/inputs/exploratory/ideation/scratchpad/archive/universal-agent-assistant-standard-draft.md). This standard should be used to define the agents and assistants in the framework.
...
- [x] Implement capabilities.yml in the `.octon/framework/capabilities/skills/capabilities.yml` file for fast skill capability lookup.
...
- [ ] Create a workspaceKit for creating and managing workspaces.
- [ ] Determine where we implement the workspace resolution algorithm that was documented in @docs/architecture/workspaces/skills/architecture.md. In the @.octon/framework/capabilities/skills/scripts directory?
...
- [x] Update the create-skill workflow to create skills that align with the agent skills spec. Use `.octon/framework/capabilities/skills/refine-prompt` as an example. Progressive disclosure is important. Also, include the following instructions for naming skills:
