...
All proof described here is planned. Future receipts must bind exact commit,
macOS build, hardware architecture, sandbox profile digest, provider/client
version, workspace and Git identities, session class, commands, times, exit
...
| --- | --- |
| sentinel environment variables and provider tokens | absent or unreadable |
...
| arbitrary network, loopback services, metadata endpoints | denied; only declared provider/task endpoints allowed |
...
includes profile/session manifests without secrets, sandbox digest, repository
...
conformance, and drift/churn review.
