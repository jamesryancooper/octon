...
| `.octon/framework/engine/runtime/crates/runtime_bus/src/transactions.rs` | No one T1/T2 authority/effect substrate exists. | Add atomic reserve/consume/capacity/attempt/outbox T1, opaque complete publication-tuple binding, route freeze, and generic result-or-UNKNOWN T2 APIs. | Planned new RP-03 module; no provider or route policy. |
...
| `.octon/framework/engine/runtime/crates/authority_engine/src/implementation/policy.rs` | `write_json`/`write_yaml` and policy receipt paths can directly write consequential control files. | Add path-classified routing so consequential mutations require runtime_bus while temporary/evidence output remains non-authoritative. | Shared file: RP-01 keeps policy semantics; RP-03 owns only persistence integration. |
