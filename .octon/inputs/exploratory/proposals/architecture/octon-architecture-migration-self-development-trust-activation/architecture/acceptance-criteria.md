...
- Trust changes land inert by exact content digest; install never changes the
  active selector or widens authority.
- `activation-authority-v1` is issued only by RP-01 for the exact version,
  epoch, and scope; tests prove no RP-09 path can mint, widen, substitute, or
  self-satisfy the envelope.
- Activation binds exact version, inventory/epoch, evidence, provider identity,
  compatibility, approval, health gates, and pre-verified rollback.
...
- SI-07 operates with inert source landing; safe-automatic remains disabled and
  unclaimed unless its distinct proof and later promotion acceptance pass.
- Setup/status/rollback are concise and add no routine prompt to ordinary
  changes or separate operator control plane.
