...
| `.octon/framework/engine/runtime/crates/local_broker/src/operation.rs` | No broker-internal one-shot dispatch handle exists. | Create/validate/consume a non-widening handle bound to RP-01 guard and RP-03 operation/attempt/epoch/client. | Planned new module; handle is not authority. |
...
| `.octon/framework/engine/runtime/spec/local-broker-ipc-request-v1.schema.json` | No typed request envelope exists. | Validate bounded authority/operation refs, target digest, client/challenge/nonce/expiry, and no secret/widening fields. | Planned new runtime schema. |
