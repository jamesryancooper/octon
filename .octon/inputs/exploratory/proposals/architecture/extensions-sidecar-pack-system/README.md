# External `.octon.extensions` Sidecar Pack System
...
- summary: Define the full Octon-side implementation that lets the harness discover, validate, compile, and consume additive extension content from a repo-root `.octon.extensions/` sidecar without creating a second authority surface.
...
9. `examples/.octon.extensions/nextjs/`
10. `examples/.octon.extensions/docs/`
11. `examples/.octon.extensions/node-ts/`

## Supporting Reference Set

This proposal also includes:

- a proposed schema for `/.octon.extensions/catalog.yml`
- a proposed schema for `/.octon.extensions/<pack-id>/pack.yml`
- a merge and precedence spec for Octon effective indexes
- concrete example packs rooted at:
  - `/.octon.extensions/nextjs/`
...
`/.octon/`. Treat the repo-root `/.octon.extensions/` surface as an implementation
