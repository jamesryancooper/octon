...
- Child id: `architecture-lens-bank-foundation`
- Bound `source`:
  `.octon/inputs/exploratory/proposals/architecture/architecture-review-method-suite-program/`
- Bound `target`:
  `.octon/inputs/exploratory/proposals/architecture/architecture-lens-bank-foundation`
...
disagrees with the live mechanism, the live mechanism wins and this child would
trigger a parent registry/design revision rather than implement a stale claim
...
2. Parent program design authority (direction for this child, not authority):
   - `architecture/lens-bank-design.md` — the load-bearing source for this child
...
- `path: .octon/inputs/exploratory/proposals/architecture/architecture-lens-bank-foundation`
- `required: true`, `deferred: false`
- `dependencies: []` (phase-0 seed; nothing precedes it)
- `dependency_gate: verification`
- `phase_id: phase-0`, `group_id: suite-foundation`
...
`architecture-review-method-taxonomy-and-routing` (phase-1), whose naming v2
methods list and routing v2 `method_selection` block must cite this verified
lens bank before its own implementation planning.

## Shared Lens Bank Design (verbatim from parent `architecture/lens-bank-design.md`)

The following is preserved verbatim as the primary source authority this child
implements. It is retained here so the packet is archive-ready without the
parent packet on disk.
...
- The mechanism directory currently holds exactly four files:
  `README.md`, `balanced-architecture-review-method.md`, `naming.yml`,
  `review-routing.yml`. Neither `architecture-lens-bank.md` nor `lens-bank.yml`
  exists yet — this child creates them (a genuine new surface).
- `naming.yml` is at `schema_version: architectural-review-naming-v1` with a
  single `method` block (Balanced). The methods-list refactor to v2 is the
  phase-1 child's job, not this child's — this child adds only the lens bank
  and must not touch naming.yml.
- `balanced-architecture-review-method.md` defines an 11-step **Required
  Sequence** and an **Output Contract**. This child expresses that existing
  sequence as lens ids (see `resources/lens-bank-authoring-spec.md`) and must
  not edit the Balanced doc's doctrine text.
- No `validate-architectural-review-lens-references.sh` exists in
  `.octon/framework/assurance/runtime/_ops/scripts/` yet; this child creates it
  alongside the existing architectural-review validators.
