...
2. Update charter contract in `.octon/framework/assurance/CHARTER.md` to the umbrella chain.
3. Update doctrine and assurance entry docs:
   - `.octon/framework/assurance/DOCTRINE.md`
   - `.octon/framework/assurance/README.md`
   - `.octon/README.md` (Assurance Engine section)
4. Update policy contract in `.octon/framework/assurance/standards/weights/weights.yml`:
...
5. Update runtime resolver and gate in `.octon/runtime/crates/assurance_tools/src/main.rs`:
...
7. Regenerate AE artifacts under `.octon/generated/assurance/` so no generated output contains old chain semantics.
...
| Downstream breakage | Consumers of `.octon/` fail on update | Publish migration notes and explicit old->new field map. |
