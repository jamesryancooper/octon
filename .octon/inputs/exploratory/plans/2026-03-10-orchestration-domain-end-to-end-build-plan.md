...
- Goal: implement the orchestration domain end-to-end from the design package without inventing architecture, then promote the resulting surfaces into live `.octon` authority in a controlled way

## Profile Selection Receipt

- `change_profile`: `transitional`
- `release_state`: `pre-1.0`
- `release_state_evidence`: executable service artifacts in the repository still carry `0.1.0` versions, including `.octon/framework/capabilities/runtime/services/interfaces/filesystem-snapshot/SERVICE.md:6`, `.octon/framework/capabilities/runtime/services/interfaces/agent-platform/SERVICE.md:8`, and `.octon/framework/capabilities/runtime/services/retrieval/query/SERVICE.md:7`
- `selection_mode`: `auto`
- `recommended_transitional_mode`: `Phased Implementation` plus `Phased Rollout`
- `profile_facts`:
  - `downtime_tolerance`: no hard zero-downtime requirement is visible, but a big-bang cutover is unnecessary because live `.octon/framework/orchestration` surfaces already exist and can be strengthened incrementally.
  ...
  - `data_migration_backfill_needs`: no large historical backfill is required for the package itself, but temporary coexistence between package-local authority and live `.octon/framework/orchestration` surfaces is required until each surface is promoted.
...
- `transitional_exception_note`:
  ...
  - `risks`: temporary dual-authority drift between the package and live `.octon` surfaces; incomplete promotion leaving engineers unsure which surface is canonical; validators lagging behind contract promotion.
  - `owner`: Octon orchestration domain owner and repository maintainer
...
Actions:
1. Wire `bash .octon/framework/assurance/runtime/_ops/scripts/validate-orchestration-design-package.sh` into local and CI execution.
...
#### Phase 8: Canonicalize Promoted Surfaces Into Live `.octon`
...
1. `bash .octon/framework/assurance/runtime/_ops/scripts/validate-orchestration-design-package.sh`
...
- `.octon/framework/orchestration/runtime/incidents/`
- `.octon/framework/orchestration/runtime/queue/`
- `.octon/framework/orchestration/runtime/watchers/`
- `.octon/framework/orchestration/runtime/campaigns/` only if justified
...
- `baseline_validation_status`: `validated on 2026-03-10 by running bash .octon/framework/assurance/runtime/_ops/scripts/validate-orchestration-design-package.sh with zero errors and zero warnings`
- `core_alignment`:
  - preserves Octon's `agent-first` and `system-governed` framing
...
1. Engineers are implementing inside this repository and can promote surfaces into `.octon/`.
...
3. Existing live `.octon/framework/orchestration` documents contradict the package in a way that cannot be solved by promotion sequencing alone.
