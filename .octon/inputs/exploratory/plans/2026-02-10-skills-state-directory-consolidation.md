...
The skills directory at `.octon/framework/capabilities/skills/` mixes operational directories (`configs/`, `resources/`, `runs/`, `logs/`) with skill group directories (`foundations/`, `meta/`, `platforms/`, etc.) at the same level. There is no visual or structural distinction between them. Moving the four operational directories under a `_ops/state/` parent creates a clear separation of mutable runtime data from skill definitions, improves top-level legibility, and simplifies gitignore/cleanup operations.
...
- **Historical output/report files** (`.octon/generated/`): Leave as-is. These are frozen audit artifacts and updating them would misrepresent what was reported.
- **Archive files** (`.octon/framework/capabilities/skills/archive/`): Leave as-is. Historical snapshots.
...
```
mkdir .octon/framework/capabilities/skills/_state
git mv .octon/framework/capabilities/skills/configs   .octon/framework/capabilities/skills/_ops/state/configs
git mv .octon/framework/capabilities/skills/resources .octon/framework/capabilities/skills/_ops/state/resources
git mv .octon/framework/capabilities/skills/runs      .octon/framework/capabilities/skills/_ops/state/runs
git mv .octon/framework/capabilities/skills/logs      .octon/framework/capabilities/skills/_ops/state/logs
```

### Step 2: Update `registry.yml`

**File:** `.octon/framework/capabilities/skills/registry.yml`
...
**File:** `.octon/framework/capabilities/skills/scripts/validate-skills.sh`
...
**File:** `.octon/framework/capabilities/skills/scripts/setup-harness-links.sh`
- Line 22: Replace `"logs"` with `"_state"` in `EXCLUDE_DIRS`

### Step 4: Update `allowed-tools` in all SKILL.md files

Global find-and-replace within `.octon/framework/capabilities/skills/**/SKILL.md` (excluding `archive/` and `_scaffold/template/`):
...
**`.octon/framework/capabilities/skills/_scaffold/template/SKILL.md`** — Lines 41, 48, 49, 87-93: update all `logs/`, `runs/`, `configs/`, `resources/` path references

**`.octon/framework/capabilities/skills/_scaffold/template/references/`** — 7 reference files with path updates:
...
- Line 13: `.octon/framework/capabilities/skills/logs/` → `.octon/framework/capabilities/skills/_ops/state/logs/`
- Line 32: `capabilities/skills/logs/` → `capabilities/skills/_ops/state/logs/`

### Step 8: Update skills README

**File:** `.octon/framework/capabilities/skills/README.md`
...
- `.octon/framework/orchestration/workflows/quality-gate/orchestrate-audit/06-report.md` (lines 88, 116)
- `.octon/framework/orchestration/workflows/meta/create-skill(x)/03-initialize-skill.md` (lines 31, 41)
- `.octon/framework/cognition/analyses/workflows-vs-skills-analysis.md` (lines 22-23, 71, 183, 479, 774)
- `.octon/framework/cognition/context/primitives.md` (line 179)
...
1. Run `.octon/framework/capabilities/skills/scripts/validate-skills.sh` — all skills should pass
2. Grep sweep for stale bare references (excluding archive/ and output/):
   ...
   grep -rn --include='*.md' --include='*.yml' --include='*.sh' \
     ...
     .octon/framework/capabilities/skills/ docs/architecture/harness/skills/ CLAUDE.md
   ```
3. Verify physical structure: `ls .octon/framework/capabilities/skills/_ops/state/` shows 4 directories; old locations are gone
