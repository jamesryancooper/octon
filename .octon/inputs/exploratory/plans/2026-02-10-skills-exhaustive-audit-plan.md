...
- `.octon/framework/capabilities/skills/` — all files: manifest.yml, capabilities.yml, registry.yml, README.md, every SKILL.md, every `references/` file, `_ops/state/`, `_scaffold/template/`, `_ops/scripts/`
...
- `.octon/framework/orchestration/workflows/` — workflow files referencing skills paths
- `.octon/framework/cognition/` — analyses and context files referencing skills

**Out of scope:**

- Runtime execution correctness (whether skills produce correct output when invoked)
- Archived skills (`.octon/framework/capabilities/skills/archive/`)
- Historical audit reports in `.octon/state/evidence/validation/` (frozen artifacts)
- Workflow subsystem internals (only cross-references to skills paths)

## Output

- **Primary report:** `.octon/state/evidence/validation/analysis/2026-02-10-skills-exhaustive-audit.md`
...
2. Confirm old top-level directories (`configs/`, `resources/`, `runs/`, `logs/` at `.octon/framework/capabilities/skills/`) no longer exist
3. Confirm per-skill subdirectories exist inside `_ops/state/` for each skill that needs them

#### 2.2 Stale path sweep

Grep the entire project for old-style paths. Each pattern must return 0 matches (excluding `archive/` and `.octon/state/evidence/validation/`):
...
2. Workflow files (`.octon/framework/orchestration/`) use `_ops/state/` paths
3. Cursor commands (`.cursor/commands/`) use `_ops/state/` paths
4. Cognition files (`.octon/framework/cognition/`) use `_ops/state/` paths
...
1. `.octon/framework/orchestration/workflows/meta/create-skill(x)/03-initialize-skill.md` — paths use `_ops/state/` prefix, references grouped directory structure
2. `.octon/framework/orchestration/workflows/quality-gate/orchestrate-audit/06-report.md` — paths use `_ops/state/` prefix
...
1. `.octon/framework/cognition/analyses/workflows-vs-skills-analysis.md` — skill path references use `_ops/state/` prefix and grouped format
2. `.octon/framework/cognition/context/primitives.md` — skills subsystem description matches current architecture
