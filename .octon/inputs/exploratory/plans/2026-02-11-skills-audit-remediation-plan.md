...
Date: 2026-02-11
Source: `.octon/state/evidence/validation/analysis/2026-02-10-skills-exhaustive-audit.md`
...
**File:** `.octon/framework/capabilities/skills/_ops/scripts/validate-skills.sh`
...
.octon/framework/capabilities/skills/_ops/scripts/validate-skills.sh 2>&1 | grep -c "Unscoped Write"
# Expected: 13
```
...
However, examining the existing comment in these files ("Write is intentionally unscoped: scaffolds into user project directories"), the correct remediation is to use a broad but explicit scope that covers the harness root. The pattern used by `create-skill` is instructive: `Write(.octon/framework/capabilities/skills/*)`. For scaffolding skills that write into the user's project tree, scope to the harness root:
...
**File:** `.octon/framework/capabilities/skills/_ops/scripts/setup-harness-links.sh`

**Root cause analysis:**

1. **Line 13:** `PROJECT_ROOT` is computed as 3 levels up from `_ops/scripts/` (`../../..`), but `_ops/scripts/` is at `.octon/framework/capabilities/skills/_ops/scripts/`, so 3 levels up gives `.octon/` — not the repo root. The correct depth is **4 levels up** (or better: derive from manifest location).
...
4. **Line 58:** Symlink target is `../../.octon/framework/capabilities/skills/$skill_id` — flat path, doesn't work for grouped skills.
...
1. **Fix `PROJECT_ROOT` resolution (line 13):**
   ...
   # Repo root is 3 levels above skills dir: .octon/framework/capabilities/skills/ → repo
...
4. **Fix `create_skill_link()` to use grouped path for symlink target (lines 48-86):**
   ...
   create_skill_link() {
       ...
       local target="../../.octon/framework/capabilities/skills/$skill_path"
...
```bash
# Run script
bash .octon/framework/capabilities/skills/_ops/scripts/setup-harness-links.sh
...
# Expected: -> ../../.octon/framework/capabilities/skills/meta/create-skill
```
...
**File:** `.octon/framework/capabilities/skills/_ops/scripts/generate-reference-headers.sh`
...
bash .octon/framework/capabilities/skills/_ops/scripts/generate-reference-headers.sh 2>&1 | grep -c "ERROR"
# Expected: 0

# Single-skill mode should work for a grouped skill
bash .octon/framework/capabilities/skills/_ops/scripts/generate-reference-headers.sh create-skill
...
**File:** `.octon/framework/capabilities/skills/_ops/state/logs/FORMAT.md`
...
**File:** `.octon/framework/capabilities/skills/_ops/state/logs/index.yml`
...
- `.octon/framework/capabilities/skills/_ops/state/logs/refactor/index.yml`
- `.octon/framework/capabilities/skills/_ops/state/logs/refine-prompt/index.yml`
- `.octon/framework/capabilities/skills/_ops/state/logs/synthesize-research/index.yml`
...
- `.octon/framework/capabilities/skills/meta/create-skill/SKILL.md` (lines 73-74)
- `.octon/framework/capabilities/skills/meta/create-skill/references/io-contract.md` (lines 32, 52, 78)
...
grep -r '{{skill-name}}\|{{run-id}}' .octon/framework/capabilities/skills/meta/create-skill/
# Expected: 0 matches
```
...
- `.octon/framework/orchestration/workflows/meta/create-skill(x)/00-overview.md`
- `.octon/framework/orchestration/workflows/meta/create-skill(x)/02-copy-template.md`
- `.octon/framework/orchestration/workflows/meta/create-skill(x)/03-initialize-skill.md`
- `.octon/framework/orchestration/workflows/meta/create-skill(x)/06-report-success.md`
...
| File | Line(s) | Change |
|------|---------|--------|
| `.octon/framework/cognition/analyses/workflows-vs-skills-analysis.md:190` | `skill_mappings:` → `skills:` in YAML example | Replace with current registry schema (`skills.<id>.io`) |
| `.octon/framework/cognition/analyses/workflows-vs-skills-analysis.md:558` | `references/behaviors.md` → `references/phases.md` | Update reference filename |
| `.octon/framework/cognition/analyses/workflows-vs-skills-analysis.md:559` | `skills/refine-prompt/` → `skills/synthesis/refine-prompt/` | Update to grouped path |
| `.octon/framework/cognition/context/decisions.md:53` (D029) | `behaviors.md` → `phases.md` | Update canonical reference file name |
| `.octon/framework/cognition/decisions/001-octon-shared-foundation.md:59` | `skills/logs/` → `skills/_ops/state/logs/` | Update path convention |
...
bash .octon/framework/capabilities/skills/_ops/scripts/validate-skills.sh

# Verify no stale behaviors.md references in active docs
grep -rn 'behaviors\.md' docs/architecture/harness/skills/ .octon/framework/cognition/ .octon/framework/orchestration/
...
**File:** `.octon/framework/capabilities/skills/manifest.yml`
...
bash .octon/framework/capabilities/skills/_ops/scripts/validate-skills.sh --strict

# Verify rules.md files are within budget
for f in $(find .octon/framework/capabilities/skills/foundations -name "rules.md"); do
    words=$(wc -w < "$f")
    tokens=$((words * 13 / 10))
