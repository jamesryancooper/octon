...
VALIDATE_SCRIPT=".octon/assurance/runtime/_ops/scripts/validate-continuity-memory.sh"
...
create_fixture_repo() {
  ...
  mkdir -p \
    ...
    "$fixture_root/.octon/continuity/_meta/architecture/schemas" \
    "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01" \
    "$fixture_root/.octon/continuity/runs"

  cp "$REPO_ROOT/.octon/assurance/runtime/_ops/scripts/validate-continuity-memory.sh" \
    "$fixture_root/.octon/assurance/runtime/_ops/scripts/validate-continuity-memory.sh"
  cp "$REPO_ROOT/.octon/continuity/_meta/architecture/schemas/decision-record.schema.json" \
    "$fixture_root/.octon/continuity/_meta/architecture/schemas/decision-record.schema.json"

  cat > "$fixture_root/.octon/continuity/tasks.json" <<'EOF'
{
  "schema_version": "1.2",
  ...
  "tasks": [
    {
      ...
      "knowledge_links": {
        ...
        "contracts": [
          ".octon/continuity/_meta/architecture/schemas/decision-record.schema.json"
        ],
        "decisions": [
          "dec-20260308-weekly-freshness-audit-allow-01"
        ],
        "evidence": [
          ".octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
        ]
      }
    }
  ]
...
EOF
  ...
  cat > "$fixture_root/.octon/continuity/entities.json" <<'EOF'
{
  "schema_version": "1.1",
...
EOF
  ...
  cat > "$fixture_root/.octon/continuity/next.md" <<'EOF'
# Next
...
EOF
  ...
  cat > "$fixture_root/.octon/continuity/decisions/README.md" <<'EOF'
# Continuity Decisions

Routing, authority, and prerequisite decision evidence.
EOF

  cat > "$fixture_root/.octon/continuity/decisions/retention.json" <<'EOF'
{
  "schema_version": "1.0",
...
EOF
  ...
  cat > "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json" <<'EOF'
{
...
EOF
  ...
  cat > "$fixture_root/.octon/continuity/runs/README.md" <<'EOF'
# Continuity Runs

Execution receipts, digests, and related run evidence.
EOF

  cat > "$fixture_root/.octon/continuity/runs/retention.json" <<'EOF'
{
  "schema_version": "1.0",
  "default_action": "retain",
  "classes": [
...
case_missing_decision_json_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  rm "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
  run_validator_in_fixture "$fixture_root"
}
...
case_invalid_outcome_fails() {
  ...
  fixture_root="$(create_fixture_repo)"
  jq '.outcome = "deny"' \
    "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json" \
    > "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.tmp"
  mv \
    "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.tmp" \
    "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/decision.json"
  run_validator_in_fixture "$fixture_root"
...
case_stray_file_fails() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  touch "$fixture_root/.octon/continuity/decisions/dec-20260308-weekly-freshness-audit-allow-01/extra.txt"
  run_validator_in_fixture "$fixture_root"
}
