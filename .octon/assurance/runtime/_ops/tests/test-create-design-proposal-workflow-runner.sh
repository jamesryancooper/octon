...
new_fixture_repo() {
  ...
  mkdir -p \
    "$fixture_root/.octon/scaffolding/runtime" \
    "$fixture_root/.octon/assurance/runtime/_ops"

  cp -R "$REPO_ROOT/.octon/scaffolding/runtime/templates" \
    "$fixture_root/.octon/scaffolding/runtime/"
  cp -R "$REPO_ROOT/.octon/assurance/runtime/_ops/scripts" \
    "$fixture_root/.octon/assurance/runtime/_ops/"
...
latest_bundle_root() {
  local fixture_root="$1"
  find "$fixture_root/.octon/output/reports/workflows" -maxdepth 1 -mindepth 1 -type d | sort | tail -n 1
}

case_domain_runtime_scaffold_passes() {
  local fixture_root output bundle_root package_root
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" runtime-package "Runtime Package" domain-runtime ".octon/orchestration/runtime/example.md")"
...
case_experience_product_scaffold_passes() {
  local fixture_root output bundle_root package_root
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" experience-package "Experience Package" experience-product ".octon/scaffolding/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
...
case_duplicate_package_failure_writes_receipts() {
  ...
  fixture_root="$(new_fixture_repo)"
  first_output="$(run_create_workflow "$fixture_root" duplicate-package "Duplicate Package" domain-runtime ".octon/orchestration/runtime/example.md")"
  [[ -n "$first_output" ]] || return 1

  if output="$(run_create_workflow "$fixture_root" duplicate-package "Duplicate Package" domain-runtime ".octon/orchestration/runtime/example.md" 2>&1)"; then
    return 1
  fi
