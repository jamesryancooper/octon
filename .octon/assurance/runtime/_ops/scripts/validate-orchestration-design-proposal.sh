...
PACKAGE_ARG="${1:-.proposals/.archive/design/orchestration-domain-design-package}"
if [[ "$PACKAGE_ARG" = /* ]]; then
...
validate_run_linkage_fixture() {
  ...
  jq -e '
    ...
    and (.continuity_run_path | nonempty_string and startswith(".octon/continuity/runs/"))
    and (.summary | nonempty_string)
    and (
      (has("workflow_ref") | not)
      or (
