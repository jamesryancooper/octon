...
This plan defines a bounded coherence audit of `.octon/capabilities/skills/` and its documentation surface. The goal is to detect **drift** (refs that didn't follow moves), **misalignment** (docs say X, source says Y), **orphans** (entries without backing files or files without entries), and **schema rot** (structural violations against declared contracts).
...
- `.octon/capabilities/skills/` — manifest.yml, registry.yml, capabilities.yml, README.md, all SKILL.md files, `references/`, `_ops/state/`, `_scaffold/template/`, `_ops/scripts/`
...
- `.octon/orchestration/workflows/` — cross-references to skills paths only
...
- **Report:** `.octon/output/reports/analysis/2026-02-11-skills-subsystem-health-audit.md`
...
Glob all directories under `.octon/capabilities/skills/` that contain a `SKILL.md`, excluding `_scaffold/template/`, `_ops/state/`, `_ops/scripts/`, and `archive/`. For each:
...
Glob all directories under `.octon/capabilities/skills/{group}/` that do NOT contain a `SKILL.md` and are not utility dirs (`_template`, `_state`, `_scripts`, `archive`, `references`, `scripts`, `assets`). These are placeholder or abandoned directories. Report as informational.
...
**Goal:** Architecture docs describe the actual current structure, flows, and conventions of `.octon/capabilities/skills/`.

### Checks

#### 4.1 Directory structure claims

Read `docs/architecture/harness/skills/` docs that describe directory layout (likely `architecture.md`, `skill-format.md`, and the skills README.md). Extract claimed directory trees. Compare against actual glob of `.octon/capabilities/skills/`.
...
Read `.octon/capabilities/skills/README.md`. Verify:
...
In `.octon/capabilities/skills/README.md`:
