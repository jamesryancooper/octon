...
- `code`:
  - `/.octon/engine/runtime/crates/kernel/src/pipeline.rs`
...
- `contracts`:
  - `workflow-execution-bundle` contract for `/.octon/output/reports/workflows/<YYYY-MM-DD>-<slug>/`
...
- The stale local `/.octon/output/reports/pipelines/` shell was removed after the contract cutover so only the canonical `reports/workflows/` surface remains for new workflow bundles.
