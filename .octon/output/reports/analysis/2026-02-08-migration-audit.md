...
**G1. `.octon/capabilities/skills/scripts/validate-skills.sh` — 31 occurrences** [HIGH]
...
**G2. `.octon/scaffolding/templates/octon/` — 6 template files** [HIGH]
...
**G3. `.octon/capabilities/skills/audit-migration/references/safety.md` — 2 occurrences** [HIGH]
...
**G4. `.octon/continuity/entities.json` — 3 issues** [CRITICAL]
...
**G5. `.octon/continuity/tasks.json` — 3 occurrences** [MEDIUM]
...
**X1. `.octon/scaffolding/templates/octon/` — 6 broken path references** [HIGH]
...
**X2. `.octon/capabilities/skills/scripts/validate-skills.sh:1940` — broken anchor** [HIGH]
...
**S1. `.octon/capabilities/skills/scripts/validate-skills.sh` — conceptual staleness** [HIGH]
...
**S2. `.octon/scaffolding/templates/octon-docs/orchestration/workflows/are/are-init.sh` — 2 occurrences** [HIGH]
...
**S3. `.octon/capabilities/skills/scripts/generate-reference-headers.sh:24`** [HIGH]

Variable `WORKSPACE_REGISTRY="$REPO_ROOT/.octon/capabilities/skills/registry.yml"` — should be renamed to `HARNESS_REGISTRY` or just `REGISTRY`.
...
**`.octon/continuity/entities.json`** — Update entity key from `"docs/architecture/workspaces/"` to `"docs/architecture/harness/"` and fix notes text.

### Batch 2: High — Operational scripts (3 files, ~35 occurrences)

1. **`.octon/capabilities/skills/scripts/validate-skills.sh`** — Rename `WORKSPACE_REGISTRY` → `REGISTRY` (or `HARNESS_REGISTRY`), rename functions, update comments and log messages. Fix broken anchor reference.
2. **`.octon/capabilities/skills/scripts/generate-reference-headers.sh`** — Rename `WORKSPACE_REGISTRY` → `REGISTRY`.
3. **`.octon/scaffolding/templates/octon-docs/orchestration/workflows/are/are-init.sh`** — Replace "workspace-specific" → "harness-specific", "workspace files" → "harness files".

### Batch 3: High — Broken template paths (6 files)

**`.octon/scaffolding/templates/octon/`** — Replace `docs/architecture/harnesses/` → `docs/architecture/harness/` in all 6 README.md files.

### Batch 4: High — Skill reference file (1 file)

**`.octon/capabilities/skills/audit-migration/references/safety.md`** — Replace "not part of workspace" → "not part of harness" (lines 90-91).

### Batch 5: Medium — State files (1 file)

**`.octon/continuity/tasks.json`** — Update task descriptions to use "harness" instead of "workspace".
...
| `.octon/output/reports/` | Historical output | 2 |
| `.octon/capabilities/skills/archive/` | Archived content | 5 |
