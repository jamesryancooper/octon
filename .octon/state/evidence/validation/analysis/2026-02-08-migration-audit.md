...
**G1. `.octon/framework/capabilities/skills/scripts/validate-skills.sh` — 31 occurrences** [HIGH]
...
All refer to the harness concept (the `.octon/` directory's skills registry), not pnpm/IDE workspace.

**G2. `.octon/framework/scaffolding/templates/octon/` — 6 template files** [HIGH]
...
**G3. `.octon/framework/capabilities/skills/audit-migration/references/safety.md` — 2 occurrences** [HIGH]
...
**G4. `.octon/state/continuity/repo/entities.json` — 3 issues** [CRITICAL]
...
**G5. `.octon/state/continuity/repo/tasks.json` — 3 occurrences** [MEDIUM]
...
**X1. `.octon/framework/scaffolding/templates/octon/` — 6 broken path references** [HIGH]
...
**X2. `.octon/framework/capabilities/skills/scripts/validate-skills.sh:1940` — broken anchor** [HIGH]
...
**S1. `.octon/framework/capabilities/skills/scripts/validate-skills.sh` — conceptual staleness** [HIGH]
...
**S2. `.octon/framework/scaffolding/templates/octon-docs/orchestration/workflows/are/are-init.sh` — 2 occurrences** [HIGH]
...
**S3. `.octon/framework/capabilities/skills/scripts/generate-reference-headers.sh:24`** [HIGH]

Variable `WORKSPACE_REGISTRY="$REPO_ROOT/.octon/framework/capabilities/skills/registry.yml"` — should be renamed to `HARNESS_REGISTRY` or just `REGISTRY`.
...
**`.octon/state/continuity/repo/entities.json`** — Update entity key from `"docs/architecture/workspaces/"` to `"docs/architecture/harness/"` and fix notes text.

### Batch 2: High — Operational scripts (3 files, ~35 occurrences)

1. **`.octon/framework/capabilities/skills/scripts/validate-skills.sh`** — Rename `WORKSPACE_REGISTRY` → `REGISTRY` (or `HARNESS_REGISTRY`), rename functions, update comments and log messages. Fix broken anchor reference.
2. **`.octon/framework/capabilities/skills/scripts/generate-reference-headers.sh`** — Rename `WORKSPACE_REGISTRY` → `REGISTRY`.
3. **`.octon/framework/scaffolding/templates/octon-docs/orchestration/workflows/are/are-init.sh`** — Replace "workspace-specific" → "harness-specific", "workspace files" → "harness files".

### Batch 3: High — Broken template paths (6 files)

**`.octon/framework/scaffolding/templates/octon/`** — Replace `docs/architecture/harnesses/` → `docs/architecture/harness/` in all 6 README.md files.

### Batch 4: High — Skill reference file (1 file)

**`.octon/framework/capabilities/skills/audit-migration/references/safety.md`** — Replace "not part of workspace" → "not part of harness" (lines 90-91).

### Batch 5: Medium — State files (1 file)

**`.octon/state/continuity/repo/tasks.json`** — Update task descriptions to use "harness" instead of "workspace".
...
|------|-----------|-------|
| `.octon/state/continuity/repo/log.md` | Append-only per D014 | 1 |
| `.octon/framework/cognition/decisions/` | Append-only per D014 | 8 |
| `.octon/framework/cognition/analyses/` | Historical analysis | 1 |
| `.octon/inputs/exploratory/ideation/` | Human-led zone per D003 | 14 |
| `.octon/state/evidence/validation/` | Historical output | 2 |
| `.octon/framework/capabilities/skills/archive/` | Archived content | 5 |
| `.octon/framework/scaffolding/prompts/2026-*` | Historical prompts | 1 |
| `.octon/framework/orchestration/workflows/flowkit/` | FlowKit IDE workspace concept | 5 |
