...
- Policy contract:
  `.octon/agency/practices/standards/ai-gate-policy.json`
- Normalized findings schema:
  `.octon/agency/practices/standards/ai-gate-findings.schema.json`
...
5. Main control-plane matches
   `.octon/agency/practices/standards/github-control-plane-contract.json`.
...
```bash
.octon/agency/_ops/scripts/github/capture-github-control-plane-snapshot.sh
```
...
- Use `.octon/agency/_ops/scripts/git/git-pr-ship.sh` for shipping; it triggers
  local cleanup after closure (or starts a watcher for manual lanes).
- On demand, run `.octon/agency/_ops/scripts/git/git-pr-cleanup.sh`.
...
- Re-run baseline capture before and after rollback:
  `.octon/agency/_ops/scripts/github/capture-github-control-plane-snapshot.sh`
- Compare baseline artifacts in `.octon/output/reports/analysis/` to confirm expected
...
- Validate catalog completeness:
  `.octon/agency/_ops/scripts/validate/validate-autonomy-labels.sh`
