...
- Typically combines:
  - audit, remediation, and refactor workflows in `.octon/orchestration/runtime/workflows/**`
...
User → apps/ai-console
     ...
     → .octon/capabilities/runtime/services (composite service contracts)
...
In both cases, **only** `apps/*`, `agents/*`, and `platform/runtimes/*-runtime/**` are actual processes. Everything else in `packages/*` and `contracts/*` is imported, with composition definitions in `.octon/capabilities/runtime/services/**`.
