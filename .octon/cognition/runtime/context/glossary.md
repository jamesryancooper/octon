...
> **See also:** `.octon/cognition/runtime/context/glossary-repo.md` for repo-wide terminology including task status values, workflow states, and roles.
...
| Harness command | Atomic operation in `.octon/capabilities/runtime/commands/`. |
| Harness workflow | Multi-step procedure in `.octon/orchestration/runtime/workflows/`. |
| Prompt | Task template in `.octon/scaffolding/practices/prompts/` requiring judgment. |
