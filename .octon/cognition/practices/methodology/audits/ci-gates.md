...
1. Audit bundle contract gate
   - Scope checks to in-change audit bundles only (new/modified bundle directories in current diff plus current bundle references touched in `/.octon/cognition/runtime/audits/index.yml`).
   - Fail if any in-scope audit bundle directory under `/.octon/output/reports/audits/` is missing required files.
2. Coverage gate
...
- `/.octon/output/reports/audits/README.md`
