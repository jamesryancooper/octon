...
struct ScoreArgs {
    #[arg(
        long,
        default_value = ".octon/assurance/governance/weights/weights.yml"
    )]
    weights: PathBuf,

    #[arg(
        long,
        default_value = ".octon/assurance/governance/scores/scores.yml"
    )]
    scores: PathBuf,

    #[arg(long, default_value = ".octon/assurance/governance/CHARTER.md")]
    charter: PathBuf,

    #[arg(
        long,
        default_value = ".octon/assurance/governance/weights/inputs/context.yml"
    )]
    context: PathBuf,

    #[arg(
        long,
        default_value = ".octon/assurance/governance/subsystem-classes.yml"
    )]
    subsystem_classes: PathBuf,

    #[arg(long, default_value = ".octon/assurance/governance/overrides.yml")]
    overrides: PathBuf,
    ...
    #[arg(long, default_value = ".octon/output/assurance/effective")]
    effective_dir: PathBuf,

    #[arg(long, default_value = ".octon/output/assurance/results")]
    results_dir: PathBuf,

    #[arg(long, default_value = ".octon/output/assurance/policy/deviations")]
    deviations_dir: PathBuf,

    #[arg(long, default_value = ".octon/assurance/runtime/_ops/state")]
    lock_dir: PathBuf,
}
...
fn default_out_dir(now: &TimestampMeta) -> PathBuf {
    ...
    PathBuf::from(format!(
        ".octon/output/assurance/scorecards/{}/{}",
        day, now.run_id
    ))
}
...
mod tests {
    ...
    fn repo_root() -> PathBuf {
        ...
        manifest_dir
            ...
            .and_then(Path::parent)
            .and_then(Path::parent)
            .expect("assurance_tools should be under <repo>/.octon/engine/runtime/crates/")
            .to_path_buf()
    }
    ...
    fn base_gate_paths() -> (PathBuf, PathBuf, PathBuf, PathBuf, PathBuf) {
        let root = repo_root();
        (
            root.join(".octon/assurance/governance/weights/weights.yml"),
            root.join(".octon/assurance/governance/scores/scores.yml"),
            root.join(".octon/assurance/governance/CHARTER.md"),
            root.join(".octon/assurance/governance/subsystem-classes.yml"),
            root.join(".octon/assurance/governance/overrides.yml"),
        )
    }
    ...
    fn parse_charter_spec_requires_attribute_umbrella_map() {
        let weights = json!({
            "charter": {
                "ref": ".octon/assurance/governance/CHARTER.md",
                "version": "2.0.0",
                "priority_chain": [
                ...
                "required_references": {"charter": ".octon/assurance/governance/CHARTER.md"}
            }
        });
    ...
    fn compute_umbrella_rollups_uses_assurance_hybrid_formula() {
        ...
        let spec = CharterSpec {
            reference_path: ".octon/assurance/governance/CHARTER.md".to_string(),
