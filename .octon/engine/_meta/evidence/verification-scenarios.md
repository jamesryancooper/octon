...
> Notes:
> - The CLI binary is `octon` (kernel crate). For development, use `.octon/engine/runtime/run ...` which falls back to `cargo run`.
...
```bash
.octon/engine/runtime/run serve-stdio
```
...
Edit policy file `.octon/engine/runtime/config/policy.yml` to remove `storage.local` from `interfaces/kv`, then invoke `kv.get`.
...
```bash
.octon/engine/runtime/run studio --help
```
...
- `.octon/continuity/runs/<run_id>/instruction-layer-manifest.json` exists
