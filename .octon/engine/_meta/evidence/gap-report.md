...
- A functional `TraceWriter` exists at `.octon/engine/runtime/crates/core/src/trace.rs`.
- It writes NDJSON trace events to `.octon/engine/_ops/state/traces/<trace_id>.ndjson`.
...
- The server enforces a max line length (default 1 MiB) in `.octon/engine/runtime/crates/core/src/jsonlines.rs`.
