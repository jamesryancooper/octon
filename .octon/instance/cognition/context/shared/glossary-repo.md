...
This glossary defines key terms used throughout Octon documentation.
...
### ASVS
OWASP Application Security Verification Standard. A framework of security requirements that Octon maps to.
...
A development approach using short-lived branches that merge frequently to trunk. Octon's default approach.

### Vercel
The deployment platform Octon integrates with for previews, production, and feature flags.
...
### Prompt
Instructions given to an AI model. In Octon, prompts are structured and versioned.

### Hallucination
When AI generates content that sounds plausible but is incorrect. Octon uses validation and golden tests to catch these.
...
orchestrating multiple services under `.octon/framework/capabilities/runtime/services/`.
