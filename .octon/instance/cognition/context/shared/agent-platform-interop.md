...
Define the canonical Octon-owned semantics for optional platform interoperability.
Octon remains fully operational with zero adapters installed.
...
3. Core semantics are provider-agnostic and owned by Octon.
...
| Capability | Octon Core Owns | Adapter Owns |
|---|---|---|
...
1. `interop_contract_version`
  - Location: `.octon/instance/cognition/context/shared/agent-platform-interop.md`
  - Scope: governance semantics and boundaries
2. `adapter_schema_version`
  - Location: `.octon/framework/capabilities/runtime/services/interfaces/agent-platform/schema/*.json`
  - Scope: machine-readable interop schema
3. `adapter_version`
  - Location: `.octon/framework/capabilities/runtime/services/interfaces/agent-platform/adapters/<id>/adapter.yml`
