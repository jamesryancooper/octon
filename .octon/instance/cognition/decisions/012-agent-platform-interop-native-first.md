...
description: Establish a native-first, provider-agnostic interop contract where Octon owns runtime semantics and adapters remain optional.
status: accepted
date: 2026-02-14
---
...
Octon needs interoperability with external agent platforms while preserving core autonomy.
Before this ADR, interop semantics (session policy, context budgeting, compaction behavior, routing precedence, and presence evidence) were not formalized as Octon-owned contract content.
...
1. Octon must run fully with zero adapters.
2. Octon owns interop semantics in a canonical context contract.
...
- `.octon/framework/capabilities/services/interfaces/agent-platform/`
...
- Session policy and context budgeting become stable Octon semantics.
...
- Phase 0.5 baseline report:
  - `.octon/state/evidence/validation/analysis/2026-02-14-platform-coupling-baseline.md`
- Core schemas:
  - `.octon/framework/capabilities/services/interfaces/agent-platform/schema/capabilities.schema.json`
  - `.octon/framework/capabilities/services/interfaces/agent-platform/schema/session-policy.schema.json`
- Enforcement script extension:
  - `.octon/framework/capabilities/services/_ops/scripts/validate-service-independence.sh`
