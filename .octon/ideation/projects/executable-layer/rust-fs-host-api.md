...
This document provides the **sandboxed filesystem host API** implementation for the Octon kernel. It covers `ScopedFs` (the core sandboxing struct), all FS operations, the `fs::Host` trait implementation, security analysis, and policy recommendations.
...
**Workspace root** = the repo root, i.e. the directory that *contains* the root `.octon/`.

---

## 1) ScopedFs (sandboxed filesystem)

Create: `.octon/runtime/crates/wasm_host/src/scoped_fs.rs`
...
In `.octon/runtime/crates/wasm_host/src/host_api.rs`, alongside the log/clock/kv implementations from [rust-kernel-reference.md §13](rust-kernel-reference.md):
...
- Enforce Octon's "no silent apply" rule at the orchestration/skill layer before enabling write paths
