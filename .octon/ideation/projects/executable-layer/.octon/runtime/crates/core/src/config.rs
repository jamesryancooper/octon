...
impl ConfigLoader {
    pub fn load(octon_dir: &Path) -> Result<RuntimeConfig> {
        ...
        let state_dir = octon_dir.join("state");
        ...
        let cache_config_path = octon_dir.join("runtime").join("config").join("wasmtime-cache.toml");
    ...
    /// - If `.octon/octon.yml` contains `runtime.policy_file`, use that (relative to `.octon/`).
    /// - Else, if `.octon/runtime/config/policy.yml` exists, use that.
    ...
    fn resolve_policy_path(octon_dir: &Path) -> Result<Option<PathBuf>> {
        ...
        let default = octon_dir.join("runtime").join("config").join("policy.yml");
        if default.is_file() {
