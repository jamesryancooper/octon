...
Analyze the architecture and content of @.octon/skills and @.workspace/skills to ensure alignment with Octon's pillars, principles, and production readiness, as well as alignment with Agent Skills spec (<https://agentskills.io/specification>).
...
| **Locality** | Does context live close to where it's needed? Is the two-tier architecture (shared `.octon/` vs local `.workspace/`) correctly separating concerns? |
...
5. **Recommendations**: Prioritized list of improvements that are:
   ...
   - Aligned with Agent Skills spec and Octon principles
