...
Output definitions are in `.octon/capabilities/runtime/skills/registry.yml` (single source of truth).

> **Note:** All `.octon/capabilities/runtime/skills/` categories follow the `{{category}}/{{skill-id}}/` pattern: `_ops/state/configs/`, `_ops/state/resources/`, `_ops/state/runs/`, `_ops/state/logs/`.

Summary:

### {{Primary Output}} (Deliverable)

- **Path:** `.octon/output/{{category}}/{{timestamp}}-{{name}}.md`
- **Format:** Markdown
...
inputs:
  - {{input_path_or_value}}
outputs:
  - .octon/output/{{category}}/{{timestamp}}-{{name}}.md
tools_used:
  - filesystem.read
  - filesystem.glob
  - filesystem.write
