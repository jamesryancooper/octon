...
For full documentation, see [.octon/capabilities/_meta/architecture/](/.octon/capabilities/_meta/architecture/README.md).
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  1. SKILL DEFINITION (/.octon/capabilities/runtime/skills/<skill-id>/)                          │
...
│                                                                             │
│  2. SHARED MANIFEST (/.octon/capabilities/runtime/skills/manifest.yml)                          │
...
│                                                                             │
│  3. REGISTRY (/.octon/capabilities/runtime/skills/registry.yml)                                 │
│     □ Add skill entry under `skills:`:                                      │
│       - version: "1.0.0"                                                    │
│       - commands: [/<skill-id>]                                             │
...
│                                                                             │
│  4. REGISTRY I/O (/.octon/capabilities/runtime/skills/registry.yml)                             │
...
`.octon/capabilities/_meta/architecture/alignment-policy.md`.
...
> **Design Note:** Capabilities determine documentation needs. Each capability maps to specific reference files. See [capabilities.md](/.octon/capabilities/_meta/architecture/capabilities.md) for the full mapping.
...
The template includes guidance for choosing capabilities. See [reference-artifacts.md](/.octon/capabilities/_meta/architecture/reference-artifacts.md) for the complete capability-to-reference mapping.

**Quick command to scaffold and validate:**

```bash
# Copy template
cp -r .octon/capabilities/runtime/skills/_template .octon/capabilities/runtime/skills/<skill-id>

# Edit files (use your editor)
# Then validate
.octon/capabilities/runtime/skills/_ops/scripts/validate-skills.sh <skill-id>

# Use --fix to see scaffolding suggestions for missing entries
.octon/capabilities/runtime/skills/_ops/scripts/validate-skills.sh <skill-id> --fix
```
...
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  TIER 1: SHARED FOUNDATION (/.octon/capabilities/runtime/skills/)                       │   │
│   │  ─────────────────────────────────────────────────────────────────  │   │
...
│                                    ▼                                        │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  TIER 2: HARNESS CONFIG (/.octon/capabilities/runtime/skills/)                        │   │
│   │  ─────────────────────────────────────────────────────────────────  │   │
...
│   │       ▲                ▲                 ▲                          │   │
│   │       └────────────────┴─────────────────┘                          │   │
│   │                   Symlinks to .octon/capabilities/runtime/skills/                      │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
| `skill_class`                                    | `.octon/capabilities/runtime/skills/manifest.yml` |
| `allowed-tools` (tool permissions)               | SKILL.md frontmatter (**authoritative**) |
| `allowed-services`                               | SKILL.md frontmatter (**authoritative**) |
| `summary`, `triggers`, `tags`                    | `.octon/capabilities/runtime/skills/manifest.yml`           |
| `version`, `commands`, `parameters`, `composition`| `.octon/capabilities/runtime/skills/registry.yml`          |
| Input/output paths                               | `.octon/capabilities/runtime/skills/registry.yml`         |

**Tool Permissions:** `allowed-tools` in SKILL.md is the single source of truth. The internal format is derived via the mapping function in `validate-skills.sh`. See [specification.md](/.octon/capabilities/_meta/architecture/specification.md) for details.
...
5. Add entry to `.octon/capabilities/runtime/skills/registry.yml` under `skills.<id>` (version, commands, parameters, composition when applicable)
6. Add I/O mapping to `.octon/capabilities/runtime/skills/registry.yml` under `skills.<id>.io` (inputs, outputs)
...
.octon/capabilities/_ops/scripts/policy-profile-resolve.sh docs
```
...
Skills are exposed to different AI agents (Claude, Cursor, Codex) via symlinks from their respective skills directories to the shared `.octon/capabilities/runtime/skills/` definitions. This allows multiple agents to share the same canonical skill definitions.
...
This creates symlinks for all skills in `.octon/capabilities/runtime/skills/` to each agent's skills directory.
...
ln -s ../../.octon/capabilities/runtime/skills/refine-prompt .claude/skills/refine-prompt
ln -s ../../.octon/capabilities/runtime/skills/refine-prompt .cursor/skills/refine-prompt
ln -s ../../.octon/capabilities/runtime/skills/refine-prompt .codex/skills/refine-prompt
```
...
| Permission denied     | Check file permissions on `.octon/capabilities/runtime/skills/`                     |
...
### Manifest Extensions (`.octon/capabilities/runtime/skills/manifest.yml`)
...
### Registry Extensions (`.octon/capabilities/runtime/skills/registry.yml`)
...
- Active skills using broad write scopes (`Write(...**)`) must have an active lease in `.octon/capabilities/_ops/state/deny-by-default-exceptions.yml`.
...
- [Full Documentation](/.octon/capabilities/_meta/architecture/README.md) — Complete architecture and reference
