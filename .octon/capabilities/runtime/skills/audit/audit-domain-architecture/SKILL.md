...
description: >
  Independent architecture critique for any existing or planned Octon domain
...
Independent architecture critique for any Octon domain target, including existing domains and planned domains not yet created on disk.
...
```text
/audit-domain-architecture domain_path=".octon/cognition"
```

With custom criteria weighting:

```text
/audit-domain-architecture domain_path=".octon/orchestration" criteria="modularity,discoverability,coupling,operability,change-safety,testability"
```
...
- `.octon/output/reports/analysis/YYYY-MM-DD-domain-architecture-audit-<run-id>.md` -- Structured critique report
...
- `domain_path` is outside `.octon/` scope and cannot be normalized safely
