...
build_snapshot() {
  "$RUNTIME_RUN" tool interfaces/filesystem-snapshot snapshot.build --json '{"root":".octon/capabilities/runtime/services/interfaces","state_dir":".octon/engine/_ops/state/snapshots","set_current":false}'
}
...
if has_file_match "\"path\":\"\\.octon/engine/_ops/state/" "$FILES_JSONL"; then
  echo "ERROR: runtime state paths leaked into snapshot artifact"
  print_file_matches "\"path\":\"\\.octon/engine/_ops/state/" "$FILES_JSONL"
  exit 1
fi
