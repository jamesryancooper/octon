...
export OCTON_OPERATION_CLASS="${OCTON_OPERATION_CLASS:-service.execute}"
export OCTON_OPERATION_PHASE="${OCTON_OPERATION_PHASE:-stage}"
source "$SCRIPT_DIR/../../../_ops/scripts/enforce-deny-by-default.sh"
octon_enforce_service_policy "agent" "$0" "$@"


STATE_ROOT=".octon/engine/_ops/state/agent"
CHECKPOINT_DIR="$STATE_ROOT/checkpoints"
RUNS_DIR="$STATE_ROOT/runs"
CONTINUITY_RUNS_DIR=".octon/continuity/runs"
POLICY_FILE=".octon/capabilities/governance/policy/deny-by-default.v2.yml"
POLICY_RUNNER=".octon/engine/runtime/policy"
PROFILE_RESOLVER=".octon/capabilities/_ops/scripts/policy-profile-resolve.sh"
BUDGET_METER=".octon/capabilities/_ops/scripts/policy-budget-meter.sh"
ACP_REQUEST_BUILDER=".octon/capabilities/_ops/scripts/policy-acp-request.sh"
ACP_EVAL=".octon/capabilities/_ops/scripts/policy-acp-eval.sh"
RECEIPT_WRITER=".octon/capabilities/_ops/scripts/policy-receipt-write.sh"
REVERSIBLE_PRIMITIVES_SCRIPT=".octon/capabilities/_ops/scripts/policy-reversible-primitives.sh"
BREAKER_ACTIONS_SCRIPT=".octon/capabilities/_ops/scripts/policy-circuit-breaker-actions.sh"
...
emit_output() {
  ...
  local context_overhead_ratio="${7:-${OCTON_CONTEXT_OVERHEAD_RATIO:-0}}"
...
run_profile_grant_eval() {
  ...
  local -a args
  args=(grant-eval --policy "$POLICY_FILE" --tier "$tier" --request-id "policy-$run_id-$attempt" --agent-id "${OCTON_AGENT_ID:-agent-unknown}" --plan-step-id "agent-preflight-$attempt")
  ...
  if [[ -n "${OCTON_REVIEW_AGENT_ID:-}" ]]; then
    args+=(--has-review-evidence)
  fi
  if [[ -n "${OCTON_QUORUM_TOKEN:-}" ]]; then
...
agent_policy_preflight_loop() {
  ...
  profile="${OCTON_POLICY_PROFILE:-$(infer_policy_profile "$plan_path")}"
  max_attempts="${OCTON_DDB_REMEDIATE_MAX_ATTEMPTS:-2}"
  auto_remediate="${OCTON_DDB_AUTO_REMEDIATE:-true}"
  risk_tier="$(echo "${OCTON_RISK_TIER:-low}" | tr '[:upper:]' '[:lower:]')"
  ...
  while (( attempt < max_attempts )); do
    ...
    local -a grant_args
    grant_args=("$profile" --emit-grant --subject "service:agent:$run_id" --request-id "policy-remediate-$run_id-$attempt" --agent-id "${OCTON_AGENT_ID:-agent-unknown}" --plan-step-id "agent-remediate-$attempt")
    if [[ -n "${OCTON_REVIEW_AGENT_ID:-}" ]]; then
...
build_evidence_json() {
  ...
  jq -n --arg status "${OCTON_CI_STATUS:-pass}" --arg ts "$(date -u +"%Y-%m-%dT%H:%M:%SZ")" '{status:$status,timestamp:$ts}' > "$ci_file"
...
build_attestations_json() {
  ...
  local proposer="${OCTON_AGENT_ID:-agent-local}"
  local now
  ...
  if [[ "${OCTON_ALLOW_INPROCESS_ATTESTATIONS:-false}" == "true" || "${OCTON_DISABLE_QUORUM:-false}" == "true" || "${OCTON_QUORUM_DISAGREE:-false}" == "true" ]]; then
    local verifier recovery verifier_sig recovery_sig
    verifier="${OCTON_VERIFIER_AGENT_ID:-agent-verifier}"
    recovery="${OCTON_RECOVERY_AGENT_ID:-agent-recovery}"
    ...
    if [[ "${OCTON_DISABLE_QUORUM:-false}" == "true" ]]; then
      jq -cn --argjson proposer "$proposer_json" '[ $proposer ]'
      return
    fi

    if [[ "${OCTON_QUORUM_DISAGREE:-false}" == "true" ]]; then
      jq -cn \
  ...
  inline_attestations="${OCTON_ATTESTATIONS_JSON:-[]}"
  ...
  if [[ -n "${OCTON_ATTESTATION_FILES:-}" ]]; then
    local -a raw_files=()
    IFS=',' read -r -a raw_files <<<"${OCTON_ATTESTATION_FILES:-}"
...
handle_circuit_trip() {
  ...
  "$BREAKER_ACTIONS_SCRIPT" run \
    ...
    --scope "service:agent" \
    --owner "${OCTON_AGENT_ID:-agent-local}" >/dev/null 2>&1 || true
}
...
if [[ -z "$run_id" ]]; then
  if [[ "${OCTON_DETERMINISTIC_RUN_ID:-false}" == "true" ]]; then
...
if [[ -x "$BUDGET_METER" ]]; then
  ...
  "$BUDGET_METER" add --file "$counters_file" --metric "net.calls" --value "${OCTON_NET_CALLS:-0}" >/dev/null
...
if [[ -z "$operation_class" || "$operation_class" == "null" ]]; then
  operation_class="${OCTON_OPERATION_CLASS:-git.commit}"
fi
...
if [[ "$reversibility_json" == "{}" || "$reversibility_json" == "null" ]]; then
  ...
  reversibility_json="$(jq -cn \
    ...
    --arg recovery_window "${OCTON_RECOVERY_WINDOW:-P30D}" \
...
"$ACP_REQUEST_BUILDER" \
  ...
  --profile "$profile" \
  --operation-class "$operation_class" \
  --actor-id "${OCTON_AGENT_ID:-agent-local}" \
  --actor-type "agent" \
  --target-json "$target_json" \
