...
Services are **control‑plane capabilities** that live under `.octon/capabilities/runtime/services/*` and are reused across the entire repo.

- **Primary placement (services):**
  - `.octon/capabilities/runtime/services/<domain>/<service-id>/...`
  - Source of truth for:
...
- **Secondary placements (runtimes/adapters) are consumers of services:**
  - `apps/*` — thin HTTP/CLI adapters that call services in `.octon/capabilities/runtime/services/*`.
...
- **Flow (service contract interface):**
  - `.octon/capabilities/runtime/services/execution/flow/` exposes typed input/output contracts and runtime semantics to launch flows (native WASM by default, optional HTTP adapter when configured).
...
- Architecture: services = control‑plane capabilities under `.octon/capabilities/runtime/services`; the LangGraph runtime is infrastructure under `agents/runner/runtime/**`.
- Methodology: services = concrete tools used to implement the Spec → Plan → Flow → Run lifecycle; see also `.octon/capabilities/runtime/services/execution/service-roles.md` for canonical roles.
...
| --- | --- | --- | --- | --- | --- | --- |
| Spec | Produce spec one‑pager + ADR | spec | `.octon/capabilities/runtime/services/planning/spec/schema/input.schema.json` | `docs/specs/*.md`, `docs/specs/adr-*.md` | `service.spec.specify` | Policy preflight (ASVS/SSDF), Observe trace open |
| Plan | Produce plan (BMAD) from spec | plan | `.octon/capabilities/runtime/services/planning/plan/schema/input.schema.json` | `plan.json` | `service.plan.plan` | Policy ruleset selected; dry‑run OK |
| Agent | Execute plan (produce artifacts only) | implement | `.octon/capabilities/runtime/services/execution/agent/schema/input.schema.json` | Proposed diffs, tests, notes under `runs/**` | `service.agent.execute` | Guard redaction; idempotency required on mutating ops |
| Tool | Deterministic action wrappers (Git/HTTP/Shell) | implement | `.octon/capabilities/runtime/services/operations/tool/schema/input.schema.json` | Structured logs, proposed changes | `service.tool.call.<action>` | Guard + Cache; fail closed on secret/redaction errors |
| Eval | Verify structure/grounding/style | verify | `.octon/capabilities/runtime/services/quality/eval/schema/input.schema.json` | `runs/eval/*.json` | `service.eval.verify` | Thresholds enforced; fail‑closed on miss |
| Policy | Evaluate policy rulesets (ASVS/SSDF/STRIDE) | spec·plan·verify·ship | `.octon/capabilities/runtime/services/governance/policy/schema/input.schema.json` | `runs/policy/*.json` | `service.policy.check` | Fail‑closed by default (`policy.failClosed = true`) |
| Test | Contract/unit/e2e invoker | verify | `.octon/capabilities/runtime/services/quality/test/schema/input.schema.json` | `runs/test/*.json` | `service.test.run` | OpenAPI diff fail‑closed; contract tests required on changed surfaces |
| Patch | Open PR + changelog | ship | `.octon/capabilities/runtime/services/delivery/patch/schema/input.schema.json` | PR number, preview URL | `service.patch.open_pr` | Feature flag OFF by default; preview smoke recommended |
| Release | Tag and release | ship | `.octon/capabilities/runtime/services/delivery/release/schema/input.schema.json` | Tag, changelog | `service.release.tag` | Requires green policy/eval gates |
| Flag | Server‑side flag evaluation/toggles | ship·operate | `.octon/capabilities/runtime/services/delivery/flag/schema/input.schema.json` | Flag states/rollout plan | `service.flag.evaluate`, `service.flag.toggle` | Progressive delivery; rollback path ready |
...
| Compliance | Assemble evidence pack | verify·ship·learn | `.octon/capabilities/runtime/services/governance/compliance/schema/input.schema.json` | Evidence pack manifest under `runs/**` | `service.compliance.assemble` | Required for high‑risk changes |
| Cache | Idempotency + memoization | implement | `.octon/capabilities/runtime/services/operations/cache/schema/input.schema.json` | Cache hit/miss records | `service.cache.hit`, `service.cache.miss` | Pure ops must declare `--cache-key` |
| Parse | Convert PDFs to Markdown (tables/figures) | implement | `.octon/capabilities/runtime/services/retrieval/parse/schema/input.schema.json` | `docs_out/parsed/*.md`, `runs/**/parse-*.json` | `service.parse.run` | Guard redaction; dry‑run OK |
| Prompt | Manage prompt templates, variables, variants, and fixtures | implement·verify | `.octon/capabilities/runtime/services/modeling/prompt/schema/input.schema.json` | `prompts_out/**` compiled prompts, `prompt_tests/**` fixtures, `runs/**/prompt-*.json` prompt metadata | `service.prompt.compile` | Eval/Test/Policy gates on prompt drift and schema violations |
...
- Contracts updated in the service-local schema path (`.octon/capabilities/runtime/services/<domain>/<service>/schema`); registry references refreshed; diffs linked in PR.
...
- Location: `.octon/capabilities/runtime/services`
  - OpenAPI: service-specific files as needed in each service folder.
  - JSON Schemas (services): `.octon/capabilities/runtime/services/<domain>/<service>/schema/`
...
- Registry exports: update `.octon/capabilities/runtime/services/registry.yml` (or service-local docs) to expose new or updated contracts to consumers.
...
- Contracts live alongside services under `.octon/capabilities/runtime/services/<domain>/<service>/schema` (OpenAPI + JSON‑Schema where applicable).
...
- Tests:
  ...
  - Place adapter contract tests adjacent to adapters; keep service contract references under `.octon/capabilities/runtime/services/**/schema`.
- CI gates:
