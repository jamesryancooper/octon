# Developer Guide — Comms Stack & Agent Layer (Octon)

**Audience:** engineers building/operating the AI Services Platform services and the Agent layer.
**Scope:** how services talk to each other today, how agents orchestrate work, and how to expose services as MCP providers—while staying true to the Octon Lean AI Accelerated Methodology.
...
## 1) Octon-aligned principles (you'll see these threads everywhere)
...
**Bottom line:** Keep core service-to-service comms boring (typed calls + artifacts + events). Use the Agent service for judgment where it clearly pays off. Add MCP providers as a facade when agents (yours or external) need to call services as tools—all without bending Octon's complexity-calibrated guardrails.
