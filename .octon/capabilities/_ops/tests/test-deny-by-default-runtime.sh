...
run_split_parser_regression_test() {
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      source '$ENFORCER_SCRIPT'
      mapfile -t tokens < <(octon_ddb_split_allowed_tools 'Read Bash(bash .octon/capabilities/runtime/services/governance/guard/impl/guard.sh *)')
      [[ \${#tokens[@]} -eq 2 ]]
      [[ \"\${tokens[1]}\" == 'Bash(bash .octon/capabilities/runtime/services/governance/guard/impl/guard.sh *)' ]]
    "
}
...
run_acp_gate_tests() {
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      receipt='.octon/continuity/runs/$phase_missing_run_id/receipt.latest.json'
      [[ -f \"\$receipt\" ]]
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      request_file='.octon/capabilities/_ops/state/.tmp/acp/$receipt_run_id-guard-request.json'
      decision_file='.octon/capabilities/_ops/state/.tmp/acp/$receipt_run_id-guard-decision.json'
      [[ ! -e \"\$request_file\" ]]
      [[ ! -e \"\$decision_file\" ]]
    "
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      receipt='.octon/continuity/runs/$receipt_run_id/receipt.latest.json'
      [[ -f \"\$receipt\" ]]
      .octon/engine/runtime/policy receipt-validate --policy '$POLICY_V2_FILE' --receipt \"\$receipt\" >/dev/null
    "
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      receipt='.octon/continuity/runs/$receipt_run_id/receipt.latest.json'
      digest='.octon/continuity/runs/$receipt_run_id/digest.latest.md'
      [[ -f \"\$receipt\" ]]
      [[ -f \"\$digest\" ]]
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      [[ -s '.octon/capabilities/_ops/state/logs/acp-decisions.jsonl' ]]
    "
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      receipt='.octon/continuity/runs/$acp4_deny_run_id/receipt.latest.json'
      [[ -f \"\$receipt\" ]]
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      digest='.octon/continuity/runs/$acp4_deny_run_id/digest.latest.md'
      [[ -f \"\$digest\" ]]
      grep -F -- 'Effective ACP:' \"\$digest\" >/dev/null
      grep -F -- 'ACP-4' \"\$digest\" >/dev/null
      grep -F -- 'RA_BREAK_GLASS_REQUIRED' '.octon/capabilities/_ops/state/logs/acp-decisions.jsonl' >/dev/null
      grep -F -- '\"run_id\":\"$acp4_deny_run_id\"' '.octon/capabilities/_ops/state/logs/acp-decisions.jsonl' >/dev/null
    "
}
...
run_direct_acp_enforce_emit_receipt_tests() {
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      ...
      receipt='.octon/continuity/runs/$run_id/receipt.latest.json'
      digest='.octon/continuity/runs/$run_id/digest.latest.md'
      set +e
      ...
      ' '.octon/capabilities/_ops/state/logs/acp-decisions.jsonl' >/dev/null || {
        echo 'run_id missing from acp decision log'
        tail -n 20 '.octon/capabilities/_ops/state/logs/acp-decisions.jsonl'
        exit 1
      }
    "
...
run_service_wrapper_budget_metering_tests() {
  ...
  assert_failure_contains \
    ...
    bash -euo pipefail -c "
      source '$ENFORCER_SCRIPT'
      target_file='.octon/cognition/governance/principles/README.md'
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      receipt='.octon/continuity/runs/$budget_run_id/receipt.latest.json'
      [[ -f \"\$receipt\" ]]
...
run_acp_breaker_action_tests() {
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      rollback_log='.octon/continuity/runs/$breaker_run_id/rollback/rollback-attempt.txt'
      [[ -f \"\$rollback_log\" ]]
...
run_agent_quorum_independence_tests() {
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      payload=\"\$(jq -cn --arg run_id '$run_stage_only' --arg plan '.octon/cognition/governance/principles/autonomous-control-points.md' '
        {
          mode:\"execute\",
          runId:\$run_id,
          planPath:\$plan,
  ...
  assert_success \
    ...
    bash -euo pipefail -c "
      attestation_dir='.octon/continuity/runs/$run_allow/attestations'
      ...
      payload=\"\$(jq -cn --arg run_id '$run_allow' --arg plan '.octon/cognition/governance/principles/autonomous-control-points.md' '
        {
          mode:\"execute\",
          runId:\$run_id,
          planPath:\$plan,
