...
create_fixture_repo() {
  ...
  local -a required_files=(
    ...
    ".octon/capabilities/governance/policy/acp-operation-classes.md"
    ".octon/capabilities/governance/policy/flags.metadata.json"
    ".octon/capabilities/governance/policy/flags.metadata.schema.json"
    ".octon/capabilities/_ops/scripts/policy-receipt-write.sh"
    ".octon/capabilities/_ops/scripts/policy-circuit-breaker-actions.sh"
...
case_detects_affirmative_legacy_terms() {
  ...
  mkdir -p "$fixture_root/.octon/cognition/governance/principles"
  cat > "$fixture_root/.octon/cognition/governance/principles/deny-by-default.md" <<'EOF'
...
case_detects_pre_promote_human_approval_language() {
  ...
  mkdir -p "$fixture_root/.octon/cognition/practices/methodology"
  cat > "$fixture_root/.octon/cognition/practices/methodology/gates.md" <<'EOF'
...
case_allows_explicit_negation_terms() {
  ...
  mkdir -p "$fixture_root/.octon/cognition/governance/principles"
  cat > "$fixture_root/.octon/cognition/governance/principles/autonomous-control-points.md" <<'EOF'
...
case_detects_tracked_temp_artifacts() {
  ...
  mkdir -p "$fixture_root/.octon/output/reports/.tmp"
  printf 'temp artifact\n' > "$fixture_root/.octon/output/reports/.tmp/fixture.tmp"
  (
    cd "$fixture_root"
    git add ".octon/output/reports/.tmp/fixture.tmp"
  )
