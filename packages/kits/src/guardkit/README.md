...
```bash
# Run GuardKit tests
pnpm --filter @harmony/kits test src/guardkit/__tests__/
```
...
## Next Steps (Setup Recommendations)

After implementing GuardKit, complete these steps to fully integrate AI guardrails:

### 1. Run the Tests
```bash
pnpm --filter @harmony/kits test
```
...
### 3. Try the CLI Command
```bash
harmony check <file>
harmony check --verify-imports src/
```

### 4. Set Up Weekly Golden Test Runs

Add to CI (`.github/workflows/golden-tests.yml`):
```yaml
name: Weekly Golden Tests
on:
  schedule:
...
jobs:
  golden-tests:
    ...
    steps:
      ...
      - run: pnpm install
      - run: pnpm --filter @harmony/prompts test:golden
      - name: Upload Report
        uses: actions/upload-artifact@v4
        with:
          name: golden-test-report
          path: packages/prompts/golden-reports/
```
...
- Significant output drift detected
