...
export class PromptLoader {
    constructor(catalog, promptsRoot = PROMPTS_ROOT) {
    ...
    load(promptId) {
        ...
        const examples = existsSync(examplesDir)
            ? readdirSync(examplesDir).filter((f) => f.endsWith(".yaml") || f.endsWith(".json"))
            : [];
        const loaded = {
            id: promptId,
            config,
     ...
     */
    loadExample(promptId, exampleName) {
 ...
 * Get the path to a prompt's directory.
 */
export function getPromptPath(promptId) {
