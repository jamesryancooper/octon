 ...
 */
export function checkForHallucinations(content, context) {
    const matches = [];
    // Run all indicators
    ...
    const confidence = Math.min(1, matches.reduce((acc, m) => acc + severityWeights[m.severity], 0));
    // Generate summary
 ...
 */
export function quickHallucinationCheck(content) {
    const quickPatterns = [
 ...
 */
export function validateWithHallucinationCheck(validationResult, content, context) {
    const hallucination = checkForHallucinations(typeof content === "string" ? content : JSON.stringify(content), context);
    const overallSafe = validationResult.valid &&
 ...
 */
export function formatHallucinationReport(result) {
    if (!result.detected) {
    ...
    for (const match of result.matches) {
        const icon = match.severity === "high" ? "🔴" : match.severity === "medium" ? "🟡" : "🟢";
