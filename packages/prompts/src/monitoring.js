...
export class GoldenTestMonitor {
    constructor(dataDir, thresholds = {}) {
     ...
     */
    recordRun(promptId, testSummary, model, temperature) {
     ...
     */
    calculateDrift(previous, current) {
        const details = [];
        let driftScore = 0;
        // Pass rate change
        const passRateChange = Math.abs(current.summary.passRate - previous.summary.passRate);
        if (passRateChange > 0.1) {
            details.push(`Pass rate changed by ${(passRateChange * 100).toFixed(1)}%`);
            driftScore += passRateChange;
        }
        // Check for new failures
        const previousFailedIds = new Set(previous.results.filter((r) => !r.passed).map((r) => r.testId));
        const newFailures = current.results.filter((r) => !r.passed && !previousFailedIds.has(r.testId));
        if (newFailures.length > 0) {
            details.push(`${newFailures.length} new test(s) failed`);
            driftScore += newFailures.length * 0.1;
        }
        // Check for recovered tests
        const currentFailedIds = new Set(current.results.filter((r) => !r.passed).map((r) => r.testId));
        const recovered = previous.results.filter((r) => !r.passed && !currentFailedIds.has(r.testId));
     ...
     */
    getMetrics(promptId, days = 30) {
        const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
        const allRuns = this.getRuns(promptId, 100);
        const runs = allRuns.filter((r) => new Date(r.timestamp).getTime() > cutoff);
        if (runs.length === 0) {
            return {
                promptId,
        ...
        const avgPassRate = runs.reduce((sum, r) => sum + r.summary.passRate, 0) / runs.length;
        ...
        if (avgPassRate < this.thresholds.minPassRate) {
            recommendations.push(`Pass rate (${(avgPassRate * 100).toFixed(1)}%) is below threshold (${(this.thresholds.minPassRate * 100).toFixed(0)}%)`);
        }
        if (trend < -0.1) {
            recommendations.push("Pass rate is declining - investigate recent changes");
        }
        if (consistency < this.thresholds.minConsistency) {
            recommendations.push("Results are inconsistent - consider lowering temperature or adding constraints");
        }
        if (runs.length < 5) {
    ...
    checkAlerts(promptId) {
        ...
        if (metrics.avgPassRate < this.thresholds.minPassRate) {
            alerts.push({
                ...
                type: "pass_rate_drop",
                severity: metrics.avgPassRate < this.thresholds.minPassRate - 0.1
                    ? "critical"
                    : "warning",
                promptId,
        ...
        if (latestRun && latestRun.summary.failed > 0) {
            ...
            const consecutiveFailures = recentRuns.filter((r) => r.summary.failed > 0).length;
 ...
 */
export function generateWeeklySummary(monitor, promptIds) {
    const lines = [
    ...
    for (const promptId of promptIds) {
        ...
        const trendIcon = metrics.trend > 0.05 ? "📈" : metrics.trend < -0.05 ? "📉" : "➡️";
        const alertCount = alerts.length;
        const alertIcon = alerts.some((a) => a.severity === "critical")
            ? "🔴"
            : alertCount > 0
                ? "🟡"
                : "✅";
        lines.push(`| ${promptId} | ${(metrics.avgPassRate * 100).toFixed(0)}% | ${trendIcon} | ${alertIcon} ${alertCount} |`);
