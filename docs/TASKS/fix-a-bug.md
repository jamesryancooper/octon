...
This guide walks you through fixing a bug using Harmony's AI-assisted workflow.
...
```bash
harmony fix "#423"              # Start from issue number
# or
harmony fix "login button broken on mobile"  # Start from description
...
```bash
harmony fix "error message shows wrong text when payment fails"
```

**Option C: From an error you're seeing**
```bash
harmony fix "TypeError: Cannot read property 'email' of undefined in UserService"
```
...
```bash
harmony fix "users reporting slow load times"
```
...
See [RISK-TIERS.md](../RISK-TIERS.md) for what T2 involves.

### You Disagree with AI's Fix

```bash
# See AI's reasoning
harmony explain "why did you use this approach?"

# Try a different approach
harmony retry --constraint "use try-catch instead of null check"
...
```bash
harmony fix "critical: all API calls returning 500" --priority critical
```
...
**Less useful:**
```bash
harmony fix "app is broken"
```

**More useful:**
```bash
harmony fix "clicking submit on /settings crashes with TypeError"
```

### Include Error Messages

```bash
harmony fix "Error: ECONNREFUSED when calling payment API"
```
...
```bash
# Give more context
harmony fix "bug description" --context "it's in the checkout flow"

# Point to specific file
harmony fix "bug description" --file src/checkout/OrderSummary.tsx
```
...
harmony retry "#425" --feedback "fix didn't handle the edge case where quantity is 0"
```
