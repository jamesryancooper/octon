...
| **T3** | `tier:3` | Full (+ STRIDE review, navigator, security) | Thorough review |
...
| Navigator review | Second reviewer | ✅ | Required |
...
- [ ] **Spec approval**: Owner + Navigator before build
...
Any team member can bump a PR to a higher tier:
...
| T3 → T2 | Yes | Navigator approval |
| T3 → T1 | Yes | Navigator + security approval |
...
| T3 | Navigator + Security | Must have explicit approval |
...
Reference: Use the PatchKit PR Template (canonical) in `docs/harmony/ai/kits/automation-and-delivery/patchkit/guide.md` to standardize PR bodies, determinism/provenance notes, and tier information.
