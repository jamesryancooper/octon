...
CostKit provides comprehensive cost management for AI-enabled workflows:
...
| Kit | Integration |
|-----|-------------|
| **PlanKit** | Cost estimates before planning, model selection by tier |
| **AgentKit** | Cost tracking per operation, budget checks before expensive operations |
| **FlowKit** | Workflow cost estimation, cost-aware routing |
| **ModelKit** | Model catalog, pricing data |
| **ObservaKit** | Cost metrics/spans, usage reporting |
| **PolicyKit** | Budget enforcement policies |
...
```typescript
// Let CostKit select the appropriate model
const model = costKit.selectModel(task.tier, stage);
...
CostKit integrates with ObservaKit for metrics:
...
- [ObservaKit Integration](../../observability-and-ops/observakit/guide.md)
