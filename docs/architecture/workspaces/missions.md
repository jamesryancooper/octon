...
Missions are **time-bounded sub-projects** within a workspace. They provide isolated progress tracking for parallel workstreams or large initiatives that need their own goal, tasks, and memory.
...
Missions let you run parallel workstreams within the same workspace without polluting each other's progress tracking.
...
## How Missions Relate to Workspace
...
graph TB
    ...
    progress -.->|workspace-level| ws_tasks[workspace tasks.json]
```
