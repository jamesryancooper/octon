...
**Tags are not archetypes.** A skill's archetype (Atomic, Atomic with examples, Complex) is determined by its documentation needs, not its semantic category.
...
Both `validate-schema` and `format-json` might be Atomic skills (same archetype) but have different tags (different semantic categories).
