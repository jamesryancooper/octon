...
The `context/` directory contains **background knowledge** that agents need to work effectively in the workspace's domain.
...
External systems, APIs, or packages the workspace interacts with.
...
| Abbreviations | Repeated terms | "workspace" → "ws" in context |
| Remove examples | After pattern is clear | Keep 1, remove 2-3 |
```

### Rules

- SHOULD include only strategies relevant to this workspace
...
Reference of available tools and capabilities for agents working in this workspace.
...
- SHOULD list only tools relevant to this workspace's domain
- SHOULD include common usage patterns
- MAY include workspace-specific tool configurations
