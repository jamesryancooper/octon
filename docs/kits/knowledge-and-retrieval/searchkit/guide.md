...
- **Integrates with:** PlanKit/AgentKit (orchestration), IngestKit (downstream normalization), DepKit (release notes/advisories), StackKit (architecture rationales).
- **I/O:** URLs/APIs → `sources/**` snapshots (md/json/html/metadata); handed to IngestKit as inputs for `ingest/*.jsonl` builds.
- **Wins:** Citable, LLM‑ready external evidence on demand; removes scraping/anti‑bot complexity from other kits.
