...
**Audience:** engineers building/operating the AI‑Toolkit kits and the Agent layer.
**Scope:** how kits talk to each other today, how agents orchestrate work, and how to expose kits as MCP providers—while staying true to the Harmony Lean AI Accelerated Methodology.
...
* Sync in‑process calls: kits call ports with JSON‑Schema’d I/O and generated types.
...
Added (when agents use kits as tools):

* MCP provider per kit (adapter façade) via stdio (local/dev) or wss (remote).
* Tools map 1:1 to kit ops; params/results reuse your JSON Schemas.
...
* **Deterministic services first.** Kits are typed, idempotent capabilities with predictable side‑effects.
...
## 4) Agent layer (policies on top of deterministic kits)

* **Role:** agents apply judgment (planning, selection, trade‑offs); kits provide the capabilities.
* **Shape:** “agent shells” wrap steps that need reasoning; everything else is direct kit calls.
...
* **Budgets:** per‑run ceilings (calls, tokens, seconds); enforced by AgentKit and respected by kits.
...
* **AgentKit as an MCP client:** discovers providers, lists tools/resources, and invokes tools with run/budget/timebox controls; still emits structured `decision_summary` and tracing.

---

## 5) MCP integration (make kits tool‑ready for agents)

* **Adapter pattern:** add an **MCP Provider** beside each kit; it forwards to the same internal port methods.
* **Transport:** `stdio` for local/dev; `wss` (with mTLS or signed tokens) for remote.
* **Protocol:** JSON‑RPC message shapes defined by MCP; methods correspond to your kit operations.
* **Tools:** 1:1 with kit operations. Parameters/returns reuse your JSON Schemas.
...
* **Capability discovery:** each kit exposes `/capabilities` or MCP `capabilities` with `{tool, versions, limits}`.
...
* **Logging:** structured JSON with `{ts, level, kit, tool, run_id, trace_id, caller, attempt, duration_ms}` (common picks: pino for TS, structlog for Python).
...
* **AuthN/Z:**
  ...
  * MCP over `wss`: mTLS or signed JWT with audience = kit; short TTL; rotate keys.
...
* **DLQ:** event consumers push poison messages to DLQ with reason; alert and surface in ObservaKit.
...
* **How do agents collaborate?** Through **AgentKit**, which sequences deterministic kit calls and wraps judgment steps in small agent shells; MCP is an adapter, not a replacement.
...
**Bottom line:** Keep core kit‑to‑kit comms boring (typed calls + artifacts + events). Use AgentKit for judgment where it clearly pays off. Add MCP providers as a façade when agents (yours or external) need to call kits as tools—all without bending Harmony’s simplicity‑first guardrails.
