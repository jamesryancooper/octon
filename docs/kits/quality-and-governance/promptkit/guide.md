...
PromptKit is the **runtime compiler/renderer** that transforms static prompt templates from `@harmony/prompts` into ready-to-use prompts with determinism guarantees. It provides the essential runtime layer for AI agent prompt management.
...
- `PromptKit` provides the **runtime layer**: compilation, rendering, hashing, variant selection
...
```typescript
const assembled = promptKit.assemble({
  system: compiledSystemPrompt,  // CompiledPrompt or string
  user: compiledUserPrompt,
});

// Convert to provider format
const openaiMessages = promptKit.toOpenAIFormat(assembled);
const anthropicFormat = promptKit.toAnthropicFormat(assembled);
```
...
```typescript
const variant = promptKit.selectVariant('spec-from-intent', {
  tier: 'T1',
  stage: 'draft',
...
```typescript
const result = promptKit.validate('spec-from-intent', { intent: '...' });
...
PromptKit provides sophisticated token budget management:

```typescript
const compiled = promptKit.compile('spec-from-intent', variables, {
  maxTokens: 8000,
...
PromptKit uses Nunjucks (Jinja2-like) syntax with custom filters:
...
PromptKit provides a CLI for development and debugging:
...
PromptKit emits spans for tracing:
...
2. **Use tier-appropriate variants** — Let PromptKit select variants automatically based on tier.
...
| Concern | PromptKit | @harmony/prompts |
|---------|-----------|------------------|
...
A: Add the template to `@harmony/prompts`, register in `catalog.yaml`, then use via PromptKit.
