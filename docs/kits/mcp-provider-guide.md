...
**Harmony-aligned.** MCP is an adapter on your hexagonal boundary. It exposes existing kit operations as **tools** and large data as **resources** for agents (local or remote) without changing your core comms stack.

---

## 1) Provider architecture

* **Adapter, not rewrite:** Keep kit ports intact. Provider calls the same internal methods.
* **Transports:**

  * **Local:** `stdio` (no network, simplest for dev/tests).
  * **Remote:** `wss` with mTLS/JWT, behind ingress with egress controls.
* **Protocol:** JSON-RPC as defined by MCP; 1:1 mapping to kit ops.

---

## 2) Tool mapping (contracts first)

* **Schema reuse:** Tool params/returns reuse the kit’s JSON Schemas.
* **Naming:** `kit.operation` (e.g., `index.build`, `ingest.load`).
...
* **AuthN:**
  ...
  * Remote `wss` → **mTLS** or **JWT** (short TTL, `aud` = kit name, issuer pinned).
...
* **Cross-matrix:** maintain `AgentKit vX ↔ Provider vY` compatibility matrix in docs.
...
* **Logging:** structured logs `{kit, tool, caller, run_id, trace_id, duration_ms, attempt, outcome}`.
...
* **Local:** start provider alongside the kit (stdio); AgentKit connects directly.
* **Staging/Prod:** deploy behind ingress; terminate TLS; enforce client certs or JWT.
* **Rollouts:** canary by routing % of AgentKit traffic to new provider version; monitor SLOs & error budget burn.
