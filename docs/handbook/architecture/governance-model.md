...
- Static Analysis Gate: No new high‑severity findings; formatting and style enforced. Treat critical warnings as errors.
- Security Scan Gate: Dependency and container scanning, SAST, and other controls must report no new critical issues. Security waivers required for exceptions.
...
- Policy Compliance Gate: Custom rules enforce architectural/policy constraints (e.g., authZ presence on public APIs). Violations fail CI.
