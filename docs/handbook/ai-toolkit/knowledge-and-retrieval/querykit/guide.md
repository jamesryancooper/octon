# QueryKit — Hybrid Retrieval & Answering
...
- **Integrates with:** IndexKit (stores), Dockit/DevKit/StackKit/UIkit (callers), AgentKit (runs), EvalKit (ground‑truth checks), CacheKit (snapshots).
...
- **Wins:** Grounded, cited answers; machine‑actionable evidence that downstream kits can trust.
- **Harmony alignment:** advances interoperability and grounded outputs via consistent answer/evidence contracts and citations; exposes governance hooks so answers are evaluable by EvalKit.
...
- **Common Qs:** *Rebuild indexes?* Requests IndexKit to refresh; does not build itself. *Missing sources?* Expands hops on link graph, then re‑ranks.
...
- IndexKit preserves `chunk_id` in artifacts; QueryKit propagates those into citations and evidence JSON.
...
- LTR (learning‑to‑rank): later, plug models trained from EvalKit signals (clicks, labels) to refine fused candidates.
...
These options let QueryKit leverage new IndexKit modes and artifacts with minimal changes to your existing `strategy` and adapters.
...
Dataset slice
- Use the same 5–10% snapshot as IndexKit pilots. Split eval into global/long vs local/pointed queries.
...
Ops & observability
- Log route, knobs, corpus hash, and latency/token usage to ObservaKit. Use EvalKit to compute Recall@k/MRR/answer accuracy.

## Ephemeral Index Adapters (in‑memory, off‑disk)

Use these when you want small‑to‑mid corpora retrieval without writing persistent index artifacts. Adapters build in RAM at runtime and are discarded when the process exits. IndexKit remains the source of truth for reproducible, on‑disk snapshots; QueryKit’s adapters are runtime conveniences.
...
Add an `adapters` block to your QueryKit request alongside `strategy`.
...
- If `source` points to IndexKit artifacts, adapters warm from disk but do not write new artifacts.
- If you omit `source`, adapters can scan normalized inputs or raw files (slower, but zero setup).
- QueryKit should log corpus hash + adapter params to ObservaKit for reproducibility.

### ANN Runtime Knobs

- FAISS IVF: `nprobe` controls recall/latency at query time.
- HNSW (hnswlib): `efSearch` controls recall/latency at query time.
- Expose via `adapters.dense` or `strategy`: QueryKit passes through to the backend without mutating persisted indexes.
...
### When to prefer IndexKit instead
...
QueryKit can retrieve from databases instead of (or alongside) on‑disk artifacts. This fits a “publish → serve” pattern where IndexKit remains the source of truth and a DB provides filters/joins and shared access. For small→mid corpora, a unified PostgreSQL stack keeps ops simple and fast.
...
- Mixed sources: you can use DB for `dense`, and local `keyword.json` / `links.jsonl` for others; QueryKit fuses scores regardless of source.
- Citations: store `uri` and metadata in `docs` so QueryKit can cite `path:line` after retrieving content for evidence packs.

PostgreSQL schemas (see IndexKit “Publishing to Databases” for full DDL)
...
- Keep DSNs in secrets (VaultKit) and prefer least‑privilege DB roles.
- Log adapter parameters and corpus IDs to ObservaKit for reproducibility.

### ParadeDB adapter (optional)

ParadeDB enhances PostgreSQL with search capabilities (e.g., BM25/semantic modules). You can point QueryKit’s keyword adapter to a ParadeDB‑backed search index if you prefer its interface and ranking.
...
- You can mix ParadeDB for keyword and pgvector for dense; QueryKit fuses scores as usual.
...
- If IngestKit labeled chunks with `lang`, QueryKit can adjust fusion weights per locale and prefer multilingual embeddings.
