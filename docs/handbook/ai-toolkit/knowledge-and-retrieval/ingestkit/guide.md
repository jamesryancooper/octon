# IngestKit — Normalization & Enrichment

- **Purpose:** Normalize internal/external sources into consistent, provenance‑rich chunks for indexing, adding an AI‑aligned ingestion pipeline to Harmony.
- **Responsibilities:** parsing multi‑format inputs, token‑aware chunking, deduplicating by content hash, labeling provenance/semantics, enforcing redaction via GuardKit.
- **Harmony alignment:** Advances Interoperability and Provenance with consistent metadata contracts and deterministic chunking; exposes GuardKit hooks and defers policy/eval gates downstream.
- **Integrates with:** SearchKit (external sources), PlanKit/AgentKit (orchestration), IndexKit (consumes chunks), DatasetKit (corpora), GuardKit (redaction).
...
- **Common Qs:**
  ...
  - *Redaction?* Enforce GuardKit patterns before writing `ingest/*.jsonl`.

## Chunk‑First Contract (Deterministic)

- Produces token‑aware, deterministic chunks with stable IDs so downstream kits can retrieve at chunk granularity.
...
- Apply near‑dup at the chunk level to keep unique evidence granularity for QueryKit.
...
- Redact PII/keys via GuardKit before hashing and storage; store redaction version in metadata.
...
- Exact + near‑duplicate filtering prior to IndexKit.
...
These extend IngestKit for newer indexing families that improve RAG on long/global queries and high‑recall scenarios. Each builder writes artifacts alongside your normal `ingest/*.jsonl` chunks so IndexKit and QueryKit can consume them without changing base contracts.
...
- Store the KG and summaries for QueryKit’s global/insight routes.
...
- Ingest changes are minimal; ensure dense and sparse signals are normalized and metadata records their scales so IndexKit can co‑index safely.
...
- Keep all advanced artifacts deterministic where possible (stable IDs, hashes) so they can be referenced in QueryKit citations.
...
Success signals (hand off to IndexKit/QueryKit/EvalKit)
- Build completes within budget; artifacts pass integrity checks.
- Downstream (measured in QueryKit/EvalKit): Recall@20, MRR, answer accuracy/citation correctness improve on the pilot slice.
